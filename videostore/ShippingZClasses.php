...
define("SHIPPINGZCLASSES_VERSION","2.0.0.47809");
...
################################################################################
	...
	class ShippingZGenericShoppingCart {
       ...
       ############### This will be involked for all commands except "ping" to check proper settings ##################
			...
			#######################################################################################################
			function array_field_to_xml( $fieldname, $array, $default = "" )
			{
				if(isset($array[ $fieldname ]))
				{
                                   if (is_numeric($default) && !is_numeric( $array[ $fieldname ] ))
					return $this->wrap_to_xml_cdata( $fieldname, $default );                                     
			...
			function order_items_xml( $order )
			{
				...
				for($prod_count=0; $prod_count < $order->num_of_products; $prod_count++)
				{
					   ...
					   $this->array_field_to_xml( 'Price', $order->order_product[$prod_count], 0 ) .
					   $this->array_field_to_xml( 'ExternalID', $order->order_product[$prod_count] ) .
					   $this->array_field_to_xml( 'Quantity' ,$order->order_product[$prod_count], 0 ) . 
					    $this->array_field_to_xml( 'Notes' ,$order->order_product[$prod_count] ) . 
					   $this->array_field_to_xml( 'Total' , $order->order_product[$prod_count], 0 );
			...
			function order_info_xml( $order )
			{
				...
				return 
					   ...
					   $this->array_field_to_xml( 'ItemsTotal', $order->order_info, 0 ) . 
					   $this->array_field_to_xml( 'Total', $order->order_info, 0 ) .
					   $this->array_field_to_xml( 'ShippingChargesPaid' , $order->order_info, 0 ) . 
					    $this->array_field_to_xml( 'ShipMethod' , $order->order_info ) . 
					   $this->array_field_to_xml( 'ItemsTax' , $order->order_info, 0 ) .
			...
			function MakeXMLSafe ($strin) 
			{
				...
				else
				{
					for ($i = 0; $i < strlen($strin); $i++) 
					{
							...
							if ((($ord > 0 && $ord < 32) || ($ord >= 127))&&($strin[$i]!="'")&&($strin[$i]!="‘")&&($strin[$i]!="’")) {
									$strout .= "&amp;#{$ord};";
							}
							else 
							{
