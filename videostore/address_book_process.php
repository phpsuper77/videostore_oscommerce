...
// error checking when updating or adding an entry
  ...
  $error = false;
	...
	// -Country-State Selector
	    if ($error == false) {
      $sql_data_array = array('entry_firstname' => strtoupper($firstname),
...
// reregister session variables
  ...
  /*if (isset($HTTP_GET_VARS['edit']) && is_numeric($HTTP_GET_VARS['edit'])) {*/
     ...
     if ($refresh) {
       ...
       if (ACCOUNT_COMPANY == 'true') $entry['entry_company'] = strtoupper($company);
       if (ACCOUNT_SUBURB == 'true') $entry['entry_suburb'] = strtoupper($suburb);
       }
     else
  // -Country-State Selector
    if (isset($HTTP_GET_VARS['edit']) && is_numeric($HTTP_GET_VARS['edit'])) {
