  ...
  if ($HTTP_GET_VARS['action']) {
    switch ($HTTP_GET_VARS['action']) {
      case 'update':
        ...
        if (intval($affiliate_order_notification)==1)
		$affiliate_order_notification="Y";
	else
		$affiliate_order_notification="N";		
	$affiliate_notification_email = tep_db_prepare_input($HTTP_POST_VARS['affiliate_notification_email']);
	$affiliate_email_code = tep_db_prepare_input($HTTP_POST_VARS['affiliate_email_code']);

	#####ADDED NEWLY####
        $affiliate_coupon=tep_db_prepare_input($HTTP_POST_VARS['affiliate_coupon']);

	//$affiliate_logo=tep_db_prepare_input($HTTP_POST_VARS['affiliate_logo']);

	if (is_uploaded_file($_FILES['affiliate_logo']['tmp_name']))
	{
		$name = $_FILES['affiliate_logo']['name'];
		$affiliate_logo = $name;
		move_uploaded_file($_FILES['affiliate_logo']['tmp_name'], "../images/" . $name);
	}
	else
		$affiliate_logo=$HTTP_POST_VARS['old_affiliate_logo'];


	if ($HTTP_POST_VARS['affiliate_display_logo'] == "on")
		$affiliate_display_logo='Y';
	else
		$affiliate_display_logo='N';

	if ($HTTP_POST_VARS['affiliate_display_coupon_box'] == "on")
	    $affiliate_display_coupon_box = 'Y';
	else
	    $affiliate_display_coupon_box = 'N';
	####################
...
// If someone uses , instead of .
        ...
        $sql_data_array = array('affiliate_firstname' => $affiliate_firstname,
                                ...
                                'affiliate_agb' => '1',
                                'affiliate_coupon' => $affiliate_coupon,  //ADDED NEWLY
		'affiliate_logo' => $affiliate_logo,  //ADDED NEWLY
		'affiliate_order_notification' => $affiliate_order_notification,  //ADDED NEWLY
		'affiliate_notification_email' => $affiliate_notification_email,  //ADDED NEWLY
		'currency' => $currency,  
		'affiliate_email_code' => $affiliate_email_code,  //ADDED NEWLY
                                'affiliate_display_logo' => $affiliate_display_logo, //ADDED NEWLY
		'affiliate_display_coupon_box' => $affiliate_display_coupon_box); //ADDED NEWLY
...
?>
      ...
      <tr>
        <td class="formArea"><table border="0" cellspacing="2" cellpadding="2">
          ...
          <tr>
			<td class="main"><?php echo ENTRY_AFFILIATE_COUPON; ?></td>
			<td class="main"><?php echo tep_draw_input_field('affiliate_coupon', $aInfo->affiliate_coupon,'maxlength="64"'); ?> <A href="javascript:popupWindow('affiliate_validcoupons.php')"><b><span class="main">Click Here</span></b></a> to view the coupons</td>
          </tr>
          <tr>
<input type="hidden" name="old_affiliate_logo" value="<?=$aInfo->affiliate_logo?>" />
<?
if ($aInfo->affiliate_logo=="") $aInfo->affiliate_logo = "spacer.gif";
?>
			<td class="main" colspan="2">
	<img src="../images/<?=$aInfo->affiliate_logo?>" border="0"/>
			</td>
          </tr>
          <tr>
			<td class="main"><?php echo ENTRY_AFFILIATE_LOGO; ?></td>
			<td class="main"><?php echo tep_draw_file_field('affiliate_logo'); ?>&nbsp;&nbsp;&nbsp;The name of the image file should be stored in ./images/</td>
          </tr>
          <tr>
			<td class="main"><?php echo ENTRY_AFFILIATE_DISPLAY_LOGO; ?></td>
			<?php
				if ($aInfo->affiliate_display_logo == 'Y')
					echo "<td class=\"main\"><input type=checkbox name=affiliate_display_logo CHECKED></td>";
				else
					echo "<td class=\"main\"><input type=checkbox name=affiliate_display_logo></td>";
			?>
          </tr>

          <tr>
		<td class="main"><?php echo ENTRY_AFFILIATE_COUPON_BOX; ?></td>
		<?php
			if ($aInfo->affiliate_display_coupon_box == 'Y')
				echo "<td class=\"main\"><input type=checkbox name=affiliate_display_coupon_box CHECKED></td>";
			else
				echo "<td class=\"main\"><input type=checkbox name=affiliate_display_coupon_box></td>";
		?>
          </tr>
          <tr>
		<td class="main">Order Notification</td>
		<?php
			if ($aInfo->affiliate_order_notification == 'Y')
				echo "<td class=\"main\"><input type=checkbox name=affiliate_order_notification value=1 CHECKED></td>";
			else
				echo "<td class=\"main\"><input type=checkbox name=affiliate_order_notification value=1></td>";
		?>
          </tr>
          <tr>
			<td class="main">Notification Email</td>
			<td class="main"><?php echo tep_draw_input_field('affiliate_notification_email', $aInfo->affiliate_notification_email,'maxlength="64"'); ?> </td>
          </tr>
<?
if ($aInfo->affiliate_email_code=='')
	$aInfo->affiliate_email_code= 'S';
?>
          <tr>
			<td class="main">Email Code</td>
			<td class="main"><input type="text" name="affiliate_email_code" value="<?=$aInfo->affiliate_email_code?>" size=1 maxlength=1 />&nbsp; ("D" Detail or "S" Summary) </td>
          </tr>
<?
if ($aInfo->currency=='')
	$aInfo->currency= 'USD';
...
?>
          <tr>
			<td class="main">Default Currency</td>
			<td class="main"><?=tep_draw_pull_down_menu('currency', $currencies_array, $aInfo->currency)?></td>
          </tr>
