          ...
          <tr>
            <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr class="dataTableHeadingRow">
                ...
                <td class="dataTableHeadingContent"><?php echo "Country"; ?></td>
...
<?php
  if ($affiliate_clickthroughs_numrows > 0) {
    ...
    while ($affiliate_clickthroughs = tep_db_fetch_array($affiliate_clickthroughs_values)) {
      ...
      ######ADDED FOR IP FLAG
      $aff_ip = $affiliate_clickthroughs['affiliate_clientip'];
	  $country_aff_query  = "SELECT countrySHORT, countryLONG FROM ipcountry WHERE ipFROM<=inet_aton('$aff_ip') AND ipTO>=inet_aton('$aff_ip') ";
	  $country_aff_exec = tep_db_query($country_aff_query);
	  $ccode_aff_array = tep_db_fetch_array($country_aff_exec);
	  $country_aff_code=$ccode_aff_array['countrySHORT'];
	  $country_aff_name=$ccode_aff_array['countryLONG'];
	  $country_aff_code = strtoupper($country_aff_code);

	  $country_flag_query ="select FLAG_NAME from ip2flag where COUNTRY_CODE2='$country_aff_code'";
	  $country_flag_exec = tep_db_query($country_flag_query);
	  $ccode_flag_array = tep_db_fetch_array($country_flag_exec);
	  $aff_flagname = $ccode_flag_array['FLAG_NAME'];
	  $caps_letter = strtoupper(substr($aff_flagname,0,2));
	  $aff_flagname = "$caps_letter.gif";

	  if(isset($aff_flagname)){
	  $aff_flagname = "images/flags/".$aff_flagname;
	  }
...
?>
                ...
                <td class="dataTableContent" align="center"><?php echo tep_datetime_short($affiliate_clickthroughs['affiliate_clientdate']); ?></td>
                <td class="dataTableContent"><?php echo $country_aff_name;?>&nbsp;<img src="<?php echo $aff_flagname;?>"></td>
<?php
...
?>
              ...
              <tr>
                ...
                <td class="dataTableContent" colspan="4"><?php  echo $affiliate_clickthroughs['affiliate_clientreferer']; ?></td>
              </tr>
	       <tr>
                <td class="dataTableContent" colspan="5"><?php echo tep_draw_separator('pixel_black.gif', '100%', '1'); ?></td>
              </tr>
<?php
    }
  } else {
