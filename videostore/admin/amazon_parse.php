    ...
    <td width="100%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            <td class="pageHeading">AMAZON FILE PARSER:</td>
...
//mail('x0661t@d-net.kiev.ua', 'TRAVELVIDEOSTORE.COM Amazon Marketplace Order Confirmation', $email_body, $headers);
...
<?
$next_po = tep_db_fetch_array(tep_db_query("select order_id from parse_tmp where currency='US' limit 1"));
if ($next_po[order_id]!='') {
?>
<input type="button" value="Proceed Next Order >>" onclick="window.location.href='amazon_parse.php?action=next&po=<?=$next_po[order_id]?>'" />
	<?
} 
else  echo "<b>All orders have been parsed...</b>&nbsp;&nbsp;<input type='button' value='Start New Parsing >>' onclick='window.location.href=\"amazon_parse.php\"'/>";
}
...
<form action="amazon_parse.php" method="post">
...
$cc = tep_db_num_rows(tep_db_query("select order_id from parse_tmp where currency='US'"));
if ($cc!=0)
	showInterface($first_po);
else
	echo "<script>window.location.href='amazon_parse.php?msg=All orders already in database!'</script>";
}
...
<form action="amazon_parse.php" method="post" enctype="multipart/form-data" name="parse">
...
<?
	$tmp = tep_db_query("select * from parse_tmp where currency='US'");
...
if ($count!=0){
	...
	$new_tmp = tep_db_query("select order_id from parse_tmp where currency='US'");
	$cnt = tep_db_num_rows($new_tmp);

	if ($cnt!=0){
	$row = tep_db_fetch_array($new_tmp);
	echo "<tr><td>Not all orders have been parsed, press <input type='button' value='Proceed parsing >>' onclick='window.location.href=\"amazon_parse.php?action=next&po=".$row[order_id]."\"'> to continue...</td></tr>";
	}
}
?>
</table>
