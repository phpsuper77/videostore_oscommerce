<?php
	...
	$query = "SELECT  p.products_always_on_hand , p.products_isbn , p.products_upc , p.products_asin , p.products_model , p.products_price , p.products_distribution,"
	              ."  p.products_id , p.products_byline , pd.products_name_prefix , p.products_image_lrg , "
	              ." pd.products_name , pd.products_description , pd.products_name_suffix , pd.products_clip_url , p.products_quantity , p.products_status "
	        ."FROM products p , products_description pd  "
	        ."WHERE p.products_id = pd.products_id AND ( ( p.products_quantity >0 ) OR ( p.products_quantity =0 AND p.products_always_on_hand =1 ) OR ( p.products_quantity <0 AND p.products_always_on_hand =1 ) OR (p.products_distribution =1) ) and p.products_status=1";
	...
	while($result_row = mysql_fetch_array($result))
	{
       //special price
		...
		if($result_row['products_upc']!="")
		{
			...
			else if($result_row['products_asin']!="")
		{
...
$total_qua = $result_row['products_quantity']+500;
		...
		echo "More Travel Videos to More Places - More Video Descriptions and Clips - Ships in 24 hours or less - all items In-Stock\t";
		echo "22\t";
		echo "n\n";
	}
?>