    ...
    <td width="100%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            <td class="pageHeading">AMAZON UK FILE PARSER:</td>
  ...
  $sql_data_array = array('customers_id' => $customer['customers_id'],
			  ...
			  'billing_company' => $billing_company, 
	                  ...
	                  'billing_address_format_id' => '2',
                          'payment_method' => 'Amazon UK Marketplace',
                          'cc_type' => '',
                          'cc_owner' => '',
                          'cc_number' => '',
                          'cc_expires' => '',
...
if (trim($sel_prod[series_name])!="") $series = "<b>".$sel_prod[series_name]."</b> - ";
    $products_body .= "<tr><td width='150'>".$products_qty[$i]."</td><td width='450'>".$series.$sel_prod[products_name_prefix]." ".$sel_prod[products_name]." ".$sel_prod[products_name_suffix]."(".$sel_prod[products_model].") = &pound;".($products_price[$i]*$products_qty[$i])."(GBP)</td></tr>";
...
$bottom_email_text = '';
	...
	for ($i=0;$i<3;$i++){
		...
		if ($i==0) {
			$bottom_email_text .= "<b>Shipping speed: ".$order_type."</b> &pound;".$one."(GBP)<br>";
			tep_db_query("insert into orders_total set orders_id='".$insert_id."', title='<b>".$order_type."</b>', text='".$currencies->format($shipping_credits,true,'GBP', '1.000000')."', value='".($one/$rt[value])."', class='ot_shipping', sort_order=1");
		}
		if ($i==1) {
			$bottom_email_text .= "<b>Sub-Total:</b> $".$two."(GBP)<br>";
			tep_db_query("insert into orders_total set orders_id='".$insert_id."', title='<b>Sub-Total:</b>', text='".$currencies->format($total_price, true,'GBP','1.000000')."', value='".($two/$rt[value])."', class='ot_subtotal', sort_order=2");
		}
		if ($i==2) {
			$bottom_email_text .= "<b>Total:</b> $".$three."(GBP)<br>";
			tep_db_query("insert into orders_total set orders_id='".$insert_id."', title='<b>Total:</b>', text='<b>".$currencies->format($tot, true, 'GBP','1.00000')."</b>', value='".($three/$rt[value])."', class='ot_total', sort_order=3");
		}
	}
...
if (trim($po_number)!='') $email_body .= "AMAZON UK MARKETPLACE ORDER NUMBER: ".$po_number."<br>";
/*
...
//mail($customer[customers_email_address], 'TRAVELVIDEOSTORE.COM Amazon UK Marketplace Order Confirmation', $email_body, $headers);
mail('amazonuksales@travelvideostore.com', 'TRAVELVIDEOSTORE.COM Amazon UK Marketplace Order Confirmation', $email_body, $headers);
//mail('donwyatt@travelvideostore.com', 'TRAVELVIDEOSTORE.COM Amazon UK Marketplace Order Confirmation', $email_body, $headers);
...
<?
$next_po = tep_db_fetch_array(tep_db_query("select order_id from parse_tmp where currency='UK' limit 1"));
if ($next_po[order_id]!='') {
?>
<input type="button" value="Proceed Next Order >>" onclick="window.location.href='amazonuk_parse.php?action=next&po=<?=$next_po[order_id]?>'" />
	<?
} 
else  echo "<b>All orders have been parsed...</b>&nbsp;&nbsp;<input type='button' value='Start New Parsing >>' onclick='window.location.href=\"amazonuk_parse.php\"'/>";
}
...
<form action="amazonuk_parse.php" method="post">
...
<tr>
	...
	<table>
		...
		<tr><td>Name: </td><td><input type="text" name="billing_name" value="Amazon UK Marketplace Orders" /></td></tr>
...
$comments_string .="Buyer's price: &pound;".$rr[total_price]."(GBP)\n\n";
}

$comments_string .="\n\n\n";
$comments_string .="Buyer's Email: ".$new_customer[buyer_email]."\n";
$comments_string .="Time of Sale: ".$new_customer[purchase_date]."\n";
$comments_string .="UK Shipping Speed: ".$new_customer[shipping_method]."\n";
$comments_string .="\n\n";
...
$cc = tep_db_num_rows(tep_db_query("select order_id from parse_tmp where currency='UK'"));
if ($cc!=0)
	showInterface($first_po);
else
	echo "<script>window.location.href='amazonuk_parse.php?msg=All orders already in database!'</script>";
}
...
<form action="amazonuk_parse.php" method="post" enctype="multipart/form-data" name="parse">
...
<?
	$tmp = tep_db_query("select * from parse_tmp where currency='UK'");
...
if ($count!=0){
	...
	$new_tmp = tep_db_query("select order_id from parse_tmp where currency='UK'");
	$cnt = tep_db_num_rows($new_tmp);

	if ($cnt!=0){
	$row = tep_db_fetch_array($new_tmp);
	echo "<tr><td>Not all orders have been parsed, press <input type='button' value='Proceed parsing >>' onclick='window.location.href=\"amazonuk_parse.php?action=next&po=".$row[order_id]."\"'> to continue...</td></tr>";
	}
}
?>
</table>
