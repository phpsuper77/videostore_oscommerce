<?php
	...
	$curr = tep_db_fetch_array(tep_db_query("select * from currencies where currencies_id='5'"));
	
	$query = "SELECT  p.products_always_on_hand, p.products_isbn , p.products_upc , p.products_asin ,p.products_model , p.products_price ,"
	              ."  p.products_id , p.products_byline ,pd.products_name_prefix ,p.products_image_lrg , "
	              ." pd.products_name , pd.products_description ,pd.products_name_suffix , pd.products_clip_url, p.products_quantity "
	        ."FROM products p , products_description pd  "
	        ."WHERE p.products_id = pd.products_id AND ( ( p.products_quantity >0 ) OR ( p.products_quantity =0 AND p.products_always_on_hand =1 ) OR ( p.products_quantity <0 AND p.products_always_on_hand =1 ) ) and p.products_upc!='' and p.products_upc!='0'";
...
//			echo $sp_result_row['specials_new_products_price' ] * $curr[value] * 1.00 ."\t";
                                                     echo round($sp_result_row['specials_new_products_price' ] * $curr[value] * 1.00, 2) ."\t";

		}
		else
		{
//			echo $result_row['products_price'] * $curr[value] * 1.00 ."\t";
			echo round($result_row['products_price'] * $curr[value] * 1.00, 2) ."\t";
...
{
$total_qua = $result_row['products_quantity']+200;
		echo $total_qua."\t";		
}
		else
		echo $result_row['products_quantity']."\t";
/*
if (tep_not_null($result_row['products_clip_url'])) {
echo " More Travel Videos to More Places - View a video clip at " . $result_row['products_clip_url'] . "\t";}
else {
*/
		echo "More Travel Videos to More Places - More Video Descriptions and Sample Clips - Ships in 24 hours or less - all items In-Stock - Daily Shipping from the USA\t";
/* }  */
		echo "6\t";
		echo "\t";
		echo "n\n";
