...
// Original edit to v1.3 by V. Meurink AKA MaDBuCe - oscommerce@planet.nl

// Additional changes to code in banner_manager.php to make it compatible
// with osCommerce 2.2MS2
// Edit by Priest Reynolds - priestreynolds@hotmail.com
// Banner Manager v1.4
  ...
  if (tep_not_null($action)) {
    switch ($action) {
      ...
      case 'update':
        ...
        $db_image_location = '';
// WebMakers.com Added - Optional new window 
        $banners_open_new_windows = tep_db_prepare_input($HTTP_POST_VARS['banners_open_new_windows']);
// WebMakers.com Added - Optional Show on SSL		
...
// WebMakers.com Added - Optional new window and Show on SSL
      $banner_query = tep_db_query("select banners_title, banners_url, banners_image, banners_group, banners_html_text, status, date_format(date_scheduled, '%d/%m/%Y') as date_scheduled, date_format(expires_date, '%d/%m/%Y') as expires_date, expires_impressions, date_status_change, banners_open_new_windows, banners_on_ssl from " . TABLE_BANNERS . " where banners_id = '" . tep_db_input($bID) . "'");
      $banner = tep_db_fetch_array($banner_query);
...
// BOF: WebMakers.com Added - Optional new window 
    switch ($bInfo->banners_open_new_windows) {
      case '0': $win_status_new = false; $win_status_same = true; break;
      case '1':
      default: $win_status_new = true; $win_status_same = false;
    }
// EOF: WebMakers.com Added - Optional new window 

// BOF: WebMakers.com Added - Optional Show on SSL
    switch ($bInfo->banners_on_ssl) {
      case '0': $banners_ssl_on = false; $banners_ssl_off = true; break;
      case '1':
      default: $banners_ssl_on = true; $banners_ssl_off = false;
    }
// EOF: WebMakers.com Added - Optional new window
...
?>
          <tr>
            <td class="main"><?php echo 'Open Window In:'; ?></td>
            <td class="main" align="left"><?php echo '&nbsp;' . tep_draw_radio_field('banners_open_new_windows', '1', $win_status_new) . '&nbsp;' . 'New Window' . '&nbsp;&nbsp;&nbsp;' . tep_draw_radio_field('banners_open_new_windows', '0', $win_status_same) . '&nbsp;' . 'Same Window'; ?></td>
          </tr>
          <tr>
...
?>
          <tr>
            <td class="main"><?php echo 'Show on SSL:'; ?></td>
            <td class="main" align="left"><?php echo '&nbsp;' . tep_draw_radio_field('banners_on_ssl', '1', $banners_ssl_on) . '&nbsp;' . 'YES' . '&nbsp;&nbsp;&nbsp;' . tep_draw_radio_field('banners_on_ssl', '0', $banners_ssl_off) . '&nbsp;' . 'NO'; ?></td>
          </tr>
          <tr>
            <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
          </tr>
<?php
// EOF: WebMakers.com Added - Optional Show on SSL 
?>		
          <tr>
