...
$flag = $_GET[flag]?$_GET[flag] : $_POST[flag];
$ref = isset($HTTP_REFERER) ? $HTTP_REFERER : $_SERVER['HTTP_REFERER'];

if ($flag == 1) $_SESSION['ref_link'] = htmlentities(urlencode($ref));
...
// END CATEGORIES DESCRIPTION v1.5
      ...
      ###################CHANGES MADE BY RAMESH FOR ENABLE/DISABLE BUTTON WHILE UPDATING A CATEGORY####
        if (isset($HTTP_POST_VARS['categories_status']))
      {
      ...
      // ####################### End Added Categories Enable / Disable ###############
          ...
          if ($action == 'insert_category') {
           ...
           $categories_id = tep_db_insert_id();
            ...
            $update_sql_data = array('last_modified' => 'now()', 'brochure_image'=>$HTTP_POST_VARS['brochure_image']);
...
//REMOVED          }
        ...
        if ($categories_image = new upload('categories_image', DIR_FS_CATALOG_IMAGES)) {
          tep_db_query("update " . TABLE_CATEGORIES . " set categories_image = '" . tep_db_input($categories_image->filename) . "' where categories_id = '" . (int)$categories_id . "'");
            ...
            tep_db_query("update " . TABLE_CATEGORIES . " set brochure_image = '" . strip_tags($HTTP_POST_VARS['brochure_image']) . "' where categories_id = '" .  tep_db_input($categories_id) . "'");
...
// END CATEGORIES DESCRIPTION v1.5
          if (USE_CACHE == 'true') {
            ...
            require_once("includes/addons/seo_urls/seo_urls_categories_insert_0.php");
...
// removing categories can be a lengthy process
      ...
      case 'delete_product_confirm':
        if (isset($HTTP_POST_VARS['products_id']) && isset($HTTP_POST_VARS['product_categories']) && is_array($HTTP_POST_VARS['product_categories'])) {
          ...
          $result = tep_db_query("delete from ".TABLE_PRODUCTS_TO_VENDORS." where products_id = '".(int)$products_id ."'"); //vendors
        }
...
// BOF MaxiDVD: Modified For Ultimate Images Pack!
            ...
            if ($HTTP_POST_VARS['delete_cover_image'] == 'yes') {
                unlink(DIR_FS_CATALOG_IMAGES . $HTTP_POST_VARS['previous_cover_image']);
            }

            if ($HTTP_POST_VARS['delete_brochure_image'] == 'yes') {
                unlink(DIR_FS_CATALOG_IMAGES . $HTTP_POST_VARS['previous_brochure_image']);
            }
...
// EOF MaxiDVD: Modified For Ultimate Images Pack!
          ...
          'customerflix_id' => tep_db_prepare_input($HTTP_POST_VARS['customerflix_id']),
          'has_rights' => tep_db_prepare_input($HTTP_POST_VARS['has_rights']),
          'products_distribution' => tep_db_prepare_input($HTTP_POST_VARS['products_distribution']),
      ...
      'products_always_on_hand' => tep_db_prepare_input($HTTP_POST_VARS['products_always_on_hand']),
      ...
      'products_youtube_id' => tep_db_prepare_input($HTTP_POST_VARS['products_youtube_id']),
      'adgregate' => tep_db_prepare_input($HTTP_POST_VARS['adgregate'])
      );
...
// BOF MaxiDVD: Modified For Ultimate Images Pack!
       if (($HTTP_POST_VARS['unlink_image'] == 'yes') or ($HTTP_POST_VARS['delete_image'] == 'yes')) {
         ...
         if (isset($HTTP_POST_VARS['products_image']) && tep_not_null($HTTP_POST_VARS['products_image']) && ($HTTP_POST_VARS['products_image'] != 'none')) {
            $sql_data_array['products_image'] =tep_db_prepare_input(trim($HTTP_POST_VARS['products_image']));
       ...
       if (($HTTP_POST_VARS['unlink_cover_image'] == 'yes') or ($HTTP_POST_VARS['delete_cover_image'] == 'yes')) {
            $sql_data_array['cover_image'] = '';
           } else {
          if (isset($HTTP_POST_VARS['cover_image']) && tep_not_null($HTTP_POST_VARS['cover_image']) && ($HTTP_POST_VARS['cover_image'] != 'none')) {
            $sql_data_array['cover_image'] = tep_db_prepare_input($HTTP_POST_VARS['cover_image']);
          }
          }

       if (($HTTP_POST_VARS['unlink_brochure_image'] == 'yes') or ($HTTP_POST_VARS['delete_brochure_image'] == 'yes')) {
            $sql_data_array['brochure_image'] = '';
           } else {
          if (isset($HTTP_POST_VARS['brochure_image']) && tep_not_null($HTTP_POST_VARS['brochure_image']) && ($HTTP_POST_VARS['brochure_image'] != 'none')) {
            $sql_data_array['brochure_image'] = tep_db_prepare_input($HTTP_POST_VARS['brochure_image']);
          }
          }
...
// EOF MaxiDVD: Modified For Ultimate Images Pack!
          ...
          if ($action == 'insert_product') {
            ...
            //tep_db_query("insert into " . TABLE_PRODUCTS_TO_CATEGORIES . " (products_id, categories_id) values ('" . (int)$products_id . "', '" . (int)$current_category_id . "')");
                ...
                {
                  if(strstr($key,"categoriesid"))
                  {
                        $ins_ptoc = tep_db_query("insert into products_to_categories (products_id, categories_id) values ('" . (int)$products_id . "', '" . $value. "')");
                        $cat_post[]=$value;
                  }
                }


            //multiple vedors -- VENDORS
      foreach($HTTP_POST_VARS as $key=>$value)
      {
        if(strstr($key,"vendors"))
        $ins_qry = tep_db_query("insert into ".TABLE_PRODUCTS_TO_VENDORS." (products_id,vendors_id) values ('" . (int)$products_id . "', '" . $value. "')");
      }
            ...
            //multiple categories
    $result_del = tep_db_query("delete from ".TABLE_PRODUCTS_TO_CATEGORIES." where products_id = '".(int)$products_id ."'");
    while(list($key,$value)=each($HTTP_POST_VARS))
    {
      if(strstr($key,"categoriesid"))
      {
    ...
    $current_vendors = tep_db_query("select vendors_id from ".TABLE_PRODUCTS_TO_VENDORS." where products_id = '".(int)$products_id ."'");
    while ($row = tep_db_fetch_array($current_vendors)) $old_current[] = $row[vendors_id];
    ...
    if (is_array($old_current)) {
      foreach($old_current as $key=>$value){
         if (@array_search($value, $new_vendors) === false)  tep_db_query("delete from products_to_vendors where products_id='".$products_id."' and vendors_id='".$value."'");
      }
    }



    if (is_array($new_vendors)) {
        foreach($new_vendors as $key=>$value)
        {
// DW removed this if statement as if a product did not have a vendor it would not add one
//          if (@array_search($value, $old_current) === false) 
            tep_db_query("insert ignore into ".TABLE_PRODUCTS_TO_VENDORS." (products_id,vendors_id) values ('" . (int)$products_id . "', '" . $value. "')");
                       }
    }
        ...
        //  $category.=",".$value;
        //}
            //tep_db_query("delete from products_to_categories where products_id = '".(int)$products_id ."' and categories_id not in(".substr($category,"1").")");

                //  $value=0;
                //  $result = tep_db_fetch_array(tep_db_query("select products_id,categories_id from products_to_categories where products_id = '".(int)$products_id ."' and categories_id ='" . (int)0 . "'"));
        //  if(!($result['products_id']))
              //      tep_db_query("insert into products_to_categories (products_id, categories_id) values ('" . (int)$products_id . "', '" . $value. "')");

      //special price

        $special_array=array('specials_new_products_price' => tep_db_prepare_input($HTTP_POST_VARS['specials_new_products_price']));
        if($special_array['specials_new_products_price'] && $special_array['specials_new_products_price']!='0.0000')
        {
                  $result = tep_db_fetch_array(tep_db_query("select products_id from specials where  products_id ='" . (int)$products_id . "'"));
          if($result['products_id'])
            tep_db_perform("specials", $special_array, 'update', "products_id = '" . (int)$products_id . "'");
          else
                    tep_db_query("insert into specials (products_id, specials_new_products_price) values ('" . (int)$products_id . "', '" . tep_db_prepare_input($HTTP_POST_VARS['specials_new_products_price'] ). "')");
        }
        else
                  tep_db_query("Delete from specials where  products_id ='" . (int)$products_id . "'");
            ...
            $sql_data_array = array('products_name' => tep_db_prepare_input($HTTP_POST_VARS['products_name'][$language_id]),
                  ...
                  'products_head_title_tag' => tep_db_prepare_input($HTTP_POST_VARS['products_head_title_tag'][$language_id]),
                      ...
                      'products_head_keywords_tag' => tep_db_prepare_input($HTTP_POST_VARS['products_head_keywords_tag'][$language_id]),
                                    ...
                                    'products_clip_url' => tep_db_prepare_input($HTTP_POST_VARS['products_clip_url']),
                                    'products_clip_name' => tep_db_prepare_input($HTTP_POST_VARS['products_clip_name']),
                                    'products_buy_download_link' => tep_db_prepare_input($HTTP_POST_VARS['products_buy_download_link']));
            require_once("includes/addons/seo_urls/seo_urls_categories_insert_3.php");
    ...
    //      $value=0;
      ...
      //    if(!($result['products_id']))
        ...
        if($special_array['specials_new_products_price'] && $special_array['specials_new_products_price']!='0.0000')
        {
          $special_array=array('products_id' => $products_id,
                   'specials_new_products_price' => tep_db_prepare_input($HTTP_POST_VARS['specials_new_products_price']));
          tep_db_perform("specials", $special_array);
        }
	...
	//echo $_SESSION[ref_link];	  die('ok');
		  
if ($_SESSION[ref_link] != ''){
    $ret = $_SESSION[ref_link];
    $_SESSION[ref_link] = '';
    echo "<script>location.href='".html_entity_decode(urldecode($ret))."'</script>";
    exit;
  }
      ...
      case 'copy_to_confirm':
        ...
        while(list($key,$value1)=each($HTTP_POST_VARS))
        {
          if(strstr($key,"categoriesid"))
          {
          ...
          {
            if ($value != $current_category_id)
            {
              $result = tep_db_fetch_array(tep_db_query("select products_id,categories_id from products_to_categories where products_id = '".(int)$products_id ."' and categories_id ='" . (int)$value . "'"));
              if(!($result['products_id']))
                tep_db_query("insert into products_to_categories (products_id, categories_id) values ('" . (int)$products_id . "', '" . $value. "')");
            }
            else
...
// BOF MaxiDVD: Modified For Ultimate Images Pack!
            ...
            /////CHANGED
              ...
              tep_db_query("insert into " . TABLE_PRODUCTS_DESCRIPTION . " (products_id, language_id, products_name, products_name_prefix, products_name_suffix,  products_description, products_head_title_tag, products_head_desc_tag, products_head_keywords_tag, products_url) values ('" . (int)$dup_products_id . "', '" . (int)$description['language_id'] . "', '" . tep_db_input($description['products_name']) . "', '" . tep_db_input($description['products_name_prefix']) . "', '" . tep_db_input($description['products_name_suffix']) . "', '" . tep_db_input($description['products_description']) . "', '" . tep_db_input($description['products_head_title_tag']) . "', '" . tep_db_input($description['products_head_desc_tag']) . "', '" . tep_db_input($description['products_head_keywords_tag']) . "', '" . tep_db_input($description['products_url']) . "')");
        tep_db_query("insert into " . TABLE_PRODUCTS_DATA . " (products_id, language_id, products_viewed) values ('" . (int)$dup_products_id . "', '" . (int)$description['language_id'] . "', '0')"); //NEWLY INSERTED
             //////
            }

            tep_db_query("insert into " . TABLE_PRODUCTS_TO_CATEGORIES . " (products_id, categories_id) values ('" . (int)$dup_products_id . "', '" . (int)$value . "')");
...
// copy image only if modified
   ...
   if (($HTTP_POST_VARS['unlink_cover_image'] == 'yes') or ($HTTP_POST_VARS['delete_cover_image'] == 'yes')) {
        ...
        $cover_image = new upload('cover_image');
        $cover_image->set_destination(DIR_FS_CATALOG_IMAGES);
        if ($cover_image->parse() && $cover_image->save()) {
          $cover_image_name = $cover_image->filename;
        } else {
          $cover_image_name = (isset($HTTP_POST_VARS['previous_cover_image']) ? $HTTP_POST_VARS['previous_cover_image'] : '');
        }
        } else {
          if (isset($HTTP_POST_VARS['cover_image']) && tep_not_null($HTTP_POST_VARS['cover_image']) && ($HTTP_POST_VARS['cover_image'] != 'none')) {
            $cover_image_name = $HTTP_POST_VARS['cover_image'];
          } else {
            $cover_image_name = (isset($HTTP_POST_VARS['previous_cover_image']) ? $HTTP_POST_VARS['previous_cover_image'] : '');
          }
        }
   ...
   if (($HTTP_POST_VARS['unlink_brochure_image'] == 'yes') or ($HTTP_POST_VARS['delete_brochure_image'] == 'yes')) {
        ...
        $brochure_image = new upload('brochure_image');
        $brochure_image->set_destination(DIR_FS_CATALOG_IMAGES);
        if ($brochure_image->parse() && $brochure_image->save()) {
          $brochure_image_name = $brochure_image->filename;
        } else {
          $brochure_image_name = (isset($HTTP_POST_VARS['previous_brochure_image']) ? $HTTP_POST_VARS['previous_brochure_image'] : '');
        }
        } else {
          if (isset($HTTP_POST_VARS['brochure_image']) && tep_not_null($HTTP_POST_VARS['brochure_image']) && ($HTTP_POST_VARS['brochure_image'] != 'none')) {
            $brochure_image_name = $HTTP_POST_VARS['brochure_image'];
          } else {
            $brochure_image_name = (isset($HTTP_POST_VARS['previous_brochure_image']) ? $HTTP_POST_VARS['previous_brochure_image'] : '');
          }
        }
...
cd.categories_description, c.categories_image, c.brochure_image, c.parent_id, c.sort_order, c.date_added, c.last_modified from " . TABLE_CATEGORIES . "
...
cd.categories_id and cd.language_id = '" . $languages_id . "' order by c.sort_order, cd.categories_name");
      ...
      $brochure_image = strip_tags($HTTP_POST_VARS['brochure_image']);
    } else {
...
$cInfo->categories_image . tep_draw_hidden_field('categories_previous_image', $cInfo->categories_image); ?></td>
    ...
    <tr>
           <td class="main">Brochure Image</td>
     <td class="main">
      <textarea name="brochure_image" wrap="soft" cols="70" rows="2">
<?
if (strip_tags($HTTP_POST_VARS['brochure_image'])) 
  echo strip_tags($HTTP_POST_VARS['brochure_image']);
  else
  echo $cInfo->brochure_image;
?>
      </textarea>
...
<?php if ((HTML_AREA_WYSIWYG_DISABLE_JPSY == 'Enable') && (HTML_AREA_WYSIWYG_DISABLE == 'Enable')){ ?>
             ...
             editor_generate('brochure_image',config);
            <?php } ?>
            </script>
     </td>
          </tr>
    ...
    <td>
<?
 if (trim($cInfo->brochure_image) != "") echo "<img src='".DIR_WS_CATALOG_IMAGES.$cInfo->brochure_image."' alt='".$cInfo->brochure_image."' title='".$cInfo->brochure_image."' />";
?>
    </td>
    </tr>
          <tr>
...
if(!empty($cInfo->categories_id)) {
                    $seo_url_sql = "
                      ...
                      where
                        cd.categories_id = '" . $cInfo->categories_id . "' and su.language_id = '1'";
                        
                    // echo $seo_url_sql;
                        
                    $categories_query = tep_db_query($seo_url_sql);
                    $category_seo = tep_db_fetch_array($categories_query);
                    $category_seo_url = new objectInfo($category_seo);
                    
                    $categories_surls_id = $category_seo_url->surls_id;
                    $categories_surls_name = $category_seo_url->surls_name;
}
?>                    
                                  <tr>
                                          ...
                                          <td class="main">
                      <font style="font-size: 8pt; font-family: arial; font-weight: bold;">http://<?php echo $_SERVER["HTTP_HOST"];?>/</font><input type=text name="surls_name" value="<?php echo $categories_surls_name;?>" size=55>
                      <font style="font-size: 8pt; font-family: arial; font-weight: bold;">/</font>
                      <input type=hidden name="surls_id" value="<?php echo $categories_surls_id;?>">
                      </td>
...
cd.categories_description, c.categories_image, c.brochure_image, c.sort_order, c.date_added, c.last_modified from " . TABLE_CATEGORIES . " c, " .
...
SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'align="right" hspace="5" vspace="5"') . $cInfo->categories_description; ?></td>
      ...
      <tr>
        <td class="main">
<?php if (trim(strip_tags($HTTP_POST_VARS['brochure_image']))) { ?>
<img src="<?=DIR_WS_CATALOG_IMAGES.strip_tags($cInfo->brochure_image)?>" alt="<?=$cInfo->categories_name?>" title="<?=$cInfo->categories_name?>" />
<input type="hidden" name="brochure_image" value="<?=strip_tags($cInfo->brochure_image)?>" />
<? } ?>
      </tr>
...
/* Re-Post all POST'ed variables */
      ...
      while (list($key, $value) = each($HTTP_POST_VARS)) {
        ...
        else
        {
          ...
          {
            echo '<input type=hidden name="$key$i" value="$val">';
            $i++;
          }
        }
      }
...
htmlspecialchars(stripslashes($categories_description[$languages[$i]['id']])));
      ...
      echo tep_draw_hidden_field('brochure_image', stripslashes($brochure_image));    
...
// EOF MaxiDVD: Modified For Ultimate Images Pack!
      ...
      $spl_parameters = array('specials_new_products_price' => '');
      $catg_parameters = array('categories_id' => '');
      $vendors_parameters = array('vendors' => ''); //vendors

    $pInfo = new objectInfo($parameters);
    $sInfo = new objectInfo($spl_parameters);
    $cInfo = new objectInfo($catg_parameters);
    $vInfo = new objectInfo($vendors_parameters); //vendors

    if (isset($HTTP_GET_VARS['pID']) && empty($HTTP_POST_VARS)) {
// BOF MaxiDVD: Modified For Ultimate Images Pack!
$sql  = "select p.products_distribution, p.has_rights, p.customerflix_id, p.products_always_on_hand, pd.products_name, pd.products_clip_url, pd.products_buy_download_link, pd.products_clip_name, pd.products_name_prefix, pd.products_name_suffix, pd.products_description, pd.products_head_title_tag, pd.products_head_desc_tag, pd.products_head_keywords_tag, pd.products_url, p.products_id, p.products_quantity, p.products_model, p.products_image, p.products_image_med, p.products_image_lrg, p.cover_image, p.brochure_image, p.products_image_sm_1, p.products_image_xl_1, p.products_image_sm_2, p.products_image_xl_2, p.products_image_sm_3, p.products_image_xl_3, p.products_image_sm_4, p.products_image_xl_4, p.products_image_sm_5, p.products_image_xl_5, p.products_image_sm_6, p.products_image_xl_6, p.products_price,p.products_weight, p.products_date_added, p.products_last_modified, date_format(p.products_date_available, '%Y-%m-%d') as products_date_available, p.products_status, p.products_tax_class_id, p.manufacturers_id, p.distributors_id, p.producers_id, p.series_id, p.products_media_type_id, p.products_video_format_id, p.products_region_code_id, p.products_set_type_id, p.products_packaging_type_id, p.products_run_time, p.products_out_of_print, p.products_audio_id, p.products_aspect_ratio_id, p.products_warehouse_location, p.products_export_id, p.products_audio_languages, p.products_subtitle_languages, p.products_closed_captioned, p.products_pc_bonus, p.products_release_date, p.products_isbn, p.products_upc, p.products_asin, p.products_color, p.products_rated, p.products_byline, p.products_summary, p.products_youtube_id, p.adgregate 
      from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd /*, " . TABLE_SPECIALS . " s*/ where 
        p.products_id = '" . (int)$HTTP_GET_VARS['pID'] . "' 
        and p.products_id = pd.products_id 
        and pd.language_id = '" . (int)$languages_id . "'";

    $product_query = tep_db_query($sql);// EOF MaxiDVD: Modified For Ultimate Images Pack!

    $spl_price_query = tep_db_query("select specials_new_products_price from " . TABLE_SPECIALS . " where products_id ='".(int)$HTTP_GET_VARS['pID'] ."'");
    $catg_query = tep_db_query("select categories_id from products_to_categories where products_id ='".(int)$HTTP_GET_VARS['pID'] ."'");

    $vendors_query = tep_db_query("select vendors_id from ".TABLE_PRODUCTS_TO_VENDORS." where products_id ='".(int)$HTTP_GET_VARS['pID'] ."'"); //vendors
###
#Changed made here
###
      $product = tep_db_fetch_array($product_query);
      $specials = tep_db_fetch_array($spl_price_query);
      ...
      {
        $sInfo->objectInfo($specials);
    }
    else
    {
        $specials = array('specials_new_products_price' => '');
      $sInfo->objectInfo($specials);
    }

      if (isset($categories) && ($categories <> ''))
      {
        $cInfo->objectInfo($categories);
    }
    else
    {
      ...
      $cInfo->objectInfo($categories);
        ...
        //Vendors
        while($vendorsid=tep_db_fetch_array($vendors_query))
        {
          $vid[] = $vendorsid['vendors_id'];
        }

          if (isset($vendorsid) && ($vendorsid <> ''))
          {
            $vInfo->objectInfo($vendorsid);
        }
        else
        {
            $vendors = array('vendors' => '');
          $vInfo->objectInfo($vendors);
        }
        //Vendors
...
//Categories
  ...
  $categories_query = tep_db_query("select categories_id,categories_name from categories_description order by categories_name");
...
// ASPECT RATIO
  ...
  if (!isset($pInfo->products_always_on_hand)) $pInfo->products_always_on_hand = '1';
    switch ($pInfo->products_always_on_hand) {
      case '0': $always_1 = false; $always_2 = true; break;
      case '1': $always_1 = true; $always_2 = false; break;
      default: $always_1 = false; $always_2 = true;
    }

    switch ($pInfo->has_rights) {
      case '0': $hr_1 = false; $hr_2 = true; break;
      case '1':
      default: $hr_1 = true; $hr_2 = false;
    }

    switch ($pInfo->products_distribution) {
      case '0': $dr_1 = false; $dr_2 = true; break;
      case '1':
      default: $dr_1 = true; $dr_2 = false;
    }
...
//--></script>
    ...
    <table border="0" width="100%" cellspacing="0" cellpadding="2">
      ...
      <tr>
        <td><table border="0" cellspacing="0" cellpadding="2">
          <tr>
            <td class="main" width="250px"><?php echo TEXT_PRODUCTS_STATUS; ?></td>
 ...
 <tr>
    ...
    <tr>
            <td class="main">Customerflix_id</td>
            <td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_input_field('customerflix_id', $pInfo->customerflix_id);?></td>
          </tr>
      ...
      <tr>
            <td class="main">Always in catalog</td>
            <td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_radio_field('products_always_on_hand', '1', $always_1) . '&nbsp;' . 'YES' . '&nbsp;' . tep_draw_radio_field('products_always_on_hand', '0', $always_2) . '&nbsp;' . 'NO'; ?></td>
          </tr>
    <tr>
            <td class="main">Limited public performance rights</td>
            <td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_radio_field('has_rights', '1', $hr_1) . '&nbsp;' . 'YES' . '&nbsp;' . tep_draw_radio_field('has_rights', '0', $hr_2) . '&nbsp;' . 'NO'; ?></td>
          </tr>
    <tr>
            <td class="main">Products on Distribution</td>
            <td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_radio_field('products_distribution', '1', $dr_1) . '&nbsp;' . 'YES' . '&nbsp;' . tep_draw_radio_field('products_distribution', '0', $dr_2) . '&nbsp;' . 'NO'; ?></td>
          </tr>

            <td class="main"><?php echo TEXT_PRODUCTS_WEIGHT; ?></td>
            <td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_input_field('products_weight', $pInfo->products_weight); ?></td>
          </tr>
      <?php require_once("includes/addons/seo_urls/seo_urls_categories_insert_1.php");?>
          <tr>
...
?>
      ...
      <TR>
      <TD class="main">Products Clip Name:</TD>
      <TD>
      <?php  echo tep_draw_separator('pixel_trans.gif', '24', '15'). '&nbsp;' . tep_draw_input_field('products_clip_name', $pInfo->products_clip_name); ?></td>
      </TR>
      <TR>
      <TD class="main">Products Clip URL:</TD>
      <TD>
      <?php  echo tep_draw_separator('pixel_trans.gif', '24', '15'). '&nbsp;' . tep_draw_input_field('products_clip_url', $pInfo->products_clip_url); ?></td>
      </TR>
      <TR>
      <TD class="main">Products Download Link:</TD>
      <TD>
      <?php  echo tep_draw_separator('pixel_trans.gif', '24', '15'). '&nbsp;' . tep_draw_input_field('products_buy_download_link', $pInfo->products_buy_download_link); ?></td>
      </TR>
          ...
          <tr bgcolor="#ebebff">
            <td class="main"><?php echo "Products Price (Special)"; ?></td>

            <td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_input_field('specials_new_products_price', $sInfo->specials_new_products_price, 'OnKeyUp=""'); ?></td>
          </tr>
<script language="javascript"><!--
updateGross();
//--></script>
...
?>
      ...
      <TD class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_input_field('products_subtitle_languages', $pInfo->products_subtitle_languages); ?>
       ...
       <TD class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_input_field('products_asin', $pInfo->products_asin); ?>
       </TD>
      </TR>
          ...
          <tr>
            <td class="main">Product Adgregate</td>
            <td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_input_field('adgregate', $pInfo->adgregate); ?></td>
          </tr>
          
          <tr>
            <td class="main">Product Youtube Id</td>
            <td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_input_field('products_youtube_id', $pInfo->products_youtube_id); ?></td>
          </tr>
      ...
      $query=tep_db_query("select * from products_to_allied where product_id ='".(int)$HTTP_GET_VARS['pID'] ."'");
$datarow=tep_db_fetch_array($query);?>

<tr height="10"></tr>
         <tr>
            <td class="main">Product ID</td>
            <td class="main"><input type="text" name="temp_product_id" value="<?php echo $datarow['product_id'];?>"></input></td>
          </tr> 
          
         <tr>
            <td class="main">Product Set Total Disk</td>
            <td class="main"><input type="text" name="temp_product_set_total_disks" value="<?php echo $datarow['product_set_total_disks'];?>"></input></td>
          </tr>
          <tr>
            <td class="main">Product Set Total Disk</td>
            <td class="main"><input type="text" name="temp_product_set_total_disks" value="<?php echo $datarow['product_set_total_disks'];?>"></input></td>
          ...
          <tr>
            <td class="main">Product Media Definition</td>
            <td class="main"><input type="text" name="temp_product_media_definition"value="<?php echo $datarow['product_media_definition'];?>"></input></td>
          </tr>
          <tr>
            <td class="main">Product Disk One Iso</td>
            <td class="main"><input type="text" name="temp_product_disk_one_iso"value="<?php echo $datarow['product_disk_one_iso'];?>"></input></td>
          </tr>
          <tr>
            <td class="main">Product Disk One Wrap</td>
            <td class="main"><input type="text" name="temp_product_disk_one_wrap"value="<?php echo $datarow['product_disk_one_wrap'];?>"></input></td>
          </tr>
          <tr>
            <td class="main">Product Disk One Disclabel</td>
            <td class="main"><input type="text" name="temp_product_disk_one_disclabel"value="<?php echo $datarow['product_disk_one_disclabel'];?>"></input></td>
          </tr>
           <tr>
            <td class="main">Product Disk One Silkscreen</td>
            <td class="main"><input type="text" name="temp_product_disk_one_silkscreen"value="<?php echo $datarow['product_disk_one_silkscreen'];?>"></input></td>
          </tr>
           <tr>
            <td class="main">Product Disk One CdPackage</td>
            <td class="main"><input type="text" name="temp_product_disk_one_cdpackage"value="<?php echo $datarow['product_disk_one_cdpackage'];?>"></input></td>
          </tr>
           <tr>
            <td class="main">Product Disk One CDLabel</td>
            <td class="main"><input type="text" name="temp_product_disk_one_cdlabel"value="<?php echo $datarow['product_disk_one_cdlabel'];?>"></input></td>
          </tr>
           <tr>
            <td class="main">Product Disk One Barcode Placement</td>
            <td class="main"><input type="text" name="temp_product_disk_one_barcode_placement"value="<?php echo $datarow['product_disk_one_barcode_placement'];?>"></input></td>
          </tr>
           <tr>
            <td class="main">Product Disk One Media Format</td>
            <td class="main" ><input type="text" name="temp_product_disk_one_media_format"value="<?php echo $datarow['product_disk_one_media_format'];?>"></input></td>
          </tr>
           <tr>
            <td class="main">Product Disk Two Iso</td>
             <td class="main" ><input type="text" name="temp_product_disk_two_iso"value="<?php echo $datarow['product_disk_two_iso'];?>"></input></td>
        
          </tr>
          <tr>
            <td class="main">Product Disk Two Disclabel</td>
              <td class="main" ><input type="text" name="temp_product_disk_two_disclabel"value="<?php echo $datarow['product_disk_two_disclabel'];?>"></input></td>
        
          </tr>
          <tr>
            <td class="main">Product Disk Two Silkscreen</td>
             <td class="main" ><input type="text" name="temp_product_disk_two_silkscreen"value="<?php echo $datarow['product_disk_two_silkscreen'];?>"></input></td>
        
          </tr>
          <tr>
            <td class="main">Product Disk Two CdPackage</td>
              <td class="main" ><input type="text" name="temp_product_disk_two_cdpackage"value="<?php echo $datarow['product_disk_two_cdpackage'];?>"></input></td>
        
            
          </tr>
          <tr>
            <td class="main">Product Disk Two CDLabel</td>
              <td class="main" ><input type="text" name="temp_product_disk_two_cdlabel"value="<?php echo $datarow['product_disk_two_cdlabel'];?>"></input></td>
        
          </tr>
          <tr>
            <td class="main">Product Disk Two Media Format</td>
             <td class="main" ><input type="text" name="temp_product_disk_two_media_format"value="<?php echo $datarow['product_disk_two_media_format'];?>"></input></td>
        
          </tr>
           <tr>
            <td class="main">Status</td>
              <td class="main" ><input type="text" name="temp_product_status"value="<?php echo $datarow['status'];?>"></input></td>
...
<!-- // BOF: MaxiDVD Added for Ulimited Images Pack! -->
          ...
          <tr>
           ...
           <td class="dataTableRow" valign="top"><span class="smallText"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_input_field('products_image') . '<br>'; ?>
           <?php } else { ?>
           ...
           echo tep_draw_separator('pixel_trans.gif', '24', '17" align="left') . $pInfo->products_image; ?><br>
           <?php list($width, $height, $type, $attr) = getimagesize("../images/".$pInfo->products_image);?><a class="change" style="cursor:pointer;color:#F00" 
   onclick="window.open( '<?php echo "../images/".$pInfo->products_image?>', 
  'windowname1', 
  'width=<?php echo $width;?>, height=<?php echo $height;?>');return false; 
   "onMouseOver="this.style.textDecoration='underline';"  onMouseOut="this.style.textDecoration='none';">Image</a><?php echo tep_draw_hidden_field('products_previous_image', $pInfo->products_image) . '<br>'. tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42');
   ...
   ?></span></td>
          ...
          <tr>
           ...
           echo tep_draw_separator('pixel_trans.gif', '24', '17" align="left') . $pInfo->products_image_med ?><br><?php list($width, $height, $type, $attr) = getimagesize("../images/".$pInfo->products_image_med);?><a class="change" style="cursor:pointer;color:#F00" 
   onclick="window.open( '<?php echo "../images/".$pInfo->products_image_med;?>', 
  'windowname1', 
  'width=<?php echo $width;?>, height=<?php echo $height;?>');return false; 
   "onMouseOver="this.style.textDecoration='underline';"  onMouseOut="this.style.textDecoration='none';">Image</a><?php echo tep_draw_hidden_field('products_previous_image_med', $pInfo->products_image_med) . '<br>'. tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image_med" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image_med" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); 
   ?></span></td>
          </tr>
          ...
          <tr>
           ...
           echo tep_draw_separator('pixel_trans.gif', '24', '17" align="left') . $pInfo->products_image_lrg ?><br><?php list($width, $height, $type, $attr) = getimagesize("../images/".$pInfo->products_image_lrg);?><a class="change" style="cursor:pointer;color:#F00" 
   onclick="window.open( '<?php echo "../images/".$pInfo->products_image_lrg;?>', 
  'windowname1', 
  'width=<?php echo $width;?>, height=<?php echo $height;?>');return false; 
   "onMouseOver="this.style.textDecoration='underline';"  onMouseOut="this.style.textDecoration='none';">Image</a> <?php echo tep_draw_hidden_field('products_previous_image_lrg', $pInfo->products_image_lrg) . '<br>'. tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image_lrg" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image_lrg" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); 
   ...
   ?></span></td>
          ...
          <tr>
           <td class="dataTableRow" valign="top"><span class="main">Cover Image</span></td>
           <?php if ((HTML_AREA_WYSIWYG_DISABLE_JPSY == 'Disable') or (HTML_AREA_WYSIWYG_DISABLE == 'Disable')){ ?>
           <td class="dataTableRow" valign="top"><span class="smallText"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_file_field('cover_image') . '<br>'; ?>
           <?php } else { ?>
           <td class="dataTableRow" valign="top"><span class="smallText"><?php echo '<table border="0" cellspacing="0" cellpadding="0"><tr><td class="main">' . tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp; </td><td class="dataTableRow">' . tep_draw_textarea_field('cover_image', 'soft', '70', '2', $pInfo->cover_image) . tep_draw_hidden_field('previous_cover_image', $pInfo->cover_image) . '</td></tr></table>';
           } if (($HTTP_GET_VARS['pID']) && ($pInfo->cover_image) != '')
           echo tep_draw_separator('pixel_trans.gif', '24', '17" align="left') . $pInfo->cover_image ?><br><?php list($width, $height, $type, $attr) = getimagesize("../images/".$pInfo->cover_image);?><a class="change" style="cursor:pointer;color:#F00" 
   onclick="window.open( '<?php echo "../images/".$pInfo->cover_image;?>', 
  'windowname1', 
  'width=<?php echo $width;?>, height=<?php echo $height;?>');return false; 
   "onMouseOver="this.style.textDecoration='underline';"  onMouseOut="this.style.textDecoration='none';">Image</a> <?php echo tep_draw_hidden_field('previous_cover_image', $pInfo->cover_image) . '<br>'. tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_cover_image" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_cover_image" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); ?></span></td>
          </tr>

          <tr>
           <td class="dataTableRow" valign="top"><span class="main">Brochure Image</span></td>
           <?php if ((HTML_AREA_WYSIWYG_DISABLE_JPSY == 'Disable') or (HTML_AREA_WYSIWYG_DISABLE == 'Disable')){ ?>
           <td class="dataTableRow" valign="top"><span class="smallText"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_file_field('brochure_image') . '<br>'; ?>
           <?php } else { ?>
           <td class="dataTableRow" valign="top"><span class="smallText"><?php echo '<table border="0" cellspacing="0" cellpadding="0"><tr><td class="main">' . tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp; </td><td class="dataTableRow">' . tep_draw_textarea_field('brochure_image', 'soft', '70', '2', $pInfo->brochure_image) . tep_draw_hidden_field('previous_brochure_image', $pInfo->brochure_image) . '</td></tr></table>';
           } if (($HTTP_GET_VARS['pID']) && ($pInfo->brochure_image) != '')
           echo tep_draw_separator('pixel_trans.gif', '24', '17" align="left') . $pInfo->brochure_image . tep_image(DIR_WS_CATALOG_IMAGES . $pInfo->brochure_image, $pInfo->brochure_image, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'align="left" hspace="0" vspace="5"') . tep_draw_hidden_field('previous_brochure_image', $pInfo->brochure_image) . '<br>'. tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_brochure_image" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_brochure_image" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); ?></span></td>
          </tr>
  ...
  <?php
    ...
    ?>
              <tr>
                <td class="dataTableRow" colspan="2" valign="top"><?php if (tep_not_null($pInfo->products_image_sm_1)) { ?><span class="smallText"><?php echo $pInfo->products_image_sm_1 . '<br>' ?><?php list($width, $height, $type, $attr) = getimagesize("../images/".$pInfo->products_image_sm_1);?><a class="change" style="cursor:pointer;color:#F00" 
   onclick="window.open( '<?php echo "../images/".$pInfo->products_image_sm_1;?>', 
  'windowname1', 
  'width=<?php echo $width;?>, height=<?php echo $height;?>');return false; 
   "onMouseOver="this.style.textDecoration='underline';"  onMouseOut="this.style.textDecoration='none';">Image</a><br><?php echo tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image_sm_1" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image_sm_1" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); 
    ?></span><?php } ?></td>
                <td class="dataTableRow" colspan="2" valign="top"><?php if (tep_not_null($pInfo->products_image_xl_1)) { ?><span class="smallText"><?php echo $pInfo->products_image_xl_1 . '<br>' ?><?php list($width, $height, $type, $attr) = getimagesize("../images/".$pInfo->products_image_xl_1);?><a class="change" style="cursor:pointer;color:#F00" 
   onclick="window.open( '<?php echo "../images/".$pInfo->products_image_xl_1;?>', 
  'windowname1', 
  'width=<?php echo $width;?>, height=<?php echo $height;?>');return false; 
   "onMouseOver="this.style.textDecoration='underline';"  onMouseOut="this.style.textDecoration='none';">Image</a> <?php echo '<br>'. tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image_xl_1" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image_xl_1" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); 
   ?></span><?php } ?></td>
              </tr>
  <?php
     }
  ...
  <?php
    ...
    ?>
              <tr>
                <td class="smallText" valign="top" colspan="2"><?php if (tep_not_null($pInfo->products_image_sm_2)) { ?><?php echo $pInfo->products_image_sm_2 . '<br>' ?><?php list($width, $height, $type, $attr) = getimagesize("../images/".$pInfo->products_image_sm_2);?><a class="change" style="cursor:pointer;color:#F00" 
   onclick="window.open( '<?php echo "../images/".$pInfo->products_image_sm_2;?>', 
  'windowname1', 
  'width=<?php echo $width;?>, height=<?php echo $height;?>');return false; 
   "onMouseOver="this.style.textDecoration='underline';"  onMouseOut="this.style.textDecoration='none';">Image</a><?php echo '<br>'. tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image_sm_2" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image_sm_2" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42');
   ?><?php } ?></td>
                <td class="smallText" valign="top" colspan="2"><?php if (tep_not_null($pInfo->products_image_xl_2)) { ?><?php echo $pInfo->products_image_xl_2 . '<br>'?><?php list($width, $height, $type, $attr) = getimagesize("../images/".$pInfo->products_image_xl_2);?><a class="change" style="cursor:pointer;color:#F00" 
   onclick="window.open( '<?php echo "../images/".$pInfo->products_image_xl_2;?>', 
  'windowname1', 
  'width=<?php echo $width;?>, height=<?php echo $height;?>');return false; 
   "onMouseOver="this.style.textDecoration='underline';"  onMouseOut="this.style.textDecoration='none';">Image</a><?php echo '<br>'. tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image_xl_2" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image_xl_2" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42');
  ...
  <?php
    ...
    ?>
              <tr>
                <td class="dataTableRow" colspan="2" valign="top"><?php if (tep_not_null($pInfo->products_image_sm_3)) { ?><span class="smallText"><?php echo $pInfo->products_image_sm_3 . '<br>' ?> <?php list($width, $height, $type, $attr) = getimagesize("../images/".$pInfo->products_image_sm_3);?><a class="change" style="cursor:pointer;color:#F00" 
   onclick="window.open( '<?php echo "../images/".$pInfo->products_image_sm_3;?>', 
  'windowname1', 
  'width=<?php echo $width;?>, height=<?php echo $height;?>');return false; 
   "onMouseOver="this.style.textDecoration='underline';"  onMouseOut="this.style.textDecoration='none';">Image</a><?php echo  '<br>'. tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image_sm_3" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image_sm_3" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); ?></span><?php } ?></td>
                <td class="dataTableRow" colspan="2" valign="top"><?php if (tep_not_null($pInfo->products_image_xl_3)) { ?><span class="smallText"><?php echo $pInfo->products_image_xl_3 . '<br>' ?> <?php list($width, $height, $type, $attr) = getimagesize("../images/".$pInfo->products_image_xl_3);?><a class="change" style="cursor:pointer;color:#F00" 
   onclick="window.open( '<?php echo "../images/".$pInfo->products_image_xl_3;?>', 
  'windowname1', 
  'width=<?php echo $width;?>, height=<?php echo $height;?>');return false; 
   "onMouseOver="this.style.textDecoration='underline';"  onMouseOut="this.style.textDecoration='none';">Image</a> <?php echo  '<br>'. tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image_xl_3" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image_xl_3" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); ?></span><?php } ?></td>
              </tr>
  <?php
     }
   ?>
  ...
  <?php
    ...
    ?>
              <tr>
                <td class="smallText" valign="top" colspan="2"><?php if (tep_not_null($pInfo->products_image_sm_4)) { ?><?php echo $pInfo->products_image_sm_4 . '<br>' ?> <?php list($width, $height, $type, $attr) = getimagesize("../images/".$pInfo->products_image_sm_4);?><a class="change" style="cursor:pointer;color:#F00" 
   onclick="window.open( '<?php echo "../images/".$pInfo->products_image_sm_4;?>', 
  'windowname1', 
  'width=<?php echo $width;?>, height=<?php echo $height;?>');return false; 
   "onMouseOver="this.style.textDecoration='underline';"  onMouseOut="this.style.textDecoration='none';">Image</a><?php echo  '<br>'. tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image_sm_4" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image_sm_4" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); ?><?php } ?></td>
                <td class="smallText" valign="top" colspan="2"><?php if (tep_not_null($pInfo->products_image_xl_4)) { ?><?php echo $pInfo->products_image_xl_4 . '<br>'?><?php list($width, $height, $type, $attr) = getimagesize("../images/".$pInfo->products_image_xl_4);?><a class="change" style="cursor:pointer;color:#F00" 
   onclick="window.open( '<?php echo "../images/".$pInfo->products_image_xl_4;?>', 
  'windowname1', 
  'width=<?php echo $width;?>, height=<?php echo $height;?>');return false; 
   "onMouseOver="this.style.textDecoration='underline';"  onMouseOut="this.style.textDecoration='none';">Image</a> <?php echo  '<br>'. tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image_xl_4" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image_xl_4" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); ?><?php } ?></td>
              </tr>
  <?php
     }
   ?>
  ...
  <?php
    ...
    ?>
              <tr>
                <td class="dataTableRow" colspan="2" valign="top"><?php if (tep_not_null($pInfo->products_image_sm_5)) { ?><span class="smallText"><?php echo $pInfo->products_image_sm_5 . '<br>' ?> <?php list($width, $height, $type, $attr) = getimagesize("../images/".$pInfo->products_image_sm_5);?><a class="change" style="cursor:pointer;color:#F00" 
   onclick="window.open( '<?php echo "../images/".$pInfo->products_image_sm_5;?>', 
  'windowname1', 
  'width=<?php echo $width;?>, height=<?php echo $height;?>');return false; 
   "onMouseOver="this.style.textDecoration='underline';"  onMouseOut="this.style.textDecoration='none';">Image</a><?php echo  '<br>'. tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image_sm_5" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image_sm_5" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); ?></span><?php } ?></td>
                <td class="dataTableRow" colspan="2" valign="top"><?php if (tep_not_null($pInfo->products_image_xl_5)) { ?><span class="smallText"><?php echo $pInfo->products_image_xl_5 . '<br>'?> <?php list($width, $height, $type, $attr) = getimagesize("../images/".$pInfo->products_image_xl_5);?><a class="change" style="cursor:pointer;color:#F00" 
   onclick="window.open( '<?php echo "../images/".$pInfo->products_image_xl_5;?>', 
  'windowname1', 
  'width=<?php echo $width;?>, height=<?php echo $height;?>');return false; 
   "onMouseOver="this.style.textDecoration='underline';"  onMouseOut="this.style.textDecoration='none';">Image</a> <?php echo  '<br>'. tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image_xl_5" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image_xl_5" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); ?></span><?php } ?></td>
              </tr>
  <?php
     }
   ?>
  ...
  <?php
    ...
    ?>
              <tr>
                <td class="smallText" valign="top" colspan="2"><?php if (tep_not_null($pInfo->products_image_sm_6)) { ?><?php echo $pInfo->products_image_sm_6 . '<br>' ?><?php list($width, $height, $type, $attr) = getimagesize("../images/".$pInfo->products_image_sm_6);?><a class="change" style=" cursor:pointer;color:#F00" 
   onclick="window.open( '<?php echo "../images/".$pInfo->products_image_sm_6;?>', 
  'windowname1', 
  'width=<?php echo $width;?>, height=<?php echo $height;?>');return false; 
   "onMouseOver="this.style.textDecoration='underline';"  onMouseOut="this.style.textDecoration='none';">Image</a><?php echo  '<br>'. tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image_sm_6" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image_sm_6" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); ?><?php } ?></td>
                <td class="smallText" valign="top" colspan="2"><?php if (tep_not_null($pInfo->products_image_xl_6)) { ?><?php echo $pInfo->products_image_xl_6 . '<br>' ?><?php list($width, $height, $type, $attr) = getimagesize("../images/".$pInfo->products_image_xl_6);?><a class="change" style="cursor:pointer;color:#F00" 
   onclick="window.open( '<?php echo "../images/".$pInfo->products_image_xl_6;?>', 
  'windowname1', 
  'width=<?php echo $width;?>, height=<?php echo $height;?>');return false; 
   "onMouseOver="this.style.textDecoration='underline';"  onMouseOut="this.style.textDecoration='none';">Image</a> <?php echo  '<br>'. tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image_xl_6" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image_xl_6" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); ?><?php } ?></td>
              </tr>
  <?php
     }
   ?>
...
?>
  <tr>
    <td class="main">
        ...
        <?php echo '<br>'.TEXT_INFO_CURRENT_CATEGORIES . '<br><b>' . tep_output_generated_category_path($pInfo->products_id, 'product') . '</b><br>';
              echo TEXT_CATEGORIES . '<br>' . tep_draw_pull_down_menu_catg('categoriesid[]', tep_get_category_tree(),$cat, "size='22'");
        ?>
    </td>
  </tr>
          <tr>
            <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
                    ...
                    <!----NEWLY INSERTED FOR PRODUCTS-VENDORS MAPPING-->
          <tr>
          <td class="main">
              <?php echo '<br>'.TEXT_INFO_CURRENT_Vendors. '<br>';
              $ven_prod_sql = tep_db_query("select v.vendors_name from ".TABLE_VENDORS." v, ".TABLE_PRODUCTS_TO_VENDORS." pv where v.vendors_id = pv.vendors_id and pv.products_id = '".(int)$HTTP_GET_VARS['pID'] ."'");
              $ven_prod_count = tep_db_num_rows($ven_prod_sql);
              if ($ven_prod_count > 0)
              {
                while ($ven_prod_rs = tep_db_fetch_array($ven_prod_sql))
                {
                  echo "<b>$ven_prod_rs[vendors_name]</b><br>";
                }
              }
              else
              {
              ...
              echo "<br>";
                  $vendors_sel_sql = tep_db_query("select vendors_id, vendors_name from ".TABLE_VENDORS." order by vendors_name");
                  while ($vendors_rs = tep_db_fetch_array($vendors_sel_sql))
                  {
                    $vendors_values[] = array('id' => $vendors_rs['vendors_id'],
                                  'text' => $vendors_name[] = $vendors_rs['vendors_name']);
                  }
                    echo TEXT_VENDORS_LIST . '<br>' . tep_draw_pull_down_menu_vendors('vendors[]', $vendors_values,$vid, "size='22'");
              ?>
          </td>
        </tr>
        <!----NEWLY INSERTED FOR PRODUCTS-VENDORS MAPPING-->
...
<?php if ((HTML_AREA_WYSIWYG_DISABLE_JPSY == 'Enable') && (HTML_AREA_WYSIWYG_DISABLE == 'Enable')){ ?>
             ...
             editor_generate('cover_image',config);
             editor_generate('brochure_image',config);
            <?php } ?>
            </script>
<?php } ?>
...
<?php
   ...
   if(isset($_POST['temp_product_id'])) {
   tep_db_query("Update products_to_allied set 
      product_set_total_disks='".$_POST['temp_product_set_total_disks']."',
      product_media_definition='".$_POST['temp_product_media_definition']."',
      product_disk_one_iso='".$_POST['temp_product_disk_one_iso']."',
      product_disk_one_wrap='".$_POST['temp_product_disk_one_wrap']."',
      product_disk_one_disclabel='".$_POST['temp_product_disk_one_disclabel']."',
      product_disk_one_silkscreen='".$_POST['temp_product_disk_one_silkscreen']."',
      product_disk_one_cdpackage='".$_POST['temp_product_disk_one_cdpackage']."',
      
      product_disk_one_cdlabel='".$_POST['temp_product_disk_one_cdlabel']."',
      product_disk_one_barcode_placement='".$_POST['temp_product_disk_one_barcode_placement']."',
      product_disk_one_media_format='".$_POST['temp_product_disk_one_media_format']."',
      product_disk_two_iso='".$_POST['temp_product_disk_two_iso']."',
      product_disk_two_disclabel='".$_POST['temp_product_disk_two_disclabel']."',
      product_disk_two_silkscreen='".$_POST['temp_product_disk_two_silkscreen']."',
      product_disk_two_cdpackage='".$_POST['temp_product_disk_two_cdpackage']."',
      product_disk_two_cdlabel='".$_POST['temp_product_disk_two_cdlabel']."',
      product_disk_two_media_format='".$_POST['temp_product_disk_two_media_format']."',
      status='".$_POST['temp_product_status']."'
      where product_id='" . $_POST['temp_product_id'] . "'");
   }
...
// EOF MaxiDVD: Modified For Ultimate Images Pack!
      ...
      $cover_image_name = $pInfo->cover_image;
      $brochure_image_name = $pInfo->brochure_image;
...
/* Re-Post all POST'ed variables */
      ...
      while (list($key, $value) = each($HTTP_POST_VARS)) {
        ...
        else
        {
          ...
          foreach($value as $val)
          {
            echo "<input type=hidden name=$key$i value=".htmlspecialchars(stripslashes($val)).">";
            $i++;
          }
        }
...
// BOF MaxiDVD: Added For Ultimate Images Pack!
      ...
      echo tep_draw_hidden_field('cover_image', stripslashes($cover_image_name));
      echo tep_draw_hidden_field('brochure_image', stripslashes($brochure_image_name));
...
<?php
    ...
    $products_count = 0;
    if (isset($HTTP_GET_VARS['search'])) {
      $products_query = tep_db_query("select p.products_id, pd.products_name, pd.products_name_prefix, pd.products_name_suffix, p.products_quantity, p.products_image, p.products_price, p.products_date_added, p.products_last_modified, p.products_date_available, p.products_status, p2c.categories_id, mt.products_media_type_name, se.series_name from ((" . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd) left join " . TABLE_MEDIA_TYPE . " mt on (p.products_media_type_id = mt.products_media_type_id)) left join " . TABLE_SERIES . " se on (p.series_id = se.series_id), " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c where p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' and p.products_id = p2c.products_id and pd.products_name like '%" . tep_db_input($search) . "%' group by p.products_id order by pd.products_name");
...
<?php
    ...
    switch ($action) {
      ...
      case 'edit_category':
        ...
        $contents[] = array('text' => '<br>' . TEXT_EDIT_SORT_ORDER . '<br>' . tep_draw_input_field('sort_order', $cInfo->sort_order, 'size="2"'));
            require_once("includes/addons/seo_urls/seo_urls_categories_insert_2.php");
    ...
    // ##################### End Added Categories Enable / Disable ###################
      ...
      case 'copy_to':
        ...
        $contents[] = array('text' => '<br>' . TEXT_INFO_CURRENT_CATEGORIES . '<br><b>' . tep_output_generated_category_path($pInfo->products_id, 'product') . '</b>');
        $contents[] = array('text' => '<br>' . TEXT_CATEGORIES . '<br>' . tep_draw_pull_down_menu_catg('categoriesid[]', tep_get_category_tree(), $current_category_id));
