  ...
  switch ($HTTP_GET_VARS['action']) {
    ...
    case 'update_confirm':
      if ( ($HTTP_POST_VARS['back_x']) || ($HTTP_POST_VARS['back_y']) ) {
		...
		if ($HTTP_POST_VARS['coupon_free_lowest_price']) $coupon_free_lowest_price = '1';
        $sql_data_array = array('coupon_code' => tep_db_prepare_input($HTTP_POST_VARS['coupon_code']),
								...
								'lowest_price' => tep_db_prepare_input($coupon_free_lowest_price),
...
function chk_coupon(a,b)
{
var x=b;
var to_id=a;

//alert('coupon_status_chk.php?id='+to_id);
var request = createRequestObject();
request.open('GET', 'coupon_status_chk.php?id='+to_id+'&x='+x
, true);
...
document.getElementById('battel').innerHTML = 'Loading...';
}
...
var response = request.responseText;
//alert(response);
document.getElementById('battel').innerHTML = response;
//document.getElementById('upload_area_multi2').innerHTML = response;
}
}
}
request.send(null);
...
<?php
	...
	echo tep_draw_hidden_field('coupon_free_lowest_price', $HTTP_POST_VARS['coupon_free_lowest_price']);
...
<?php      
  ...
  case 'voucheredit':
    ...
    $coupon_query=tep_db_query("select coupon_code, coupon_amount, coupon_type, lowest_price, coupon_minimum_order, coupon_start_date, coupon_expire_date, uses_per_coupon, uses_per_user, restrict_to_products, restrict_to_categories from " . TABLE_COUPONS . " where coupon_id = '" . $HTTP_GET_VARS['cid'] . "'");
    $coupon=tep_db_fetch_array($coupon_query);
	...
	if ($coupon['lowest_price']=='1') {
      $coupon_free_lowest_price .= true;
    }   
...
?>
	  ...
	  <tr>
        <td align="left" class="main"><?php echo COUPON_FREE_LOWEST; ?></td>
        <td align="left"><?php echo tep_draw_checkbox_field('coupon_free_lowest_price', $coupon_free_lowest_price); ?></td>
        <td align="left" class="main"><?php echo COUPON_FREE_LOWEST_HELP; ?></td>
      </tr>
      <tr>
...
<?php
    ...
    while ($cc_list = tep_db_fetch_array($cc_query)) {
      ...
      if ( (is_object($cInfo)) && ($cc_list['coupon_id'] == $cInfo->coupon_id) ) {
        echo '          <tr class="dataTableRowSelected" onmouseover="this.style.cursor=\'hand\'" ondoubleclick="document.location.href=\'' . tep_href_link('coupon_admin.php', tep_get_all_get_params(array('cid', 'action')) . 'cid=' . $cInfo->coupon_id . '&action=edit') . '\'">' . "\n";
      } else {
        echo '          <tr class="dataTableRow" onmouseover="this.className=\'dataTableRowOver\';this.style.cursor=\'hand\'" onmouseout="this.className=\'dataTableRow\'" ondoubleclick="document.location.href=\'' . tep_href_link('coupon_admin.php', tep_get_all_get_params(array('cid', 'action')) . 'cid=' . $cc_list['coupon_id']) . '\'">' . "\n";
      }
    ...
    default:
      ...
      if ($HTTP_GET_VARS['action'] == 'voucherdelete') {
        ...
        $contents[] = array('text'=>COUPON_NAME . '&nbsp;::&nbsp; ' . $coupon_name['coupon_name'] . '<br>' .
                     ...
                     '<a href="'.tep_href_link('coupon_admin.php','action=voucherdelete&cid='.$cInfo->coupon_id,'NONSSL').'">'.tep_image_button('button_delete.gif','Delete Voucher').'</a>' .
