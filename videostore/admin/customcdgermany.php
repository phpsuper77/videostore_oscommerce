...
<form action="" method="POST">
	...
	<!--select name="carry">
		<option value=""></option>
		<option value="DEFAULT">Deutsche Post</option>
	...
	<!--select name="carry_type">
		<option value=""></option>
		<option value="DEUTSCHE_POST_DOM">Deutsche Post domestic Germany
</option>
		<option value="DEUTSCHE_POST_INTL">Deutsche Post international</option>
		<option value="GDHL-PACKET">DHL Paket within the EU (6,90 EUR in DE to 17,00 EUR in EU)</option>
		<option value="GDHL-INTL">DHL Paket International outside the EU (17,00 EUR to 40,00 EUR)</option>
		<option value="GDHL-INTL-PM">DHL Paket International Premium (faster delivery - 24,00 EUR to 62,00 EUR)</option>
	</select-->
	<select name="carry_type">
		<option value=""></option>
                                <option value="DEUTSCHE_POST_DOM">Deutsche Post domestic
</option>
		<option value="DEUTSCHE_POST_INTL">Deutsche Post international</option>
		<option value="GDHL-PACKET">DHL Paket within the EU (6,90 EUR in DE to 17,00 EUR in EU delviered in 1-2 days)</option>
		<option value="GDHL-INTL">DHL Paket International outside the EU (17,00 EUR to 40,00 EUR)</option>
		<option value="GDHL-INTL-PM">DHL Paket International Premium (faster delivery - 24,00 EUR to 62,00 EUR)</option>
	</select>
...
?>
<form action="customcdgermany.php" method="post">
	<input type="hidden" name="list" value="<?=$productList?>" />
...
foreach($pos as $key=>$value) {
	...
	mysql_query("update orders_products set date_sent_to_allied='".$sent."', is_allied=1, date_shipped_checkbox=1, fulfilled_by='GER' where orders_products_id='".$value."'");
...
//$comments = "Sent to Germany Fullfilment Center on " . date("m-d-Y H:i:s") ." - SentOrderID=" .$oID ;
...
//$comments = "Sent to Germany Fulfillment Center " . date("m-d-Y H:i:s") ." - SentOrderID=" .$oID ;

$comments = "Sent to Germany Fulfillment Center " . date("m-d-Y H:i:s") ." - Order ID=" .$oID." - ".$carrier." ".$carry_type;
