...
global $maxrecs;
$maxrecs = 500; // default, seems to work for most people.  Reduce if you hit timeouts
...
$default_image_series = '';
$default_image_producers = '';
$default_image_distributors = '';
...
$default_these = array(
	...
	'v_distributors_name',
	'v_distributors_id',
	'v_products_media_type_id',
	'v_products_video_format_id',
	'v_products_audio_id',
	'v_products_aspect_ratio_id',
	'v_products_region_code_id',
	'v_products_set_type_id',
	'v_products_packaging_type_id',
	'v_products_warehouse_location',
	'v_products_export_id',
	//'v_products_byline',
	'v_products_summary',
	'v_products_audio_languages',
	'v_products_subtitle_languages',
	'v_products_closed_captioned',
	'v_products_pc_bonus',
	'v_products_out_of_print',
	'v_distributors_id',
	'v_producers_id',
	'v_series_id'
...
if ( $download == 'stream' or  $download == 'tempfile' ){
	...
	while ($row){
		...
		foreach ($langcode as $key => $lang){
			...
			$row['v_products_name_prefix_' . $lid] 	= $row2['products_name_prefix'];
			$row['v_products_name_suffix_' . $lid] 	= $row2['products_name_suffix'];
...
// if the filelayout says we need series name, get it
		if (isset($filelayout['v_series_name'])){
			if ($row['v_series_id'] != ''){
				$sql2 = "SELECT series_name
					FROM ".TABLE_SERIES."
					WHERE
					series_id = " . $row['v_series_id']
					;
				$result2 = tep_db_query($sql2);
				$row2 =  tep_db_fetch_array($result2);
				$row['v_series_name'] = $row2['series_name'];
			}
		}
// if the filelayout says we need a producers name, get it
		if (isset($filelayout['v_producers_name'])){
			if ($row['v_producers_id'] != ''){
				$sql2 = "SELECT producers_name
					FROM ".TABLE_PRODUCERS."
					WHERE
					producers_id = " . $row['v_producers_id']
					;
				$result2 = tep_db_query($sql2);
				$row2 =  tep_db_fetch_array($result2);
				$row['v_producers_name'] = $row2['producers_name'];
			}
		}
// if the filelayout says we need a distributors name, get it
		if (isset($filelayout['v_distributors_name'])){
			if ($row['v_distributors_id'] != ''){
				$sql2 = "SELECT distributors_name
					FROM ".TABLE_DISTRIBUTORS."
					WHERE
					distributors_id = " . $row['v_distributors_id']
					;
				$result2 = tep_db_query($sql2);
				$row2 =  tep_db_fetch_array($result2);
				$row['v_distributors_name'] = $row2['distributors_name'];
			}
		}
...
function ep_create_filelayout($dltype){
	...
	case 'full':
		...
		$filelayout = array(
			...
			'v_products_run_time'		=> $iii++,
			'v_products_release_date'		=> $iii++,
			'v_products_isbn'		=> $iii++,
			'v_products_upc'		=> $iii++,
			'v_products_media_type_id'		=> $iii++,
			'v_products_video_format_id'		=> $iii++,
			'v_products_audio_id'		=> $iii++,
			'v_products_aspect_ratio_id'		=> $iii++,
			'v_products_region_code_id'		=> $iii++,
			'v_products_set_type_id'		=> $iii++,
			'v_products_packaging_type_id'		=> $iii++,
			'v_products_warehouse_location'		=> $iii++,
			'v_products_export_id'		=> $iii++,
			'v_products_byline'		=> $iii++,
			'v_products_summary'		=> $iii++,
			'v_products_audio_languages'		=> $iii++,
			'v_products_subtitle_languages'		=> $iii++,
			'v_products_closed_captioned'		=> $iii++,
			'v_products_pc_bonus'		=> $iii++,
			'v_products_out_of_print'		=> $iii++,
		...
		foreach ($langcode as $key => $lang){
			...
			$filelayout  = array_merge($filelayout , array(
					...
					'v_products_name_prefix_' . $l_id		=> $iii++,
					'v_products_name_suffix_' . $l_id		=> $iii++,
		...
		$filelayout  = array_merge($filelayout , array(
			...
			'v_series_name'		=> $iii++,
			'v_producers_name'		=> $iii++,
			'v_distributors_name'		=> $iii++,
			));
		...
		$filelayout_sql = "SELECT
			...
			p.products_run_time as v_products_run_time,
			p.products_release_date as v_products_release_date,
			...
			p.series_id as v_series_id,
			p.producers_id as v_producers_id,
			p.distributors_id as v_distributors_id,
			subc.categories_id as v_categories_id,
			p.products_status as v_status,
			p.products_media_type_id as v_products_media_type_id,
			p.products_video_format_id as v_products_video_format_id,
			p.products_audio_id as v_products_audio_id,
			p.products_aspect_ratio_id as v_products_aspect_ratio_id,
			p.products_region_code_id as v_products_region_code_id,
			p.products_set_type_id as v_products_set_type_id,
			p.products_packaging_type_id as v_products_packaging_type_id,
			p.products_warehouse_location as v_products_warehouse_location,
			p.products_export_id as v_products_export_id,
			p.products_byline as v_products_byline,
			p.products_summary as v_products_summary,
			p.products_audio_languages as v_products_audio_languages,
			p.products_subtitle_languages as v_products_subtitle_languages,
			p.products_closed_captioned as v_products_closed_captioned,
			p.products_pc_bonus as v_products_pc_bonus,
			p.products_out_of_print as v_products_out_of_print 
			FROM
			".TABLE_PRODUCTS." as p,
...
//				";
			...
			$row['v_products_name_prefix_' . $lang['id']] 		= $row2['products_name_prefix'];
			$row['v_products_name_suffix_' . $lang['id']] 		= $row2['products_name_suffix'];
	...
	foreach ($langcode as $lang){
		...
		if (isset($filelayout['v_products_name_' . $l_id ])){
			...
			$v_products_name_prefix[$l_id] 	= $items[$filelayout['v_products_name_prefix_' . $l_id]];
			$v_products_name_suffix[$l_id] 	= $items[$filelayout['v_products_name_suffix_' . $l_id]];
	...
	if (trim($v_products_image_med)==''){
		$v_products_image_med = $default_image_product_med;
	}
	
	if (trim($v_products_image_lrg)==''){
		$v_products_image_lrg = $default_image_product_lrg;
	}
	...
	// OK, we need to convert the series's name into id's for the database
	if ( isset($v_series_name) && $v_series_name != '' ){
		$sql = "SELECT ser.series_id
			FROM ".TABLE_SERIES." as ser
			WHERE
				ser.series_name = '" . $v_series_name . "'";
		$result = tep_db_query($sql);
		$row =  tep_db_fetch_array($result);
		if ( $row != '' ){
			foreach( $row as $item ){
			...
			// to add, we need to put stuff in categories and categories_description
			$sql = "SELECT MAX( series_id) max FROM ".TABLE_SERIES;
			$result = tep_db_query($sql);
			$row =  tep_db_fetch_array($result);
			$max_ser_id = $row['max']+1;
			// default the id if there are no series yet
			if (!is_numeric($max_ser_id) ){
				$max_ser_id=1;
			}

			// Comment this query out if you have an older 2.2 codebase
			$sql = "INSERT INTO ".TABLE_SERIES."(
				series_id,
				series_name,
				series_image,
				date_added,
			...
			$result = tep_db_query($sql);
			$v_serie_id = $max_ser_id;
		}
	}	
	
	// OK, we need to convert the producer's name into id's for the database
	if ( isset($v_producers_name) && $v_producers_name != '' ){
		$sql = "SELECT pro.producers_id
			FROM ".TABLE_PRODUCERS." as pro
			WHERE
				pro.producers_name = '" . $v_producers_name . "'";
		$result = tep_db_query($sql);
		$row =  tep_db_fetch_array($result);
		if ( $row != '' ){
			foreach( $row as $item ){
			...
			// to add, we need to put stuff in categories and categories_description
			$sql = "SELECT MAX( producers_id) max FROM ".TABLE_PRODUCERS;
			$result = tep_db_query($sql);
			$row =  tep_db_fetch_array($result);
			$max_pro_id = $row['max']+1;
			// default the id if there are no producers yet
			if (!is_numeric($max_pro_id) ){
				$max_pro_id=1;
			}

			// Comment this query out if you have an older 2.2 codebase
			$sql = "INSERT INTO ".TABLE_PRODUCERS."(
				producers_id,
				producers_name,
				producers_image,
				date_added,
			...
			$result = tep_db_query($sql);
			$v_producer_id = $max_pro_id;
		}
	}
	
	// OK, we need to convert the distributor's name into id's for the database
	if ( isset($v_distributors_name) && $v_distributors_name != '' ){
		$sql = "SELECT dis.distributors_id
			FROM ".TABLE_DISTRIBUTORS." as dis
			WHERE
				dis.distributors_name = '" . $v_distributors_name . "'";
		$result = tep_db_query($sql);
		$row =  tep_db_fetch_array($result);
		if ( $row != '' ){
			foreach( $row as $item ){
				$v_distributor_id = $item;
			}
		} else {
			// to add, we need to put stuff in categories and categories_description
			$sql = "SELECT MAX( distributors_id) max FROM ".TABLE_DISTRIBUTORS;
			$result = tep_db_query($sql);
			$row =  tep_db_fetch_array($result);
			$max_dis_id = $row['max']+1;
			// default the id if there are no DISTRIBUTORS yet
			if (!is_numeric($max_dis_id) ){
				$max_dis_id=1;
			}

			// Comment this query out if you have an older 2.2 codebase
			$sql = "INSERT INTO ".TABLE_DISTRIBUTORS."(
				distributors_id,
				distributors_name,
				distributors_image,
				date_added,
		...
		if (tep_db_num_rows($result) == 0)  {
			...
			*/
			...
			$query = 'UPDATE '.TABLE_PRODUCTS.'
					...
					'" ,products_run_time="'.$v_products_run_time.					
					'" ,products_release_date="'.$v_products_release_date.
					'" ,products_isbn="'.$v_products_isbn.
					'" ,products_upc="'.$v_products_upc.
					'" ,products_media_type_id="'.$v_products_media_type_id.
					'" ,products_video_format_id="'.$v_products_video_format_id.
					'" ,products_audio_id="'.$v_products_audio_id.
					'" ,products_aspect_ratio_id="'.$v_products_aspect_ratio_id.
					'" ,products_region_code_id="'.$v_products_region_code_id.
					'" ,products_set_type_id="'.$v_products_set_type_id.
					'" ,products_packaging_type_id="'.$v_products_packaging_type_id.
					'" ,products_warehouse_location="'.$v_products_warehouse_location.
					'" ,products_export_id="'.$v_products_export_id.
					'" ,products_byline="'.$v_products_byline.
					'" ,products_summary="'.$v_products_summary.
					'" ,products_audio_languages="'.$v_products_audio_languages.
					'" ,products_subtitle_languages="'.$v_products_subtitle_languages.
					'" ,products_closed_captioned="'.$v_products_closed_captioned.
					'" ,products_pc_bonus="'.$v_products_pc_bonus.
					'" ,products_out_of_print="'.$v_products_out_of_print;
...
*/
			...
			$query .= '", products_weight="'.$v_products_weight .
					...
					' ,series_id=' . $v_serie_id . 
					' ,producers_id=' . $v_producer_id . 
					' ,distributors_id=' . $v_distributor_id . 
					...
					if (tep_db_num_rows($result) == 0) {
						...
						$sql =
							"INSERT INTO ".TABLE_PRODUCTS_DESCRIPTION."
								...
								VALUES (
									...
									" . $key . ",
									'" . $name . "',
									'". $v_products_name_prefix[$key] . "',
									'". $v_products_name_suffix[$key] . "',
						...
						$sql =
							"UPDATE ".TABLE_PRODUCTS_DESCRIPTION." SET
								products_name='$name',
								products_name_prefix='" . $v_products_name_prefix[$key] . "',
								products_name_suffix='" . $v_products_name_suffix[$key] . "',
