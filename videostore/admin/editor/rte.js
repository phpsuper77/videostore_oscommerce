...
function insert(what) {
   ...
   case "char":
      ...
      break;
   case "image":
      window.open("image.php", "", "width=580,height=360");
      return;
//val//      strPage = "dlg_ins_image.html";
//val//      strAttr = "status:no;dialogWidth:400px;dialogHeight:200px;help:no";' '
      break;
   case "about":
...
function link(on)
{
   ...
   if (el && el.nodeName == "A") {
      ...
      if (document.selection.type != "Control") {
        ...
        document.execCommand("UnLink", false, el.href);
        return;
      }
   }
   else el = null;
   ...
   if (document.selection.type != "Control") strText = r.htmlText;
   // Prompt for the URL
   ...
   if (strURL) {
      ...
      if (document.selection.type != "Control") { 
        ...
        if (el) el.href = strURL;
        else r.pasteHTML('<A href=' + strURL + ' target=_new>' + strText + '</a>');
      } else {
        if (el) el.href = strURL;
        else {
          document.execCommand("CreateLink", false, strURL);
          el = r(0).parentNode;
