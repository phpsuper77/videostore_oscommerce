    ...
    switch ($HTTP_GET_VARS['action']) {
      ...
      if (($HTTP_POST_VARS['edit_x']) || ($HTTP_POST_VARS['edit_y']) || ($HTTP_POST_VARS['manufacturers_id']) || ($HTTP_POST_VARS['cPath']) || ($HTTP_POST_VARS['products_id']) || ($HTTP_POST_VARS['upcoming']) ) {
        ...
        for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
          ...
          $event_title = stripslashes($HTTP_POST_VARS['title'][$language_id]);
...
?>
     ...
     <tr>
      ...
      tep_draw_input_field('title[' . $language_id . ']',(($HTTP_POST_VARS['title'][$language_id]) ? stripslashes($HTTP_POST_VARS['title'][$language_id]) : $events_language[$i]['title']), 'size="50"', false); ?>&nbsp;<font color="red"> * required </font></td>
     </tr>
<?php
    }
?>
...
?>
      ...
      <td colspan="2">
       <table border="0" width="100%" cellspacing="0" cellpadding="2" class="columnLeft">
        ...
        <tr>
         <?php include(DIR_WS_BOXES . 'event_drop_dns.php'); ?>
        </tr>
        <tr>
...
<?php
    ...
    $language_id = $languages[$i]['id'];
          ...
          echo '<table border="0" width="100%" cellspacing="0" cellpadding="4" class="event">'.
               '<tr>'.
               '<td width="100%" bgcolor="#D9DEE6" class="main" style="border-bottom: 1px solid #D9DEE6" nowrap>'. TEXT_EVENT_TITLE .'&nbsp;&nbsp;'. stripslashes($HTTP_POST_VARS['title'][$language_id]) .  tep_draw_hidden_field('title[' . $language_id . ']', stripslashes($HTTP_POST_VARS['title'][$language_id])) .'</td>';
          ...
          echo '</tr><tr>'.
               ...
               if ($event_image != ''){
                  echo'<table border="0" cellspacing="0" cellpadding="0" align="right"><tr>'.
                      '<td class="main">'. tep_image(DIR_WS_CATALOG_IMAGES .'events_images/' . $event_image , stripslashes($HTTP_POST_VARS['title'][$language_id]), '', '', 'align="right" hspace="5" vspace="5"') .'</td>'.
                      '</tr></table>';
                      }

          echo stripslashes($HTTP_POST_VARS['description'][$language_id]) . tep_draw_hidden_field('description[' . $language_id . ']',  stripslashes($HTTP_POST_VARS['description'][$language_id])) .'</td>';
