...
<?php
  if ( ($HTTP_GET_VARS['action'] == 'new') || ($HTTP_GET_VARS['action'] == 'edit') ) {
    ...
    if ( ($HTTP_GET_VARS['action'] == 'edit') && ($HTTP_GET_VARS['sID']) ) {
      ...
      $product_query = tep_db_query("select p.products_id, pd.products_name, pd.products_name_prefix, pd.products_name_suffix, se.series_name, mt.products_media_type_name, s.expires_date from " . TABLE_PRODUCTS . " p left join " . TABLE_SERIES . " se on (p.series_id = se.series_id) left join " . TABLE_MEDIA_TYPE . " mt on (p.products_media_type_id = mt.products_media_type_id), " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_FEATURED . " s where p.products_id = pd.products_id and pd.language_id = '" . $languages_id . "' and p.products_id = s.products_id and s.featured_id = '" . $HTTP_GET_VARS['sID'] . "' order by pd.products_name");
...
<?php
    $featured_query_raw = "select p.products_id, pd.products_name, pd.products_name_prefix, pd.products_name_suffix, mt.products_media_type_name, se.series_name, s.featured_id, s.featured_date_added, s.featured_last_modified, s.expires_date, s.date_status_change, s.status from " . TABLE_PRODUCTS . " p left join " . TABLE_SERIES . " se on (p.series_id = se.series_id) left join " . TABLE_MEDIA_TYPE . " mt on (p.products_media_type_id = mt.products_media_type_id), " . TABLE_FEATURED . " s, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = pd.products_id and pd.language_id = '" . $languages_id . "' and p.products_id = s.products_id order by pd.products_name";
...
?>
                <td  class="dataTableContent"><?php echo '<b>' . $featured['series_name'] . '</b><br>' . $featured['products_name_prefix'] . '&nbsp;<b>' . $featured['products_name'] . '&nbsp;</b>' . $featured['products_name_suffix'] . '-' . $featured['products_media_type_name']; ?></td>
