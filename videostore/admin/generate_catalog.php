...
//$currdate = mktime("H-i-s", "m-d-Y");
//

$sql_query = "SELECT p.products_date_available, p.products_closed_captioned, media.products_media_type_name, p.products_release_date, p.products_price, sr.series_name, p.products_image_med, p.products_id, p.products_model, p.products_run_time, p.products_upc, p.products_release_date, p.products_youtube_id,  f.products_video_format_name, pd.products_name_prefix, pd.products_description, pd.products_name, pd.products_name_suffix, pd.products_clip_url, c.producers_name, IF (s.status, s.specials_new_products_price, NULL ) AS specials_new_products_price, IF (s.status, s.specials_new_products_price, p.products_price) AS final_price FROM ((((((products p) LEFT JOIN products_video_formats f ON p.products_video_format_id = f.products_video_format_id) LEFT JOIN specials s ON p.products_id = s.products_id) LEFT JOIN series sr ON p.series_id = sr.series_id) LEFT JOIN products_description pd ON p.products_id = pd.products_id) LEFT JOIN products_media_types media ON p.products_media_type_id=media.products_media_type_id) LEFT JOIN producers c ON p.producers_id = c.producers_id where p.products_status!=0 and p.products_out_of_print!=0 or (p.products_out_of_print=0 and p.products_quantity>0) GROUP BY products_id ORDER BY c.producers_name ";
...
$pdf->Text(5,295,'TravelVideoStore Full Catalog (DVD/VHS) '.date("m/d/Y"));
$pdf->SetFont('Arial','',9);
...
//bof mod for php5 - mktime yields error when date is null
if ($products[products_date_available]==0000-00-00 || $products[products_date_available]=='NULL') $prods_date_avail = 2009-02-02; else $prods_date_avail = $products[products_date_available];

//$part = explode("-",substr($products[products_date_available],0,10));
$part = explode("-",substr($prods_date_avail,0,10));
//eof 
...
// if (trim($products['products_youtube_id'])!=''){
//	$pdf->Image('../images/video_clip.jpg',$MorefromLeft+18,45+$percentTop, 18, 6);
//	$link = $pdf->Link($MorefromLeft+18,45+$percentTop,'18','6', 'http://www.youtube.com/watch?feature=player_embedded&v='. $products['products_youtube_id']);

//}


 $prod_name = $products[producers_name];//eregi_replace("&#39", "'", $products[producers_name]);
