...
$pdf->SetCreator('Digital Distribution Products');
$pdf->SetTitle('Digital Distribution Products');
$pdf->SetFont('Arial','',9);

$pdf->AddPage();
$pdf->Text(185,295,'Page '.$pdf->PageNo().' of '.ceil($total));
$pdf->SetFont('Arial','I',9);
$pdf->Text(5,295,'TravelVideoStore Digital Distribution Catalog '.date("m/d/Y"));
$pdf->SetFont('Arial','',9);
...
if (!empty($products['products_clip_url'])){
	$pdf->Image('../images/video_clip.jpg',$MorefromLeft+18,44+$percentTop, 18, 6);
	$link = $pdf->Link($MorefromLeft+18,$fromText+$percentTop,'18','6', $products['products_clip_url']);
}
/*
...
$cnt++;
	if ($cnt==10){
		...
		$pdf->Text(185,295,'Page '.$pdf->PageNo().' of '.ceil($total));
		$pdf->SetFont('Arial','I',9);
		$pdf->Text(5,295,'TravelVideoStore Digital Distribution Catalog as of '.date("m/d/Y"));
		$pdf->SetFont('Arial','',9);
		$pdf->Rect('1','1','208.2','295.2');
		$i=0; $counter=0;
	    }
