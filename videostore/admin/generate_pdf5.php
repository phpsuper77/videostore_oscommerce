...
if ($products['products_clip_url']!=''){
	$pdf->Image('../images/video_clip.jpg',$MorefromLeft+18,44+$percentTop, 18, 6);
	$link = $pdf->Link($MorefromLeft+18,44+$percentTop,'18','6', $products['products_clip_url']);
}
...
if (!empty($products[series_name])){
		...
		$pdf->Text($ProducerBlock,54+$percentTop, $products[series_name]);
		$pdf->SetFont('Arial','',8);
}
