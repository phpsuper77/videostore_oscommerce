<?php
  ...
  function tep_get_series_description($series_id) {
    $series_query = tep_db_query("select series_description from " . TABLE_SERIES . " where series_id = '" . $series_id . "'");
    $series = tep_db_fetch_array($series_query);
    return $series['series_description'];
  }
  
  function tep_get_producers_description($producers_id) {
    $producers_query = tep_db_query("select producers_description from " . TABLE_PRODUCERS . " where producers_id = '" . $producers_id . "'");
    $producers = tep_db_fetch_array($producers_query);
    return $producers['producers_description'];
  }
  
  function tep_get_distributors_description($distributors_id) {
    $distributors_query = tep_db_query("select distributors_description from " . TABLE_CATEGORIES_DESCRIPTION . " where distributors_id = '" . $distributors_id . "'");
    $distributors = tep_db_fetch_array($distributors_query);
    return $distributors['distributors_description'];
  }
