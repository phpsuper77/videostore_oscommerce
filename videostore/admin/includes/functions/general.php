...
// NOTE: Includes a workaround for dates before 01/01/1970 that fail on windows servers
  ...
  function tep_draw_products_pull_down($name, $parameters = '', $exclude = '') {
    ...
    $products_query = tep_db_query("select p.products_id, se.series_name, pd.products_name_prefix, pd.products_name, pd.products_name_suffix, mt.products_media_type_name, p.products_price from " . TABLE_PRODUCTS . " p left join " . TABLE_SERIES . " se on (p.series_id = se.series_id) left join " . TABLE_MEDIA_TYPE . " mt on (p.products_media_type_id = mt.products_media_type_id), " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' order by series_name, products_name_prefix, products_name, products_name_suffix");
    while ($products = tep_db_fetch_array($products_query)) {
      if (!in_array($products['products_id'], $exclude)) {
        $select_string .= '<option value="' . $products['products_id'] . '">' . $products['series_name'] . ' ' . $products['products_name_prefix'] . ' ' . $products['products_name'] . ' ' . $products['products_name_suffix'] . '-' . $products['products_media_type_name'] . ' (' . $currencies->format($products['products_price']) . ')</option>';
      }
    }
...
// Text Mode
  ...
  function tep_get_products_name($product_id, $language_id = 0) {
    ...
    function tep_get_products_name_suffix($product_id, $language_id = 0) {
    global $languages_id;

    if ($language_id == 0) $language_id = $languages_id;
    $product_query = tep_db_query("select products_name_suffix from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$product_id . "' and language_id = '" . (int)$language_id . "'");
    $product = tep_db_fetch_array($product_query);

    return $product['products_name_suffix'];
  }

    function tep_get_products_name_prefix($product_id, $language_id = 0) {
    global $languages_id;

    if ($language_id == 0) $language_id = $languages_id;
    $product_query = tep_db_query("select products_name_prefix from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$product_id . "' and language_id = '" . (int)$language_id . "'");
    $product = tep_db_fetch_array($product_query);

    return $product['products_name_prefix'];
  }
...
//Wishlist addition to delete products from the wishlist when deleted
  ...
  function tep_remove_order($order_id, $restock = false) {
    ...
    tep_db_query("delete from affiliate_sales where affiliate_orders_id = '" . (int)$order_id . "'");
  }
...
// vendors add in
function tep_get_vendors_terms_choices() {
	  
      $vendors_terms_query = tep_db_query("select vendors_terms_id, vendors_terms from " . TABLE_VENDORS_TERMS . " order by vendors_terms");
      $vendors_list_terms_count = mysql_num_rows($vendors_terms_query);
      if ($vendors_list_terms_count == 0) { $vendors_list_array[] = array('id' => '0', 'text' => 'No Terms Available'); } else {
      $vendors_terms_array[] = array('id' => '0', 'text' => 'Select Terms'); 
       while ($vendors_list_terms = tep_db_fetch_array($vendors_terms_query)) {
      $vendors_terms_array[] = array('id' => $vendors_list_terms['vendors_terms_id'], 'text' => $vendors_list_terms['vendors_terms']);
    }          
    return $vendors_terms_array;
  } }  

function tep_get_vendors() {
	  
      $vendors_list_query = tep_db_query("select vendors_id, vendors_name from " . TABLE_VENDORS . " order by vendors_name");
      $vendors_list_query_count = mysql_num_rows($vendors_list_query);
      if ($vendors_list_query_count == 0) { $vendors_list_array[] = array('id' => '0', 'text' => 'No Vendors Available'); } else {
      $vendors_list_array[] = array('id' => '0', 'text' => 'Select Vendor');
    while ($vendors_list = tep_db_fetch_array($vendors_list_query)) {
      $vendors_list_array[] = array('id' => $vendors_list['vendors_id'], 'text' => $vendors_list['vendors_name']);
    }      }

    return $vendors_list_array;
  } 
 
function tep_output_display_vendors($vend_prod_id){
	$ven_list = '';
	$current_vendors_query = tep_db_query("select v.vendors_id, v.vendors_name from " . TABLE_VENDORS . " v, " . TABLE_PRODUCTS_TO_VENDORS . " vp where vp.products_id = '" . $vend_prod_id . "' and v.vendors_id = vp.vendors_id");
	while ($current_vend_list = tep_db_fetch_array($current_vendors_query)) {
		$ven_list .= "&nbsp;".$current_vend_list['vendors_name']."<br>";
		}
		return $ven_list;
}

function tep_output_display_vendors_checkbox($vend_prod_id){
	$ven_list = '';
	
	$current_vendors_query = tep_db_query("select v.vendors_id, v.vendors_name from " . TABLE_VENDORS . " v, " . TABLE_PRODUCTS_TO_VENDORS . " vp where vp.products_id = '" . $vend_prod_id . "' and v.vendors_id = vp.vendors_id");
	while ($current_vend_list = tep_db_fetch_array($current_vendors_query)) {
		$ven_list .= tep_draw_checkbox_field('vnid[]', $current_vend_list['vendors_id'])."&nbsp;".$current_vend_list['vendors_name']."<br>";
...
function tep_get_vendor_terms($terms_id) {
    $vendor_query = tep_db_query("select vendors_terms from " . TABLE_VENDORS_TERMS . " where vendors_terms_id = '" . (int)$terms_id . "'");
    $vendor = tep_db_fetch_array($vendor_query);

    return $vendor['vendors_terms'];
  }		

/////////// START UPDATIONS //////////////////

function tep_get_vendor_product_payment_type($type_id) {
    $vendor_query = tep_db_query("select vendors_product_payment_type from " . TABLE_VENDORS_PRODUCT_PAYMENT_TYPE . " where vendors_product_payment_type_id = '" . (int)$type_id . "'");
    $vendor = tep_db_fetch_array($vendor_query);

    return $vendor['vendors_product_payment_type'];
  }

function tep_get_vendors_product_sale_type($type_id) {
    $vendor_query = tep_db_query("select vendors_product_sale_type from " . TABLE_VENDORS_PRODUCT_SALE_TYPE  . " where vendors_product_sale_type_id = '" . (int)$type_id . "'");
    $vendor = tep_db_fetch_array($vendor_query);

    return $vendor['vendors_product_sale_type'];
  }

 function tep_get_product_payment_type() {
       $vendors_payment_query = tep_db_query("select vendors_product_payment_type_id, vendors_product_payment_type from " . TABLE_VENDORS_PRODUCT_PAYMENT_TYPE . " order by vendors_product_payment_type_id");
       $vendors_list_payment_count = mysql_num_rows($vendors_payment_query);
       if ($vendors_list_payment_count == 0) { $vendors_list_array[] = array('id' => '0', 'text' => 'No payment Available'); } else {
       $vendors_payment_array[] = array('id' => '0', 'text' => 'Select Payment Type');
        while ($vendors_list_payment = tep_db_fetch_array($vendors_payment_query)) {
       $vendors_payment_array[] = array('id' => $vendors_list_payment['vendors_product_payment_type_id'], 'text' => $vendors_list_payment['vendors_product_payment_type']);
     }
     return $vendors_payment_array;
   } }

 function tep_get_product_sale_type() {
       $vendors_sale_query = tep_db_query("select vendors_product_sale_type_id, vendors_product_sale_type from " . TABLE_VENDORS_PRODUCT_SALE_TYPE . " order by vendors_product_sale_type_id");
       $vendors_list_sale_count = mysql_num_rows($vendors_sale_query);
       if ($vendors_list_sale_count == 0) { $vendors_list_array[] = array('id' => '0', 'text' => 'No Sale Available'); } else {
       $vendors_sale_array[] = array('id' => '0', 'text' => 'Select Sale Type');
        while ($vendors_list_sale = tep_db_fetch_array($vendors_sale_query)) {
       $vendors_sale_array[] = array('id' => $vendors_list_sale['vendors_product_sale_type_id'], 'text' => $vendors_list_sale['vendors_product_sale_type']);
     }
     return $vendors_sale_array;
   } }

  function tep_get_vendors_consignment_type($type_id)
  {
	if ($type_id <> '')
	{
		if ($type_id == '1')
			$vendor_consginment = 'N/A';
		elseif ($type_id == '2')
			$vendor_consginment = 'Fixed';
		elseif ($type_id == '3')
			$vendor_consginment = 'Variable';
	}
	return $vendor_consginment;
  }

  function tep_get_vendors_royalty($type_id)
  {
	if ($type_id <> '')
	{
		if ($type_id == '1')
			$vendor_royalty = 'N/A';
		elseif ($type_id == '2')
			$vendor_royalty = 'Fixed';
		elseif ($type_id == '3')
			$vendor_royalty = 'Variable';
	}
	return $vendor_royalty;
  }

  function tep_get_consignment()
  {
	$vendor_consginment_array[] = array('id' => '1', 'text' => 'N/A');
	$vendor_consginment_array[] = array('id' => '2', 'text' => 'Fixed');
	$vendor_consginment_array[] = array('id' => '3', 'text' => 'Variable');

	return $vendor_consginment_array;
  }

  function tep_get_royalty()
  {
	$vendor_royalty_array[] = array('id' => '1', 'text' => 'N/A');
	$vendor_royalty_array[] = array('id' => '2', 'text' => 'Fixed');
	$vendor_royalty_array[] = array('id' => '3', 'text' => 'Variable');

	return $vendor_royalty_array;
  }

  function tep_get_vendors_products($prod_id) {
	$vendors_list_query = tep_db_query("select a.vendors_id, b.vendors_name from " . TABLE_PRODUCTS_TO_VENDORS ." a, ". TABLE_VENDORS ." b ". " where b.vendors_id=a.vendors_id and a.products_id=". $prod_id ." order by b.vendors_name");
	$vendors_list_query_count = mysql_num_rows($vendors_list_query);
	if ($vendors_list_query_count == 0) { $vendors_list_array[] = array('id' => '0', 'text' => 'No Vendors Available'); } else {
	$vendors_list_array[] = array('id' => '0', 'text' => 'Select Vendor');
  while ($vendors_list = tep_db_fetch_array($vendors_list_query)) {
