...
if ($action=='add_product') $product_id = $_POST['add_product_products_id'];
elseif ($action=='add_product_model') $product_id = $cnt['products_id'];
else	$product_id = $order['products_id'];
...
//mail('x0661t@d-net.kiev.ua', 'test', $sql_query);
    ...
    if ($prod_vendor['vendors_product_payment_type']=='1')	$final_prod_vendor = $prod_vendor['vendors_item_cost'];

    if ($prod_vendor['vendors_product_payment_type']=='2'){
    ...
    if ($prod_vendor['vendors_product_payment_type']=='3'){
	...
	$_sql = "update ".TABLE_ORDERS_PRODUCTS." set products_sale_type='".$prod_vendor['vendors_product_payment_type']."', products_item_cost='".$final_prod_vendor."' where orders_products_id='".$orders_products_id."'";
	tep_db_query($_sql);
