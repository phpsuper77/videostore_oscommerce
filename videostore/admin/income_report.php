...
<? if ($_GET[action]==''){ ?>
		...
		<tr class="dataTableHeadingRow">
			...
			<td class="dataTableHeadingContent">PayPal Sales</td>
                        ...
                        <td class="dataTableHeadingContent">Ebay Sales</td>
                        <td class="dataTableHeadingContent">License Sales</td>
...
<?
		...
		$cs_total = tep_db_fetch_array(tep_db_query("select sum(round(ot.value,2)) as total from orders o left join orders_total ot on (o.orders_id = ot.orders_id) where ot.class = 'ot_total' and o.payment_method like '%customflix%' and DATE_FORMAT(o.date_purchased,'%Y-%m')='".$year."-".$index."'"));
                             $es_total = tep_db_fetch_array(tep_db_query("select sum(round(ot.value,2)) as total from orders o left join orders_total ot on (o.orders_id = ot.orders_id) where ot.class = 'ot_total' and o.payment_method like '%ebay%' and DATE_FORMAT(o.date_purchased,'%Y-%m')='".$year."-".$index."'"));
                             $ls_total = tep_db_fetch_array(tep_db_query("select sum(round(ot.value,2)) as total from orders o left join orders_total ot on (o.orders_id = ot.orders_id) where ot.class = 'ot_total' and o.payment_method like '%license%' and DATE_FORMAT(o.date_purchased,'%Y-%m')='".$year."-".$index."'"));
...
?>
		...
		<td class="dataTableContent"><?=$currencies->format($es_total[total],true,'USD','1.000000')?></td>
		<td class="dataTableContent"><?=$currencies->format($ls_total[total],true,'USD','1.000000')?></td>
...
<?                      $gt = $gt + $g_total[total];
			...
			$est = $est + $es_total[total];
			$lst = $lst + $ls_total[total];
...
?>
		<tr class="dataTableRow">
			...
			<td class="dataTableContent"><b><?=$currencies->format($est,true,'USD','1.000000')?></b></td>
			<td class="dataTableContent"><b><?=$currencies->format($lst,true,'USD','1.000000')?></b></td>
	...
	<tr class="dataTableHeadingRow">
		...
		<td class="dataTableHeadingContent">Unidentified CC Sales</td>
						...
						<tr class="dataTableRow">
							...
							<td class='dataTableContent' width="10%" align="center">
							<?
							$result = tep_db_fetch_array(tep_db_query("select sum(round(ot.value,2)) as total from orders o left join orders_total ot on (o.orders_id = ot.orders_id) where ot.class = 'ot_total' and o.payment_method like '%ebay%' and DATE_FORMAT(o.date_purchased,'%Y-%m-%d')='".$year."-".$month."-".$d_index."'"));
							if(intval($result[total])!=0)
							echo $currencies->format($result[total], 'true', 'USD', '1.000000')."&nbsp;<a href='income_report.php?action=day_detail&year=".$year."&month=".$month."&day=".$d_index."&type=12'>Details</a>"; else echo "&nbsp;";
							$es_total = $es_total+$result[total];
							?>
...
<td class='dataTableContent' width="10%" align="center">
							<?
							$result = tep_db_fetch_array(tep_db_query("select sum(round(ot.value,2)) as total from orders o left join orders_total ot on (o.orders_id = ot.orders_id) where ot.class = 'ot_total' and o.payment_method like '%license%' and DATE_FORMAT(o.date_purchased,'%Y-%m-%d')='".$year."-".$month."-".$d_index."'"));
							if(intval($result[total])!=0)
							echo $currencies->format($result[total], 'true', 'USD', '1.000000')."&nbsp;<a href='income_report.php?action=day_detail&year=".$year."&month=".$month."&day=".$d_index."&type=13'>Details</a>"; else echo "&nbsp;";
							$ls_total = $ls_total+$result[total];
							?>
							...
							if(intval($result[total])!=0)
							echo $currencies->format($result[total], 'true', 'USD', '1.000000')."&nbsp;<a href='income_report.php?action=day_detail&year=".$year."&month=".$month."&day=".$d_index."&type=14'>Details</a>"; else echo "&nbsp;";
      ...
      <tr class='dataTableRow'>
		...
		<td width="10%" class="dataTableContent" align="center"><?=$currencies->format($es_total, 'true', 'USD', '1.000000')?></td>
		<td width="10%" class="dataTableContent" align="center"><?=$currencies->format($ls_total, 'true', 'USD', '1.000000')?></td>
...
if ($type == 12) {
		echo "Ebay Sale";
	$prod = tep_db_query("select o.orders_id, o.date_purchased, ot.value, o.payment_method, o.cc_type  from orders o left join orders_total ot on (o.orders_id = ot.orders_id) where ot.class = 'ot_total' and o.payment_method like '%ebay%' and DATE_FORMAT(o.date_purchased,'%Y-%m-%d')='".$year."-".$month."-".$day."'");
}
if ($type == 13) {
		echo "License Sale";
	$prod = tep_db_query("select o.orders_id, o.date_purchased, ot.value, o.payment_method, o.cc_type  from orders o left join orders_total ot on (o.orders_id = ot.orders_id) where ot.class = 'ot_total' and o.payment_method like '%license%' and DATE_FORMAT(o.date_purchased,'%Y-%m-%d')='".$year."-".$month."-".$day."'");
}
if ($type == 14) {
