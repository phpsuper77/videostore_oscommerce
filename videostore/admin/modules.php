  ...
  if (tep_not_null($set)) {
    switch ($set) {
      case 'shipping':
        ...
        break;
      case 'giftwrap':
        $module_type = 'giftwrap';
        $module_directory = DIR_FS_CATALOG_MODULES . 'giftwrap/';
        $module_key = 'MODULE_GIFTWRAP_INSTALLED';
        define('HEADING_TITLE', HEADING_TITLE_MODULES_GIFTWRAP);
        break;
...
// USPS START
//        $contents[] = array('text' => '<br>' . $keys);
        $contents[] = array('text' => '<br />' . preg_replace(array('/RM/', '/TM/', '/International/', '/Envelope/'), array('&reg;', '&trade;', 'Intl', 'Env'), $keys));
// USPS END
      } else {
