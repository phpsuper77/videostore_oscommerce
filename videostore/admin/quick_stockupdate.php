...
$msg_stack = 'Fetched products from category';
 ...
 if ($HTTP_POST_VARS['stock_update']) {
  ...
  while (list($key, $items) = each($stock_update)) {
   ...
   $sql = "UPDATE products SET customerflix_id = '". $items['customerflix_id'] ."', products_always_on_hand = ".$items['always'].", products_quantity = ".$items['stock'].", products_warehouse_location = '".$items['location']."', products_model = '".$items['model']."', products_upc = '".$items['upc']."', products_isbn = '".$items['isbn']."', products_asin = '".$items['asin']."', products_status = '".$items['status']."', products_image = '".$items['image']."', products_image_med = '".$items['imagemed']."', products_image_lrg = '".$items['imagelrg']."', products_price = ". $items['products_price'] .", products_out_of_print = ". $items['products_out_of_print'] .",products_distribution = '". $items['products_distribution'] ."' WHERE products_id = $key";//NEWLY INSERTED

   $update = tep_db_query($sql);

$sql = "UPDATE products_description SET products_clip_url = '".$items['clipurl']."' WHERE products_id = $key";//NEWLY INSERTED
   $update = tep_db_query($sql);
...
<?php
  ...
  if ($HTTP_POST_VARS['cat_id']) {
      ...
      // start the table
      echo '<form method="post" action="quick_stockupdate.php"><table border="0" width="100%"><tr>';
       $i = 0;

      // get all active prods in that specific category

       $sql2 = tep_db_query("SELECT p.customerflix_id, p.products_always_on_hand, p.products_distribution, p.products_id, p. products_quantity, p. products_model, p.products_price, p.products_out_of_print, p.products_status, p.products_image, p.products_image_med, p.products_image_lrg, p.products_warehouse_location, p.products_upc, p.products_isbn, p.products_asin,  pd.products_head_title_tag, pd.products_name, pd.products_clip_url from products p, products_to_categories ptc, products_description pd where p.products_id = ptc.products_id and p.products_id = pd.products_id and  language_id = $languages_id and ptc.categories_id = '" . $HTTP_POST_VARS['cat_id'] . "' order by p. products_model");
...
if ($results['products_always_on_hand']==1) {
	$flag1 = 'checked'; $flag2 = '';
}
	else{
	$flag1 = ''; $flag2 = 'checked';
}
			 echo '<input type="radio" name="stock_update[' . $results['products_id'] . '][always]" value="1" '.$flag1.'>YES&nbsp;&nbsp;&nbsp;<input type="radio" name="stock_update[' . $results['products_id'] . '][always]" value="0" '.$flag2.'>NO&nbsp;&nbsp;&nbsp;&nbsp;always in catalog<br><i>';
			 echo '<input type="text" size="12" name="stock_update[' . $results['products_id'] . '][customerflix_id]" value="' . $results['customerflix_id'] . '">customerflix_id<br><i>';
			 ...
			 echo '<input type="text" size="12" name="stock_update[' . $results['products_id'] . '][clipurl]" value="' . $results['products_clip_url'] . '">Clip<br><i>';
...
echo '<input type="text" size="1" name="stock_update[' . $results['products_id'] . '][products_distribution]" value="' . $results['products_distribution'] . '">Products on Distribution<br><i>';//NEWLY ADDED
