...
?>
<br><form method="post" action="quick_stockupdate_a1.php">
<?php
   ...
   // first select all categories that have 0 as parent:
      $sql = tep_db_query("SELECT c.categories_id, cd.categories_name from categories c, categories_description cd WHERE c.parent_id = 0 AND c.categories_id = cd.categories_id AND cd.language_id = 1");
	   ...
	   if (tep_db_num_rows($check) > 0) {
              ...
              $all_list = '<form method="post" action="quick_stockupdate_a1.php"><th class="smallText" align="left" valign="top">All categories:<br>' . $dropdown . '</form></th>';
           ...
           // get the tree for that parent
             ...
             // draw a dropdown with it:
                ...
                $list .= '<form method="post" action="quick_stockupdate_a1.php"><th class="smallText" align="left" valign="top">' . $parents['categories_name'] . '<br>' . $dropdown . '</form></th>';
        }
       }
  ...
  if ($HTTP_POST_VARS['cat_id']) {
      ...
      // start the table
      echo '<form method="post" action="quick_stockupdate_a1.php"><table border="0" width="100%"><tr>';
       $i = 0;

      // get all active prods in that specific category

       $sql2 = tep_db_query("SELECT p.products_id, p. products_quantity, p. products_model, p.products_price, p.products_out_of_print, p.products_status, p.products_image, p.products_image_med, p.products_image_lrg, p.products_warehouse_location, p.products_upc, p.products_isbn, p.products_asin,  pd.products_head_title_tag, pd.products_name from products p,  products_description pd where p.products_id=pd.products_id and p.products_warehouse_location < 'B1' order by p.products_warehouse_location");
// added changes for image update thowden 10/2004 stock_update becomes a multi-dim array
 ...
 echo '<input type="text" size="20" name="stock_update[' . $results['products_id'] . '][model]" value="' . $results['products_model'] . '">model<br><i>';
  ...
  echo '<input type="checkbox" name="update_status">Check to set status on each individual product based on items in stock<br><i>( one or more in stock will become <font color="009933"><b>active</b></font> / a1 in stock will become <font color="ff0000"><b>not active</b></font> )</i><p>';
