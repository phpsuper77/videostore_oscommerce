  ...
  if (tep_not_null($action)) {
    switch ($action) {
      ...
      case 'save':
		...
		//changed by tamil
		$series_description=tep_db_prepare_input($HTTP_POST_VARS['series_description']);

        $sql_data_array = array('series_name' => $series_name,'series_description' => $series_description);
...
<?php
  $series_query_raw = "select series_id, series_name,series_description, series_image, date_added, last_modified from " . TABLE_SERIES . " order by series_name";
...
<?php
  ...
  switch ($action) {
    case 'new':
      ...
      $contents[] = array('text' => '<br>' . 'Series Description :' . '<br>' . tep_draw_textarea_field('series_description', 'soft', '40', '10'));
    ...
    case 'edit':
      ...
      $contents[] = array('text' => '<br>' . 'Series Description :' . '<br>' . tep_draw_textarea_field('series_description' ,'soft',  '40', '10',$mInfo->series_description));
