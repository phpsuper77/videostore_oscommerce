...
$request_referer = 'http://www.travelvideostore.com/'; // base url for site, e.g.: 'yoursite.com'
...
// start the array for fedex
      ...
      ,13=>   trim($order_info['delivery_street_address']." ".$order_info['delivery_suburb']) // delivery address
      ...
      ,38 => $HTTP_POST_VARS['package_department'] // department name
			...
			,1266 => $HTTP_POST_VARS['saturday_delivery'] // Saturday delivery
			,1200 => $HTTP_POST_VARS['hold_at_location'] // Hold At Fedex Location
				,44 => $HTTP_POST_VARS['hal_address'] // Hold At Location address
				,46 => $HTTP_POST_VARS['hal_city'] // Hold At Location city
				,47 => $HTTP_POST_VARS['hal_state'] // Hold At Location state
				,48 => $HTTP_POST_VARS['hal_postcode'] // Hold At Location postal code
				,49 => $HTTP_POST_VARS['hal_phone'] // Hold At Location phone number
	  	,57 => $HTTP_POST_VARS['dim_height'] // "your package" height dimension
	  	,58 => $HTTP_POST_VARS['dim_width'] // "your package" width dimension
	  	,59 => $HTTP_POST_VARS['dim_length'] // "your package length dimension
	  	,3008 => $HTTP_POST_VARS['autopod'] // Automatic Proof of Delivery
	    );
...
// if it's a Saturday delivery
		if ($HTTP_POST_VARS['service_type'] == 03 && $HTTP_POST_VARS['saturday_delivery'] == 1) {
			$shipData[1266] = 'Y';
			}
		if ($HTTP_POST_VARS['service_type'] == 01 && $HTTP_POST_VARS['saturday_delivery'] == 1) {
			$shipData[1266] = 'Y';
			}
		else {
			$shipData[1266] = 'N';
...
// if it's a Hold At Fedex Location delivery
		if ($HTTP_POST_VARS['hold_at_location'] == 1) {
			$shipData[1200] = 'Y';
			}
		else {
			$shipData[1200] = 'N';
...
// data for shipping_manifest
			...
			delivery_company => '',
			delivery_address_1 => $order_info['delivery_street_address']." ".$order_info['delivery_suburb'],
			delivery_address_2 => '',
			...
			hal_address => $hal_address,
			hal_city => $hal_city,
			hal_state => $hal_state,
			hal_postcode => $hal_postcode,
			hal_phone => $hal_phone,
...
//echo "update " . TABLE_ORDERS . " set fedex_tracking='" . $trackNum . "' where orders_id = " . $order . "";
...
// ...mark the order record "delivered"...
			...
			//tep_db_query("update " . TABLE_ORDERS . " set orders_status='3' where orders_id = " . $order . "");
...
// ...mark the order record "cancelled"...
			$update_status = array ('orders_status' => 12);
			tep_db_perform(TABLE_ORDERS, $update_status, 'update', "orders_id = '" . $order . "'");
...
?>
          <tr>
            ...
            <td class="pageHeading" align="right"><?php echo '<a href="' . tep_href_link(FILENAME_ORDERS, 'oID=' . $order.'&page='.$page) . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a>'; ?></td>
          </tr>
          ...
          <?php echo $order_info['delivery_street_address']." ".$order_info['delivery_suburb']; ?><br>&nbsp;
          	...
          	<tr>
          	  <td colspan="3"><?php echo tep_draw_separator('pixel_trans.gif', '1', '15'); ?></td>
						...
						<tr>
							...
							<td class="main">&nbsp;</td>
<!--$order_qty-->
							<td class="main"><?php echo tep_draw_input_field('package_num',1,'size="2"'); ?></td>
						</tr>
						<tr>
