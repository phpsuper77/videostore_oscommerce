<?php
/*
  $Id: stats_monthly_sales.php, v 2.1.1 2004/04/14  $
...
*/
  ...
  $sel_month = $_GET['month']?$_GET['month']:$_POST['month'];
  $sel_year = $_GET['year']?$_GET['year']:$_POST['year'];

if ($sel_year == '') $sel_year = date("Y", time());;

if ($sel_month == 'All') $sel_month = '0';
if ($sel_month == '') $sel_month = date("m", time());
...
if (!$print) { ?>
			...
			<table border="0" width="100%" cellspacing="0" cellpadding="0">
			  <tr><td class="smallText" align="right">
				<form action="<? echo FILENAME_STATS_MONTHLY_SALES; ?>" method="get">
				<?
				// get list of orders_status names for dropdown selection
				  $orders_statuses = array();
				  $orders_status_array = array();
                ...
                echo HEADING_TITLE_STATUS . ': ' . tep_draw_pull_down_menu('status', array_merge(array(array('id' => '', 'text' => TEXT_ALL_ORDERS)), $orders_statuses), '', ''); ?><br/>
<?
		$month_list[] = array('id'=>'all', 'text'=>'All');
		for ($i=1;$i<13;$i++) {
			if (strlen($i)==1) $ind = "0".$i; else $ind = $i;
		$month_list[] = array('id'=>$ind, 'text'=>date("F", mktime(0, 0, 1, $ind, 1, '2007')));
		}
                 echo 'Month: ' . tep_draw_pull_down_menu('month', $month_list, $sel_month, ''); ?><br/>
<?
		for ($i=2003; $i<date("Y")+1;$i++) 	$year_list[] = array('id' => $i, 'text' => $i);

                 echo "Year: " . tep_draw_pull_down_menu('year', $year_list, $sel_year, '', ''); ?><br/>
...
if ($sel_month!='all')  $str = "and month(o.date_purchased) = " . $sel_month."";
$sales_query_raw .= $str." and year(o.date_purchased) = " . $sel_year;
...
//echo "<b>".$net_sales_query_raw."</b><br/>";
...
//echo "<b>" . $tax_coll_query_raw . "</b><br/>";
...
/*====== Error was here last update was on 9.10.2007 ============*/
	...
	$taxed_query_raw = "select distinct sum(round(t0.value,2)) taxed_sales from " . TABLE_ORDERS_TOTAL . " t0, " .  TABLE_ORDERS . " o left join " . TABLE_ORDERS_TOTAL . " t1 on (o.orders_id = t1.orders_id) where t0.orders_id=t1.orders_id ";
	...
	if ($sel_month<>0) {
		...
		$taxed_query_raw .= " and dayofmonth(o.date_purchased) = '" . $sales['row_day'] . "'";
	} else {
		$taxed_query_raw .= " group by month(o.date_purchased)";
	};
//echo $taxed_query_raw."<hr/>";
