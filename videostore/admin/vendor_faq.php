...
switch($faq_action) {
  ...
  case "Added":
	$data = browse_faq_vendor($language,$HTTP_GET_VARS);
	$no = 1;
	if (sizeof($data) > 0) {
	...
	$title = FAQ_ADD . ' #' . $no;
	echo tep_draw_form('','vendor_faq.php', 'faq_action=AddSure');
	include('vendor_faq_form.php');
  break;

  case "AddSure":
	function add_faq ($data) {
	  $query = "INSERT INTO " . TABLE_FAQ . " VALUES(null, '$data[visible]', '$data[v_order]', '$data[question]', '$data[answer]', NOW(''),'$data[faq_language]','2')";
	  tep_db_query($query);
  ...
  case "Edit":
	if ($faq_id) {
	  ...
	  echo tep_draw_form('','vendor_faq.php', 'faq_action=Update');
  ...
  case "Delete":
	if ($faq_id) {
	  ...
	  echo '
	  	...
	  	<tr><td align="right">
	  ';
	  echo tep_draw_form('','vendor_faq.php', 'faq_action=DelSure&faq_id='.$val[faq_id]);
	  echo tep_draw_hidden_field('faq_id', $faq_id);
	  echo tep_image_submit('button_delete.gif', IMAGE_DELETE);
	  echo '<a href="' . tep_href_link('vendor_faq.php', '', 'NONSSL') . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>';
	  echo '</form></td></tr>';
	} else {
	  $error = 80;
	}
