...
<!-- body_text //-->
    <td width="100%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
	  ...
	  <tr><Td colspan="2"><input type="button" value="Show All"  onclick="location.href='whos_online.php'"/></td></tr>
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
...
$gi = geoip_open(DIR_WS_INCLUDES . 'GeoIP.dat', GEOIP_STANDARD);
  ...
  $whos_online_query_full21 = tep_db_query("select customer_id, full_name, ip_address, time_entry, time_last_click, last_page_url, http_referer, session_id from " . TABLE_WHOS_ONLINE . " order by time_last_click desc");
$s12=1000;
$per2 = tep_db_num_rows($whos_online_query_full21);
$per21 = $per2/$s12;

//$s=1000;
if (!isset($_GET['ctr'])) $ctr21=0; else $ctr21=$_GET['ctr'];
$st12 = $ctr21*$s12;
$limit21=" limit $st12, $s12";


  $qq = "select customer_id, full_name, ip_address, time_entry, time_last_click, last_page_url, http_referer, session_id from " . TABLE_WHOS_ONLINE . " order by time_last_click desc ".$limit21;
  $whos_online_query = tep_db_query($qq);
...
if ((tep_not_null($total_content_qty)) && ($total_content_qty <> 0)){
    ...
    if ($whos_online['session_id'] == $info) {
      ...
      echo '              <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(FILENAME_WHOS_ONLINE_CART, tep_get_all_get_params(array('info', 'action')) . 'info=' . $whos_online['session_id'], 'NONSSL') . '\'">' . "\n";
    }
...
?>
       ...
       <?
			...
			for ($kt=1;$kt<$per21+1;$kt++){
				$t21=$kt-1;
				echo '<option value="whos_online_cart.php?ctr='.$t21.'">'.$kt.'</option>'; 
			}
	 ?>
