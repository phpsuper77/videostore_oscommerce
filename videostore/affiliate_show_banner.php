...
//
  ...
  define('TABLE_CATEGORIES', 'categories'); // NEWLY ADDED
...
// Register needed Post / Get Variables
  ...
  if ($HTTP_POST_VARS['affiliate_pbanner_id']) $prod_banner_id = $HTTP_POST_VARS['affiliate_pbanner_id'];
   ...
   ##########Included newly
   if ($HTTP_POST_VARS['affiliate_cbanner_id']) $cat_banner_id = $HTTP_POST_VARS['affiliate_cbanner_id'];
   if ($HTTP_GET_VARS['affiliate_cbanner_id']) $cat_banner_id = $HTTP_GET_VARS['affiliate_cbanner_id'];
   #########
  ...
  if ($prod_banner_id) {
	...
	#########Included for category Images
	if ($cat_banner_id) {

	  $sql = "select categories_affiliate_banner1  from " . TABLE_CATEGORIES . " where categories_id = '" . $cat_banner_id  . "' and categories_status = 1";
	  $banner_values = tep_db_query($sql);
	  if ($banner_array = tep_db_fetch_array($banner_values)) {
		$banner = $banner_array['categories_affiliate_banner1'];
		$categories_id = $prod_banner_id;
	  }
	}
	#########
...
// Show default Banner if none is found
  if (is_file(AFFILIATE_SHOW_BANNERS_DEFAULT_PIC)) {
    ...
    echo "<br> $banner $categories_id "; // Output something to prevent endless loading
  }
  exit();
?>