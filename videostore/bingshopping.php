...
header("Content-Disposition: attachment;filename=bingshopping.txt");
	...
	$query = "SELECT  pd.products_head_keywords_tag, pd.products_url, pd.products_name , pd.products_head_keywords_tag, pd.products_description , "
	               ...
	               ." p.manufacturers_id, p.products_distribution , p.products_weight , p.products_status, p.products_media_type_id , p.products_set_type_id , p.products_video_format_id , p.products_packaging_type_id , p.products_audio_id, p.products_aspect_ratio_id, p.products_run_time, p.products_region_code_id , p.products_date_available "
...
$check = mktime($result_row['products_date_available']);
$today = time();
		...
		{
			#$merchantCat.=">".$category['categories_name'];
		}
		#$merchantCat =substr(ltrim($categoryname),1);
		###############################
...
echo tep_href_link('product_info.php','products_id='.$result_row[products_id])."&ref=115775\t";
...
if (tep_not_null($result_row['products_isbn'])) { 
 if(strlen($isbn) < 10) {
		...
		echo "0\t";
                                echo "Travel Videos\t";
		echo $result_row['products_model']."\t";
		echo "Movies>Special Interest>Travel";
		echo $result_row['products_weight']."\t";
		echo $quantity."\t";
