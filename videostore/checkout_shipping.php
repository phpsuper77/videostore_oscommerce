...
// if the order contains only zero weight products, forward the customer to the billing page as
// a shipping address is not needed
...
// load giftwrap module
  ...
  $giftwrap_modules = new gift;
	...
	if ($HTTP_POST_VARS['giftwrap'] == '1')  //THIS LOOP IS TO SELECT ONLY ONE OPTION
	{
		tep_session_register('giftwrap');
		tep_session_register('giftwrap_info');
	}
	else
	{
...
// process the selected giftwrap method
  if ( isset($HTTP_POST_VARS['action']) && ($HTTP_POST_VARS['action'] == 'process')  && $HTTP_POST_VARS['giftwrap'] <> '') {
...
//var_dump($quotes);
if ($cart->show_weight() != 0){
//if ($whole[iswholesale] == 1 && $cart->show_weight() != 0){
...
// method if more than one module is now enabled
 ...
 //bof modified to fix cheapest selection with freecount module
if ((MODULE_SHIPPING_FREECOUNT_STATUS == 'True') && ($order->delivery['country_id'] == STORE_COUNTRY) ) {
$cheapship = 'freecount_freecount'; 
}else {
$cheapship = $shipping_modules->cheapest();
}
 ...
 //eof  
  ...
  if ($cheapestOne) 
  {
  $shipping[id] = 'freecount_freecount';
  }
  
  if($quotes[0][id] != 'freecount')
  {
...
<!-- body_text //-->
	...
	<!--Ends here-->
	  ...
	  ?>
			  ...
			  <td class="main" width="50%" valign="top"><?php echo TEXT_CHOOSE_GIFTWRAP_METHOD; ?></td>
	                  <td class="main" width="50%" valign="top" align="right"><?php echo '<b>' . TITLE_PLEASE_SELECT . '</b><br>' . tep_image(DIR_WS_IMAGES . 'arrow_east_south.gif'); ?></td><!--INSERTED NEWLY-->
	                  <td><?php echo tep_draw_separator('pixel_trans.gif', '10', '1'); ?></td>
...
<?php
           ...
           for ($i=0, $n=sizeof($quotes); $i<$n; $i++) {
			    for ($j=0, $n2=sizeof($quotes[$i]['methods']); $j<$n2; $j++) {
					 if($min_mums>$quotes[$i]['methods'][$j]['cost']){
					    $min_mums = $quotes[$i]['methods'][$j]['cost'];
					  }
				}
		   }
...
//echo $shipping[id];
              $checked=false;
//			  echo $quotes[$i]['id'] . '_' . $quotes[$i]['methods'][$j]['id'];

             if($quotes[$i]['id'] . '_' . $quotes[$i]['methods'][$j]['id']==""){
			      if($min_mums==$quotes[$i]['methods'][$j]['cost']){ $checked = (true); }else{  $checked = (false); }
			 }else{
			      $checked = (($quotes[$i]['id'] . '_' . $quotes[$i]['methods'][$j]['id'] == $shipping['id']) ? true : false);
			 }

            if ( ($min_mums==$quotes[$i]['methods'][$j]['cost']) || ($n == 1 && $n2 == 1) ) {
              echo '                  <tr id="defaultSelected" class="moduleRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="selectRowEffect(this, ' . $radio_buttons . ')">' . "\n";
            } else {
              echo '                  <tr class="moduleRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="selectRowEffect(this, ' . $radio_buttons . ')">' . "\n";
            }
...
?>
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="2">
          <tr>
            <td class="main"><b>Selecting a Shipping Method :</b>
</td>
          </tr>
		  <tr>
