...
header("Content-Disposition: attachment;filename=frooglefeedfileau.txt");

	require('includes/application_top.php');


$rt = tep_db_fetch_array(tep_db_query("select value from currencies where code='AUD' limit 1"));
	...
	$query = "SELECT  pd.products_head_keywords_tag, pd.products_url, pd.products_name , pd.products_head_keywords_tag, pd.products_description , "
	               ...
	               ." p.manufacturers_id, p.products_distribution , p.products_weight , p.products_status, p.products_media_type_id , p.products_set_type_id , p.products_video_format_id , p.products_packaging_type_id , p.products_audio_id, p.products_aspect_ratio_id, p.products_run_time, p.products_region_code_id, p.products_status "
...
echo tep_href_link('product_info.php','products_id='.$result_row[products_id])."?ref=115119&currency=AUD\t";
...
$title = " ";
 ...
 if(strlen($title) < 56) {
		...
		if($result_row['products_image_lrg'] !="")
			...
			echo "\t";
		echo $result_row['products_model']."-AUD\t";
                                echo $feature."\t";
		echo $instock."\t";
