...
// user_tracking modifications 
   ...
   if (!isset($_SESSION[tep_session_name()]) || $_SESSION[tep_session_name()] == ''){
	$_SESSION[tep_session_name()] = tep_session_id();
   }
   // echo $_SESSION[tep_session_name()]."<br />";
   // echo tep_session_id()."<br />";
   // echo session_id()."<br />";
   // echo var_dump($session_started)."<br />";

if (empty($_SESSION['referer_url_real']))
		$_SESSION['referer_url_real'] = $referer_url;

$referer_url_real = $_SESSION['referer_url_real'];
//var_dump($cart);
...
//Wishlist actions (must be before shopping cart actions)
  if(isset($HTTP_POST_VARS['wishlist_x'])) {
	...
	if(isset($HTTP_POST_VARS['wish_product_id'])) {
		if(isset($HTTP_POST_VARS['id'])) {
			$attributes_id = $HTTP_POST_VARS['id'];
			tep_session_register('attributes_id');
		}
		$wishlist_id = $HTTP_POST_VARS['wish_product_id'];
		tep_session_register('wishlist_id');
	}
	tep_redirect(tep_href_link(FILENAME_WISHLIST)."?wishlist_id=".$products_id);
  }
...
// redirect the customer to a friendly cookie-must-be-enabled page if cookies are disabled
    ...
    //Start New function for multiple addition********
         ...
         // exit();
          ...
          $cart->add_cart($w, $cart->get_quantity(tep_get_uprid($w, $HTTP_POST_VARS['id']))+1, $HTTP_POST_VARS['id']);
          }
...
/* This is to add the distributors name (selected) in the top blue bar that
   shows the path of the user. like Top>>Store>>CNN etc. */
  }elseif (isset($HTTP_GET_VARS['distributors_id'])) {
     $distributors_query = tep_db_query("select distributors_name from " . TABLE_DISTRIBUTORS . " where distributors_id = '" . (int)$HTTP_GET_VARS['distributors_id'] . "'");
     if (tep_db_num_rows($distributors_query)) {
       $distributors = tep_db_fetch_array($distributors_query);
       $breadcrumb->add($distributors['distributors_name'], tep_href_link(FILENAME_DEFAULT, 'distributors_id=' . $HTTP_GET_VARS['distributors_id']));
     }
   }
   elseif (isset($HTTP_GET_VARS['producers_id'])) {
     $producers_query = tep_db_query("select producers_name from " . TABLE_PRODUCERS . " where producers_id = '" . (int)$HTTP_GET_VARS['producers_id'] . "'");
     if (tep_db_num_rows($producers_query)) {
       $producers = tep_db_fetch_array($producers_query);
       $breadcrumb->add($producers['producers_name'], tep_href_link(FILENAME_DEFAULT, 'producers_id=' . $HTTP_GET_VARS['producers_id']));
     }
   }
   elseif (isset($HTTP_GET_VARS['series_id'])) {
     $series_query = tep_db_query("select series_name from " . TABLE_SERIES . " where series_id = '" . (int)$HTTP_GET_VARS['series_id'] . "'");
     if (tep_db_num_rows($series_query)) {
       $series = tep_db_fetch_array($series_query);
       $breadcrumb->add($series['series_name'], tep_href_link(FILENAME_DEFAULT, 'series_id=' . $HTTP_GET_VARS['series_id']));
     }
   }
/*SYSTWEAK EDIT ENDS*/
...
// Header Tags Controller v1.0
  require(DIR_WS_FUNCTIONS . 'header_tags.php');
  require(DIR_WS_FUNCTIONS . 'clean_html_comments.php');

if (!$HTTP_SESSION_VARS['ref']) {
   tep_session_register('ref');
   if (($HTTP_GET_VARS['ref'] || $HTTP_POST_VARS['ref'])) {
     if ($HTTP_GET_VARS['ref']) $ref = $HTTP_GET_VARS['ref'];
     if ($HTTP_POST_VARS['ref']) $ref = $HTTP_POST_VARS['ref'];
	$HTTP_SESSION_VARS['ref'] = $ref;
	}
}
//B.O.F added by indianelite.com
if ($cart->count_contents() > 0) {
$products = $cart->get_products();
for ($i=0, $n=sizeof($products); $i<$n; $i++) {
$smart_holder[]=$products[$i]['price'];
}
$one_freeitem = min($smart_holder); //This is the final value that u want....minimum price value in cart
}
//E.O.F added by indianelite.com
require(DIR_WS_INCLUDES . 'add_ccgvdc_application_top.php');  // ICW CREDIT CLASS Gift Voucher Addittion

$check_c = tep_db_fetch_array(tep_db_query("select customers_id, iswholesale, disc1, disc2, shipping1, shipping2, nondistribution_percentage, distribution_percentage, customers_firstname, customers_password, customers_email_address, customers_default_address_id from " . TABLE_CUSTOMERS . " where customers_id = '" . (int)$customer_id . "'"));

		//echo "iswholesale : ".$check_c['iswholesale']."<br>";
        $whole['iswholesale'] = $check_c['iswholesale'];
        $whole['disc1'] = $check_c['disc1'];
        $whole['disc2'] = $check_c['disc2'];
        $whole['shipping1'] = $check_c['shipping1'];
        $whole['shipping2'] = $check_c['shipping2'];
        $whole['distribution_percentage'] = $check_c['distribution_percentage'];
        $whole['nondistribution_percentage'] = $check_c['nondistribution_percentage'];
	tep_session_register('whole');
	//print_r($_SESSION);

if (intval($ref)!=0){
	$curObject = tep_db_fetch_array(tep_db_query("select currency from affiliate_affiliate where affiliate_id='".$ref."'"));
	$currency = $curObject[currency];
	if ($currency=='') $currency = 'USD';
	tep_session_register('currency');
}
//echo session_id();

if (!tep_session_is_registered('IP')) {
   $IP = getenv("REMOTE_ADDR");
   tep_session_register('IP');
}

$LAST_VISITED = getenv('REQUEST_URI');
tep_session_register('LAST_VISITED');
if(isset($customer_id)){


$getorders=tep_db_query("SELECT orders_id from orders where customers_id = '" . $customer_id . "'");
//$oplist['9871']=true;
while($orderslist=tep_db_fetch_array($getorders)){

$getordersproducts=tep_db_query("SELECT products_id from orders_products where orders_id = '".$orderslist['orders_id']."'");
while($plist=tep_db_fetch_array($getordersproducts)){
$oplist[$plist['products_id']]=true;
}
