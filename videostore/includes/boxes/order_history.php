...
<?php
      ...
      $products_query = tep_db_query("select pd.products_id, pd.products_name, pd.products_name_prefix, pd.products_name_suffix, se.series_name, mt.products_media_type_name, p.series_id from " . TABLE_PRODUCTS . " p left join " . TABLE_SERIES . " se on (p.series_id = se.series_id) left join " . TABLE_MEDIA_TYPE . " mt on (p.products_media_type_id = mt.products_media_type_id), " . TABLE_PRODUCTS_DESCRIPTION . " pd where pd.products_id in (" . $product_ids . ") and language_id = '" . (int)$languages_id . "' order by products_name");
      while ($products = tep_db_fetch_array($products_query)) {
        $customer_orders_string .= '  <tr>' .
                                   '    <td class="infoBoxContents"><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products['products_id']) . '">' . '<b>' . $products['series_name'] . '</b><BR>' . $products['products_name_prefix'] . '&nbsp;<b>' . $products['products_name'] . '</b>&nbsp;' . $products['products_name_suffix'] . '-' . $products['products_media_type_name'] .
