...
***************************************/
	...
	class smtp{
		...
		**   timeout - 5 default The timeout in seconds for the call	Default: 30
		**             to fsockopen()
        ***************************************/
		...
		function &connect($params = array()){
			...
			if(!isset($this->status)){
				...
				if(is_resource($this->connection)){
					...
					$this->errors[] = 'Failed to connect to server: ('.$this->host.' - '.$this->port.' - '.$errno.' - '.$errstr.' - '.$this->timeout.')'.$errstr;
					$result = FALSE;
				}
				...
				if ($this->debug AND !$result){
					        ...
					        preg_replace(array("/\r/", "/\n/", "/#EOL#/"),
									...
									$this->errors[] = 'Connected to server: ('.$this->host.' - '.$this->port.' - '.$errno.' - '.$errstr.' - '.$this->timeout.')'.$errstr;
                                       $fd = @fopen("smtp.log", "a");
                                        if ($fd){
                                                ob_start();
				                var_dump($this->errors,
        ...
        ***************************************/
		...
		function auth(){
			if(is_resource($this->connection)
				...
				if ($this->debug){
					$this->errors[] = 'Auth sucessful: ('.$this->host.' - '.$this->port.' - '.$errno.' - '.$errstr.' - '.$this->timeout.')'.$error;
					   $fd = @fopen("smtp.log", "a");
						if ($fd){
								ob_start();
				var_dump($this->errors,
