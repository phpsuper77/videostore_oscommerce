...
// class constructor
	...
	if ($whole[iswholesale] == 1){
		$array_to_delete = array ("0" => "ot_coupon.php", "1"=>"ot_qty_discount.php", "2"=>"ot_gv.php");
		$this->modules = array_diff($this->modules, $array_to_delete);
	}
//var_dump($this->modules);
        ...
        //  if (($class <> "ot_coupon") && ($class <> "ot_gv"))           //Chages made here yesterday
      ...
      if (is_array($this->modules)) {
        ...
        while (list(, $value) = each($this->modules)) {
          ...
          if ($GLOBALS[$class]->enabled) {
            ...
            for ($i=0; $i<$size; $i++) {
              $output_string .= '              <tr>' . "\n" .
                                '                <td align="right" width="85%" class="main">' . $GLOBALS[$class]->output[$i]['title'] . '</td>' . "\n" .
                                '                <td align="right" width="15%" class="main">' . $GLOBALS[$class]->output[$i]['text'] . '</td>' . "\n" .
                                '              </tr>';
            }
          }
        }
            ...
            /* --- Code Start - by K.v.karthik -----*/
              if ($_SESSION['affiliate_id_banner'] == '')
			  {
				...
				if ( ($use_credit_string !='' ) || ($selection_string != '') ) {
           	  ...
           	  else if($_SESSION['affiliate_id_banner'] <> NULL)/* This code is Created For show the coupon box for entering the coupon */
           	  {
				...
				if ($aff_couponbox_fetch['affiliate_display_coupon_box'] == 'Y')
				{
					if ($selection_string =='') $selection_string = $GLOBALS[$class]->credit_selection();
