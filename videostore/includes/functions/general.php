...
// TABLES: products
  function tep_get_products_set_type($products_set_type_id) {

    $product_query = tep_db_query("select products_set_type_name from " . TABLE_SET_TYPE . " where products_set_type_id = '" . (int)$products_set_type_id . "'");
    $product = tep_db_fetch_array($product_query);

    return $product['products_set_type_name'];
  }

  function tep_get_products_series($series_id) {

    $product_query = tep_db_query("select series_name from " . TABLE_SERIES . " where series_id = '" . (int)$series_id . "'");
    $product = tep_db_fetch_array($product_query);
  ...
  function tep_get_products_name($product_id, $language = '') {
    ...
    function tep_get_products_name_prefix($product_id, $language = '') {
    global $languages_id;

    if (empty($language)) $language = $languages_id;

    $product_query = tep_db_query("select products_name_prefix from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$product_id . "' and language_id = '" . (int)$language . "'");
    $product = tep_db_fetch_array($product_query);

    return $product['products_name_prefix'];
  }

    function tep_get_products_name_suffix($product_id, $language = '') {
    global $languages_id;

    if (empty($language)) $language = $languages_id;

    $product_query = tep_db_query("select products_name_suffix from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$product_id . "' and language_id = '" . (int)$language . "'");
    $product = tep_db_fetch_array($product_query);
...
if (!defined('CHECKOUT_ONE_PAGE')) {
  function tep_get_tax_rate($class_id, $country_id = -1, $zone_id = -1) {
    global $customer_zone_id, $customer_country_id, $checkout_flag;

//CHANGES BY ITZY TO ACCOUNT FOR CITY AND STATE TAX RATE
	//echo "send to = ".$_SESSION['sendto']."<br>";  //debugging
	...
	if ($cust_entry_zone_id == '19')	
	{
		return 7.0000;		//THIS IS THE STATE + CITY TAX RATE.... CHANGE THIS TO YOUR OWN
	} //END IF
	//END OF CHANGES BY ITZY


if (($checkout_flag==true) and ($zone_id==18) and ($country_id==223)){
...
/* FUNCTIONAL VERSION OF tep_get_tax_rate.  The above function should be deleted */
  function tep_get_tax_rate($class_id, $country_id = -1, $zone_id = -1) {
...
// TABLES: tax_rates;
  ...
  //CHANGES BY ITZY TO ACCOUNT FOR CITY AND STATE TAX RATE
	//echo "send to = ".$_SESSION['sendto']."<br>";
	$customer_zip_query = tep_db_query("select entry_postcode, entry_zone_id from " . TABLE_ADDRESS_BOOK . " where address_book_id =  '" . $_SESSION['sendto'] . "'");
	$address_query = tep_db_fetch_array($customer_zip_query);
	$cust_zip_code = $address_query['entry_postcode'];
	$cust_entry_zone_id  = $address_query['entry_zone_id'];
...
// TABLES: address_format
  function tep_address_format($address_format_id, $address, $html, $boln, $eoln) {
    ...
    } elseif (isset($address['country']['title']) && tep_not_null($address['country']['title'])) {
...
//for Distributors
  function tep_get_distributors($distributors_array = '') {
    if (!is_array($distributors_array)) $distributors_array = array();

    $distributors_query = tep_db_query("select distributors_id, distributors_name from distributors order by distributors_name");
    while ($distributors = tep_db_fetch_array($distributors_query)) {
      $distributors_array[] = array('id' => $distributors['distributors_id'], 'text' => $distributors['distributors_name']);
    }

    return $distributors_array;
  }
//for Producers
  function tep_get_producers($producers_array = '') {
    if (!is_array($producers_array)) $producers_array = array();

    $producers_query = tep_db_query("select producers_id, producers_name from producers order by producers_name");
...
//for Series
  function tep_get_series($series_array = '') {
    ...
    $series_query = tep_db_query("select series_id, series_name from series order by series_name");
...
//for Video types
  function tep_get_videotypes($videotypes_array = '') {
    if (!is_array($videotypes_array)) $videotypes_array = array();

    $videotypes_query = tep_db_query("select products_video_format_id, products_video_format_name from products_video_formats order by products_video_format_name");
    while ($videotypes = tep_db_fetch_array($videotypes_query)) {
      $videotypes_array[] = array('id' => $videotypes['products_video_format_id'], 'text' => $videotypes['products_video_format_name']);
    }

    return $videotypes_array;
  }
...
// Return table heading with sorting capabilities
  function tep_create_sort_heading($sortby, $colnum, $heading) {
    ...
    if ($sortby) {
      ...
      $sort_suffix = (substr($sortby, 0, 1) == $colnum ? (substr($sortby, 1, 1) == 'a' ? '&nbsp;<img src="images/sort_up.png" width="18" height="10" border="0"></a>' : '&nbsp;<img src="images/sort_down.png" width="18" height="10" border="0"></a>') : '&nbsp;<img src="images/sort_up_down.png" width="18" height="10" border="0"></a>');
    }
...
//                    e.g. info@mytepshop.com
  ...
  function tep_mail($to_name, $to_email_address, $email_subject, $email_text, $from_email_name, $from_email_address) {
    ...
    if (EMAIL_USE_HTML == 'true') {
		...
		$domain = $domain[1];
		// if ($from_email_address == EMAIL_SMTP_USERNAME || strstr($from_email_address,$domain)){
		if ($from_email_address == EMAIL_SMTP_USERNAME){
...
/*==========Wholesale Price Function=========*/
  function tep_customer_price($price, $product_id, $type) {
    global $customer_id, $whole;

	if (intval($type) == 1){ 
		if (intval($whole[distribution_percentage])>0){
		return round(($price - ($price/100*intval($whole[distribution_percentage]))), 3);	
	}
}
	if (intval($type) != 1) {
		if (intval($whole[nondistribution_percentage])>0){
		return round(($price - ($price/100*intval($whole[nondistribution_percentage]))), 3);	
	}
   }
  ...
  function shippingLabel($distribution){
	global $whole;
	if ($whole['iswholesale'] == 1){
		if ($distribution == 1) {
			 if ($whole['shipping1'] == 1) 
				$ret = "<br/>Free<br/>Shipping";
			 else 
				$ret = "<br/>Plus<br/>Shipping";
		}

		if ($distribution == 0) {
			 if ($whole['shipping2'] == 1) 
				$ret = "<br/>Free<br/>Shipping";
			 else
			 	$ret = "<br/>Plus<br/>Shipping";
		}
	}
  return "<font color='blue'><b>".$ret."</b></font>";
  }

 function getCustomerName(){
	global $customer_id, $whole;

  if (intval($customer_id) == 0)  $str = $customer_id."Someone not logged in";
  else {
	$customer = tep_db_fetch_array(tep_db_query("select customers_firstname, customers_lastname from customers where customers_id=".$customer_id));
	$str = $customer[customers_firstname]." ".$customer[customers_lastname];
   }
	if ($whole[iswholesale]) $addon = '(Wholesaler)';
	return $str.$addon;
 }



$debug_logfile_path = DIR_FS_CATALOG.'logs/php_errors/'.date('YmdHis').'.log';
...
function user_log ($errno, $errmsg, $file, $line) {
  ...
  if ($errno == 1) {
    ...
    $log_file_name = DIR_FS_CATALOG.'logs/php_errors/'.date('YmdHis').'.log';
    $file = str_replace('/home/terrae2/public_html', '');
