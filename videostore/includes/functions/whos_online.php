...
*/
  ...
  function tep_update_whos_online() {
    global $customer_id, $spider_flag, $user_agent, $referer_url_real;
    ...
    if (tep_session_is_registered('customer_id')) {
      ...
      $wo_customer_id = '';
// $wo_full_name = 'Guest';
      $wo_full_name = (($spider_flag)? $user_agent : 'Guest');
    }
    ...
    $pos = strpos($wo_last_page_url, 'product_generator.php');
if ($pos === false) {
    $current_time = time();
...
tep_db_query("insert into " . TABLE_WHOS_ONLINE . " (customer_id, full_name, session_id, ip_address, time_entry, time_last_click, last_page_url, http_referer) values ('" . (int)$wo_customer_id . "', '" . tep_db_input($wo_full_name) . "', '" . tep_db_input($wo_session_id) . "', '" . tep_db_input($wo_ip_address) . "', '" . tep_db_input($current_time) . "', '" . tep_db_input($current_time) . "', '" . tep_db_input($wo_last_page_url) . "', '" . tep_db_input($referer_url_real) . "')");
    }
   }
  }
?>
