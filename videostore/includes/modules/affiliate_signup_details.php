...
?>
          ...
          <tr>
<input type="hidden" value="<?=$affiliate['affiliate_logo']?>" name="old_affiliate_logo" />
<?
if ($affiliate['affiliate_logo']=="") $affiliate['affiliate_logo'] = "spacer.gif";
?>
			<td class="main" colspan="2">
	<img src="../images/<?=$affiliate['affiliate_logo']?>" border="0"/>
			</td>
          </tr>
          <tr>

          <tr>
			<td class="main">&nbsp;Affiliate Logo</td>
			<td class="main">&nbsp;&nbsp;<input type="file" name="affiliate_logo" /></td>
          </tr>
          <tr>
		<td class="main">&nbsp;Order Notification</td>
		<?php
  if ( $error == 1) {
    	if ($HTTP_POST_VARS['affiliate_order_notification']=="on") echo "<td>&nbsp;&nbsp;Yes</td>"; else echo "<td>&nbsp;&nbsp;No</td>";
?>
	<input type="hidden" value="<?=$HTTP_POST_VARS['affiliate_order_notification']?>" name="affiliate_order_notification" />
<?
}	else{
			if ($affiliate['affiliate_order_notification'] == 'Y')
				echo "<td class=\"main\">&nbsp;<input type=checkbox name=affiliate_order_notification CHECKED></td>";
			else
				echo "<td class=\"main\">&nbsp;<input type=checkbox name=affiliate_order_notification></td>";
}
		?>
          </tr>
          <tr>
...
<?php 
	if ($error==1){
		echo $HTTP_POST_VARS[affiliate_notification_email];
?>
	<input type="hidden" name="affiliate_notification_email" value="<?=$HTTP_POST_VARS[affiliate_notification_email]?>" />
<?
}
	else
	echo tep_draw_input_field('affiliate_notification_email', $affiliate[affiliate_notification_email],'maxlength="64"'); ?> </td>
          </tr>
        </table></td>
      </tr>
    </table></td>
