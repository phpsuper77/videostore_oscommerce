...
//      }
 ...
 // ADDED BY TVS
                 $OrderPaymentMethod = $orderInfo['transaction']['paymentMethod'];
                 if ($OrderPaymentMethod == 'PayPal')  {
                 $OrderPaymentMethod = 'EBAY'; 
                 $orderStatus = 1;	
} elseif (($OrderPaymentMethod == 'Amazon.co.uk') && ($orderInfo['currency'] == 'GBP')) {
                 $OrderPaymentMethod = 'Amazon UK Marketplace'; 
                 $orderStatus = 1;                
} elseif (($OrderPaymentMethod == 'Amazon.com') && ($orderInfo['currency'] == 'USD')) {           
                 $OrderPaymentMethod = 'Amazon Marketplace'; 
                 $orderStatus = 1;              
} elseif (($OrderPaymentMethod == 'Amazon.ca') && ($orderInfo['currency'] == 'CAD')) {             
                 $OrderPaymentMethod = 'Amazon CA Marketplace'; 
                 $orderStatus = 1;              
} elseif (($OrderPaymentMethod == 'Amazon.de') && ($orderInfo['currency'] == 'EUR')) {             
                 $OrderPaymentMethod = 'Amazon DE Marketplace'; 
                 $orderStatus = 1;	
} elseif ($OrderPaymentMethod == 'Alibris') {	
                 $OrderPaymentMethod = 'Alibris';
                 $orderStatus = 1;
}  else { $orderStatus = 6; }
...
// deleted by TVS    'payment_method'                => $orderInfo['transaction']['paymentMethod'], 
            'payment_method'                => $OrderPaymentMethod, // Added by TVS
...
//            'date_purchased'                 => date('Y-m-d H:i:s', parse_iso8601_datetime($orderInfo['createdOn'])), // UTC to local time 
            ...
            'purchase_order_number' => $orderInfo['transaction']['transactionId'], // added by TVS
      ...
      $comment =    "AuctionBlox Order Import" . "\r\n"  .
                    ...
                    " Payment Currency  = " . $orderInfo['currency'] .  "\r\n" .
...
// ADDED BY TVS TO TEXT MESSAGE ORDER INFO
                 $to_name = 'TVS';               
                 $to_email_address  = '8133805407@txt.att.net';
                 $email_subject = 'Sale';
                 $from_email_name = 'AuctionBlox';
                 $headers = 'From: "AuctionBlox" <customerservice@travelvideostore.com>';
                 $from_email_address = 'customerservice@travelvideostore.com'; 
                 $email_text =    "AuctionBlox" . "\r\n"  .
                    " Payment = " . $orderInfo['transaction']['paymentMethod'] . "\r\n" . 
                    " Amount   = " . abxCurrencies::format($orderInfo['transaction']['amount'], $orderInfo['transaction']['currency']) . "\r\n" . 
                    " Currency = " . $orderInfo['currency'];

 //                abx_mail($to_name, $to_email_address, $email_subject, $email_text, $from_email_name, $from_email_address);

                 if (mail($to_email_address , $email_subject, $email_text, $headers)) {

                 } elseif (tep_mail($to_name, $to_email_address, $email_subject, $email_text, $from_email_name, $from_email_address)) {
                 }
...
// END ADDED BY TVS
    ...
    */
    function createStatusHistory ($orderId, $status, $comment, $timestamp) {
        ...
        $sql_data_array = array(
            'orders_id'         => $orderId,
            'orders_status_id'  => 1, // CHANGED BY TVS FROM $status to 1
...
// ADDED BY TVS
	$sql_query = "SELECT vendors_item_cost, 

vendors_product_payment_type, 
vendors_consignment, vendors_consignment_percentage, vendors_royalty , 
vendors_royalty_percentage FROM products_to_vendors where 
products_id=".$itemDetails['productId']." limit 0,1";

	$prod_vendor = $abxDatabase->fetch_row($sql_query);
  	switch($prod_vendor['vendors_product_payment_type']){
  		case "1":
  			$final_prod_vendor = $prod_vendor['vendors_item_cost'];
  			break;
  		case "2":
  			if (intval($prod_vendor['vendors_consignment'])==1) $final_prod_vendor = 
$prod_vendor['vendors_item_cost'];
  			if (intval($prod_vendor['vendors_consignment'])==2) $final_prod_vendor = 
($itemDetails['subtotal']/$itemDetails['quantity']/100)*$prod_vendor['vendors_consignment_percentage'];
  			if (intval($prod_vendor['vendors_consignment'])==3) $final_prod_vendor = 
($itemDetails['subtotal']/$itemDetails['quantity']/100)*$prod_vendor['vendors_consignment_percentage'];
  		break;
  		case "3":
  			if (intval($prod_vendor['vendors_royalty'])==1) 

$final_prod_vendor = $prod_vendor['vendors_item_cost'];
  			if (intval($prod_vendor['vendors_royalty'])==2) 

$final_prod_vendor = ($itemDetails['subtotal']/$itemDetails['quantity']/100)*$prod_vendor['vendors_royalty_percentage'];
...
//          'final_price'         => ($itemDetails['total']/$itemDetails['quantity']), CHANGED BY TVS
          'final_price'         => ($itemDetails['subtotal']/$itemDetails['quantity']),
          'products_tax'        => $taxRate,
          'products_quantity'   => $itemDetails['quantity'],
          'products_sale_type' =>  $prod_vendor['vendors_product_payment_type'], // ADDED BY TVS
          'products_item_cost' => $final_prod_vendor  // ADDED BY TVS
        );
...
*/
    ...
    function getNewPassword() {
        ...
        while(!preg_match("/[a-zA-Z0-9]/", $sChar)) {
          if ($sChar == @$sLastChar) {
            continue;
          }
