...
function process() {
global $PHP_SELF, $order, $currencies, $cc_id, $tax_rate_cal;

  if (!defined('CHECKOUT_KLUDGE')) {
    $od_amnt=$_SESSION['car_amt'];
  } else {
    $od_amnt = $order->info['subtotal'];
	}
	
	$od_amount = $this->calculate_credit($od_amnt);
	...
	if ($od_amount > 0) {
		...
		##########
		##Code to detect the shipping cost (Free Shipping) 20-Jul-05
		##########
		if (isset($cc_id) && ($cc_id <> ''))
		{
			...
			if ($coupon_type == 'S')
			{
				$order->info['shipping_cost'] = $order->info['shipping_cost'] - $od_amount;
				$order->info['total'] = $order->info['total'] - round($od_amount,2);

			}
			else
			{
				$order->info['giftwrap_cost']."##";
				...
				list($key, $value) = each($order->info['tax_groups']);  #####NEWLY INSERTED
				$tax_rate_cal = tep_get_tax_rate_from_desc($key);  #####NEWLY INSERTED
				$order->info['tax'] = ($order->info['subtotal'] * $tax_rate_cal) / 100;
				//$order->info['total'] = ($order->info['shipping_cost'] + $order->info['subtotal']);
				//$order->info['total'] = $order->info['total'] + $order->info['tax']; #####NEWLY INSERTED
//echo $order->info['total'];
			}
		}

		tep_session_register('disc_amt');
		$_SESSION['disc_amt']=$od_amount;

		//$order->info['total'] = $order->info['total'] - round($od_amount,2);

		##############get_error
		$coupon_query_desc=tep_db_query("select coupon_description from " . TABLE_COUPONS_DESCRIPTION . " where coupon_id='".$cc_id."'");
		$coupon_result_desc=tep_db_fetch_array($coupon_query_desc);
		$desc = $coupon_result_desc['coupon_description'];
		if ($coupon_type == 'P'){
		$desc .=" (".round($coupon_result_cnt[coupon_amount],2)."%)";
		}
		##############
		$this->output[] = array('title' => $desc  . ':','text' => '<b>-' . $currencies->format($od_amount) . '</b>', 'value' => "-".$od_amount); //Fred added hyphen
	}
...
global $HTTP_POST_VARS, $one_freeitem, $customer_id, $currencies, $cc_id;
...
// get some info from the coupon table
    ...
    if (tep_db_num_rows($date_query)==0) {
		...
		} elseif ($coupon_result['coupon_type']=='F') {
			list($newval)=mysql_fetch_array(mysql_query("select lowest_price from " . TABLE_COUPONS . " where coupon_id='".$cc_id."'"));
			if ($newval ==1) {
			$coupon_amount = $currencies->format($one_freeitem) . ' ';
			}else{
...
global $one_freeitem, $languages_id, $customer_id, $order, $cc_id;
//$cc_id = $_SESSION['cc_id']; //Fred commented out, do not use $_SESSION[] due to backward comp. Reference the global var instead.
 ...
 $coupon_desc = tep_db_fetch_array($coupon_desc_query);
    ...
    if (tep_db_num_rows($coupon_query) !=0 ) {
	  ...
	  $this->coupon_description = $coupon_desc['coupon_description'];
   ...
   $sql_no  =mysql_query("select lowest_price from coupons where coupon_id='".$cc_id."'"); 
list($coupon_getval)=mysql_fetch_array($sql_no);
	
	if ($coupon_getval ==1) {
$c_deduct = $one_freeitem;
