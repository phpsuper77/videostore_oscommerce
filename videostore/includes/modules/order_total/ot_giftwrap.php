  ...
  class ot_giftwrap {
    ...
    function process() {
	 ...
	 if (!is_object($ot_subtotal))
          	$order->info['subtotal'] += tep_calculate_tax($order->info['giftwrap_cost'], $giftwrap_tax);		
...
if (!is_object($ot_subtotal)){
	$order->info['total'] = $order->info['total']+$order->info['giftwrap_cost'];
	}
	else{
	$order->info['subtotal'] = $order->info['subtotal']+$order->info['giftwrap_cost'];
	$order->info['total'] = $order->info['total']+$order->info['giftwrap_cost'];
	}
}
//echo $order->info['total']."####".$order->info['subtotal']."<br/>";
