...
*/
  ...
  class ot_qty_discount {
    ...
    function process() {
      ...
      {
      	$_SESSION['qty_disc_tax']=1;
      	$tod_amount = $this->calculate_tax_effect($od_amount);
      ...
      if ($od_amount > 0) {
        ...
        $this->output[] = array('title' => sprintf(MODULE_QTY_DISCOUNT_FORMATED_TITLE, $title_ext),
                                'text' => sprintf(MODULE_QTY_DISCOUNT_FORMATED_TEXT, $currencies->format($this->deduction)),
                                //'value' => $od_amount);
...
//var_dump($order);
    	//if (($order->info['subtotal']+$order->info['tax']) <> $order->info['total'])
		//{
	      	//$tod_amount_temp = $this->calculate_tax_effect($od_amount);
	      	//$order->info['total'] -= $tod_amount_temp;
	      	//$order->info['tax'] -= $tod_amount_temp;
		//}

		#####When discount is not used#####################
		if (!(isset($_SESSION['od_amount'])))
		{
			$order->info['tax'] = tep_calculate_tax($order->info['subtotal'],$order->products[0][tax]);
			$order->info['total'] = $order->info['subtotal'] + $order->info['tax'] ;
		}
		###############################################################
		//print $order->info['total']."###".$order->info['subtotal']."###".$order->info['tax'];
    ...
    function getTitle($amount){
      global $order;	
      if ($amount > 0) {
        if (MODULE_QTY_DISCOUNT_RATE_TYPE == 'percentage') $title_ext = "<b>(".$this->calculate_rate($_SESSION['cart']->count_contents())."%)</b>";
	}
	return $title_ext;	
    }
