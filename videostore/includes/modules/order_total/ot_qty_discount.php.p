...
*/
  ...
  class ot_qty_discount {
	...
	if(!(tep_session_is_registered('qty_disc_tax')))
	{
		tep_session_register('qty_disc_tax');
	  ...
	  if ($od_amount > 0) {
		...
		####################################################################################################
		if (isset($order->products[0][tax]) && $order->products[0][tax]<>0)
		{
			$tax_disc=tep_calculate_tax($order->info['subtotal'],$order->products[0][tax]);
			$order->info['total'] = $order->info['subtotal']+$tax_disc;
		}
		####################################################################################################
      }
    }
