...
*/
  ...
  class ot_tax {
    ...
    function process() {
      ...
      while ((list($key, $value) = each($order->info['tax_groups']))&& ($_SESSION[whole][iswholesale] != 1))  {
        if ($value > 0) {
	...
	$tax_rate_cal = tep_get_tax_rate_from_desc($key);  
	$value = ($order->info['subtotal'] * $tax_rate_cal) / 100;
	$order->info['total'] = ($order->info['shipping_cost'] + $order->info['subtotal']); #####NEWLY INSERTED
