			...
			if ($get_result['coupon_minimum_order'] <= $this->get_order_total()) {
				if ($get_result['restrict_to_products'] || $get_result['restrict_to_categories']) {
					for ($i=0; $i<sizeof($order->products); $i++) {
											...
											if ($sub_cat_ids[$iii] == $cat_ids[$ii]) {
												if ($get_result['coupon_type'] == 'P') {
													...
													*/
													...
													$pod_amount = ($pr_c*10)/10*$c_deduct/100;
													$od_amount = $od_amount + $pod_amount;
												} else {
													$od_amount = $c_deduct;
												}
						...
						if ($get_result['coupon_type'] !='P') {
							$od_amount = $c_deduct;
							$od_amount = $amount * $get_result['coupon_amount'] / 100;/* NEWLY ADDED By KVKARTHIK Because of affiliate coupon discount Jan_23_2006*/
						} else {
