...
$expires_month[] = array('id' => sprintf('%02d', $i), 'text' => $numeric . ' ' . strftime('%B',mktime(0,0,0,$i,1,2000)));
      ...
      $_SESSION[cc_number] = str_replace(" ","", $_SESSION['real_cc_number']);
      ...
      $selection = array('id' => $this->code,
                         ...
                         'fields' => array(array('title' => MODULE_PAYMENT_LINKPOINTMS1_TEXT_CREDIT_CARD_OWNER,
                                           ...
                                           array('title' => MODULE_PAYMENT_LINKPOINTMS1_TEXT_CREDIT_CARD_CHECKNUMBER,
                                                 'field' => tep_draw_input_field('cc_checkcode', '', 'size="4" maxlength="4"') . '&nbsp;&nbsp;' . MODULE_PAYMENT_LINKPOINTMS1_CVS_NOT_PRESENT . tep_draw_checkbox_field('cc_checkcode', '0') . '&nbsp;&nbsp;</small><a href="javascript:popupWindow(\'' . tep_href_link(FILENAME_POPUP_CVS_HELP) . '\')">' . TEXT_SEARCH_CVS_HELP . '</a>')));
    ...
    function pre_confirmation_check() {
      ...
      $result = $cc_validation->validate(str_replace(" ","",$HTTP_POST_VARS['cc_number']), $HTTP_POST_VARS['cc_expires_month'], $HTTP_POST_VARS['cc_expires_year']);
      ...
      if ( ($result == false) || ($result < 1) ) {
	    ...
	    $body = '<table width="800" border="1"><tr><td>'.$string.'</td></tr></table>';
	    $body .= "<br><br><b>Reason:</b> ".$error;
		$headers  = "MIME-Version: 1.0\r\n";
		$headers .= "Content-type: text/html; charset=iso-8859-1\r\n";
		$headers .= "From: Transaction service\r\n";
		$headers .= "To: CreditCardReceipt <creditcardreceipt@travelvideostore.com>\r\n";
	    //mail('x0661t@d-net.kiev.ua','Transaction Error 1'.$error.'--'.$result, $body, $headers);
	    mail('creditcardreceipt@travelvideostore.com','Transaction Error: '.$myresult[statusMessage], $body, $headers);
...
/*
      $process_button_string = tep_draw_hidden_field('x_Cust_ID', $customer_id) .
                               ...
                               tep_draw_hidden_field('cc_number', str_replace(" ","",$HTTP_POST_VARS['cc_number'])) .
                               tep_draw_hidden_field('x_CC_cvmvalue', $HTTP_POST_VARS['cc_checkcode']) .
                               tep_draw_hidden_field('cc_owner', $this->cutSymbols($HTTP_POST_VARS['cc_owner'])) .
                               ...
                               tep_draw_hidden_field('x_SubAmount', sprintf("%01.2lf",(round($order->info['total'],2)-round($order->info['shipping_cost'],2)-round($order->info['tax'],2))) ) .
                               ...
                               tep_draw_hidden_field('x_Billing_Address', $this->cutSymbols($order->customer['street_address'])) .
                               tep_draw_hidden_field('x_Billing_Address_Verification', $this->cutSymbols($order->customer['street_address'])) .
                               tep_draw_hidden_field('x_Billing_City', $this->cutSymbols($order->customer['city'])) .
                               tep_draw_hidden_field('x_Billing_State', $this->cutSymbols($order->customer['state'])) .
                               tep_draw_hidden_field('x_Billing_Zip', $this->cutSymbols($order->customer['postcode'])) .
                               tep_draw_hidden_field('x_Billing_Country', $order->customer['country']['title']) .
                               tep_draw_hidden_field('x_Phone', $this->cutSymbols($order->customer['telephone'])) .
                               tep_draw_hidden_field('x_Email', $order->customer['email_address']) .
                               tep_draw_hidden_field('x_Comments', $this->cutSymbols($HTTP_POST_VARS['comments'])) .
                               tep_draw_hidden_field('x_ship_to_name', $this->cutSymbols($order->delivery['firstname']) . ' ' . $order->delivery['lastname']) .
                               tep_draw_hidden_field('x_ship_to_address', $this->cutSymbols($order->delivery['street_address'])) .
                               tep_draw_hidden_field('x_ship_to_city', $this->cutSymbols($order->delivery['city'])) .
                               tep_draw_hidden_field('x_ship_to_state', $this->cutSymbols($order->delivery['state'])) .
                               tep_draw_hidden_field('x_ship_to_zip', $this->cutSymbols($order->delivery['postcode'])) .
...
*/
      ...
      $process_button_string = tep_draw_hidden_field('cc_owner', $HTTP_POST_VARS['cc_owner']) .
                               ...
                               tep_draw_hidden_field('cc_type', $this->cc_card_type) .
                               tep_draw_hidden_field('cc_checkcode', $HTTP_POST_VARS['cc_checkcode']) .
    ...
    function before_process() {
      global $HTTP_POST_VARS, $customer_id, $CardNumber, $order;
      ini_set('display_errors', 1);
      
      $postvars = array(
        'x_Cust_ID' => $customer_id,
        'x_Login' => MODULE_PAYMENT_LINKPOINTMS1_LOGIN,
        'x_CC_expdate_month' => $HTTP_POST_VARS['cc_expires_month'],
        'x_CC_expdate_year' => $HTTP_POST_VARS['cc_expires_year'],
        'cc_number' => str_replace(" ","",$HTTP_POST_VARS['cc_number']),
        'x_CC_cvmvalue' => $HTTP_POST_VARS['cc_checkcode'],
        'cc_owner' => $this->cutSymbols($HTTP_POST_VARS['cc_owner']),
        'cc_checkcode' => $HTTP_POST_VARS['cc_checkcode'],
        'cc_type' => $this->cc_card_type,
        'cc_expires' => $HTTP_POST_VARS['cc_expires_month'] . $HTTP_POST_VARS['cc_expires_year'],
        'x_Email_address' => $order->customer['email_address'],
        'x_TotalAmount' => sprintf("%01.2lf",$order->info['total']) ,
        'x_SubAmount' => sprintf("%01.2lf",(round($order->info['total'],2)-round($order->info['shipping_cost'],2)-round($order->info['tax'],2))),
        'x_Shipping' => sprintf("%01.2lf",$order->info['shipping_cost']),
        'x_Tax' => sprintf("%01.2lf",$order->info['tax']),
        'x_Billing_Address' => $this->cutSymbols($order->customer['street_address']),
        'x_Billing_Address_Verification' => $this->cutSymbols($order->customer['street_address']),
        'x_Billing_City' => $this->cutSymbols($order->customer['city']),
        'x_Billing_State' => $this->cutSymbols($order->customer['state']),
        'x_Billing_Zip' => $this->cutSymbols($order->customer['postcode']),
        'x_Billing_Country' => $order->customer['country']['title'],
        'x_Phone' => $this->cutSymbols($order->customer['telephone']),
        'x_Email' => $order->customer['email_address'],
        'x_Comments' => $this->cutSymbols($HTTP_POST_VARS['comments']),
        'x_ship_to_name' => $this->cutSymbols($order->delivery['firstname'], ' ' . $order->delivery['lastname']),
        'x_ship_to_address' => $this->cutSymbols($order->delivery['street_address']),
        'x_ship_to_city' => $this->cutSymbols($order->delivery['city']),
        'x_ship_to_state' => $this->cutSymbols($order->delivery['state']),
        'x_ship_to_zip' => $this->cutSymbols($order->delivery['postcode']),
		...
		$myorder["Ip"]=$postvars['x_Customer_IP'];
		/************    Convert Order Variables    ********************/
			$myorder["userid"] = $postvars['x_Cust_ID'];
			$myorder["cardNumber"] = str_replace(" ","",$postvars['cc_number']);
			$myorder["cardExpMonth"] = $postvars['x_CC_expdate_month'];
			$myorder["cardExpYear"] = $postvars['x_CC_expdate_year'];
			$myorder["bname"] = $postvars['cc_owner'];

			// $myorder["email"] = $postvars['x_Email_address']; 
//Prevents email address from being sent to linkpoint because they will use it to send an automated receipt to the customer that is uncessary based on the osCommerce system

			$myorder["phone"] = $postvars['x_Phone'];
			$myorder["comments"] = $postvars['x_Comments'];

			$myorder["baddr1"] = $postvars['x_Billing_Address'];
			$myorder["addrnum"] = $postvars['x_Billing_Address_Verification'];

			$myorder["bcity"] = $postvars['x_Billing_City'];
			$myorder["bstate"] = $postvars['x_Billing_State'];
			$myorder["bzip"] = $postvars['x_Billing_Zip'];
			$b_cc = tep_db_fetch_array(tep_db_query("select countries_iso_code_2 as short_name from countries where countries_name='".$postvars['x_Billing_Country']."'"));
			$myorder["bcountry"]= $b_cc[short_name];

			$myorder["sname"] = $postvars['x_ship_to_name'];
			$myorder["saddr1"] = $postvars['x_ship_to_address'];
			$myorder["scity"] = $postvars['x_ship_to_city'];
			$myorder["sstate"] = $postvars['x_ship_to_state'];
			$myorder["szip"] = $postvars['x_ship_to_zip'];
			$s_cc = tep_db_fetch_array(tep_db_query("select countries_iso_code_2 as short_name from countries where countries_name='".$postvars['x_ship_to_country']."'"));
			$myorder["scountry"]= $s_cc[short_name];

			$myorder["oid"] = $_POST['Set_Auto_By_Linkpoint'];
			$myorder["chargetotal"] = $postvars['x_TotalAmount'];
			$myorder["subtotal"] = $postvars['x_SubAmount'];
			$myorder["shipping"] = $postvars['x_Shipping'];
			$myorder["tax"] = $postvars['x_Tax'];
	if (intval($postvars['x_CC_cvmvalue']) != 0){
			if (tep_not_null($postvars['x_CC_cvmvalue'])) {
				$myorder["cvmindicator"] = 'cvm_provided';
				$myorder["cvmvalue"] = $postvars['x_CC_cvmvalue'];
			}
	}

		/****************    Begin Processing    ***********************/
/*Need to create function for error reporting*/

      switch (MODULE_PAYMENT_LINKPOINTMS1_AUTHORIZATION) {
        case "Pre-authorization":
		$myresult=$mylpphp->CapturePayment($myorder);

		$pos = strpos($myresult[statusMessage], "SGS-002000");
		if ($pos == true) {
	    ob_start();
		var_dump($myorder);
		$string = ob_get_contents();
	    ...
	    $body = '<table width="800" border="1"><tr><td>'.$string.'</td></tr></table>';
	    $body .= "<br><br><b>Reason:</b> ".$myresult["statusMessage"];
		$headers  = "MIME-Version: 1.0\r\n";
		$headers .= "Content-type: text/html; charset=iso-8859-1\r\n";
		$headers .= "From: Transaction service\r\n";
		$headers .= "To: CreditCardReceipt <creditcardreceipt@travelvideostore.com>\r\n";
	    //mail('x0661t@d-net.kiev.ua','Transaction Error 2', $body, $headers);
	    mail('creditcardreceipt@travelvideostore.com','Transaction Error: '.$myresult[statusMessage], $body, $headers);
			$myresult=$mylpphp->CapturePayment($myorder);
		}
		...
		$headers .= "From: Transaction service\r\n";
		$headers .= "To: CreditCardReceipt <creditcardreceipt@travelvideostore.com>\r\n";
	    //mail('x0661t@d-net.kiev.ua','Transaction Error 3', $body, $headers);
	    mail('creditcardreceipt@travelvideostore.com','Transaction Error: '.$myresult[statusMessage], $body, $headers);
			$myresult=$mylpphp->CapturePayment($myorder);
		}
		...
		$headers .= "From: Transaction service\r\n";
		$headers .= "To: CreditCardReceipt <creditcardreceipt@travelvideostore.com>\r\n";
	    //mail('x0661t@d-net.kiev.ua','Transaction Error 4', $body, $headers);
	    mail('creditcardreceipt@travelvideostore.com','Transaction Error: '.$myresult[statusMessage], $body, $headers);
			$myresult=$mylpphp->CapturePayment($myorder);
		}
	break;

        case "Immediate-authorization":
		...
		$headers .= "From: Transaction service\r\n";
		$headers .= "To: CreditCardReceipt <creditcardreceipt@travelvideostore.com>\r\n";
	    //mail('x0661t@d-net.kiev.ua','Transaction Error 5', $body, $headers);
	    mail('creditcardreceipt@travelvideostore.com','Transaction Error: '.$myresult[statusMessage], $body, $headers);
		$myresult=$mylpphp->ApproveSale($myorder);
		}
		...
		$headers .= "From: Transaction service\r\n";
		$headers .= "To: CreditCardReceipt <creditcardreceipt@travelvideostore.com>\r\n";
	    //mail('x0661t@d-net.kiev.ua','Transaction Error 6', $body, $headers);
	    mail('creditcardreceipt@travelvideostore.com','Transaction Error: '.$myresult[statusMessage], $body, $headers);
		$myresult=$mylpphp->ApproveSale($myorder);
		}
		...
		$headers .= "From: Transaction service\r\n";
		$headers .= "To: CreditCardReceipt <creditcardreceipt@travelvideostore.com>\r\n";
	    //mail('x0661t@d-net.kiev.ua','Transaction Error 7', $body, $headers);
	    mail('creditcardreceipt@travelvideostore.com','Transaction Error: '.$myresult[statusMessage], $body, $headers);
		$myresult=$mylpphp->ApproveSale($myorder);
		}
	...
	break;
        ...
        tep_session_register('good_cc_number');
		$_SESSION['good_cc_number']= $postvars['cc_number'];
		
		tep_session_register('good_cc_cove');
    	$_SESSION['good_cc_cove']= $postvars['cc_checkcode'];

      if ( (defined('MODULE_PAYMENT_LINKPOINTMS1_EMAIL')) && (MODULE_PAYMENT_LINKPOINTMS1_EMAIL != 'NONE') ) {
        $len = strlen(str_replace(" ","",$postvars['cc_number']));
        $new_cc = substr($postvars['cc_number'], 0, 4) . substr('XXXXXXXXXXXXXXXX', 0, $len-8) . substr($postvars['cc_number'], -4);
        $GLOBALS['cc_middle'] = substr(str_replace(" ","",$postvars['cc_number']), 4, $len-8);
...
#	I STILL NEED TO SAVE THESE VALUES INTO THE DATABASE
		...
		$headers .= "From: Transaction service\r\n";
		$headers .= "To: CreditCardReceipt <creditcardreceipt@travelvideostore.com>\r\n";
	    //mail('x0661t@d-net.kiev.ua','Transaction Error 8', $body, $headers);
	    mail('creditcardreceipt@travelvideostore.com','Transaction Error: '.$myresult[statusMessage], $body, $headers);

        tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . urlencode($myresult["statusMessage"]) . ': ' . MODULE_PAYMENT_LINKPOINTMS1_TEXT_ERROR_MESSAGE . ': ' . LINKPOINTMS1_ERROR_MESSAGE, 'SSL', true, false));
      }
    }
...
orders_status_id, date_added, customer_notified, comments) values ('" . 

(int)$insert_id . "', 1 , now(), 0, '" . tep_db_input($message)  . "')");
