...
// class methods
    ...
    function selection() {
      return array('id' => $this->code,
                   'module' => $this->title .  '&nbsp;&nbsp;' . tep_image(DIR_WS_IMAGES . 'paypal_cc_logos.jpg', 'PayPal supports over 190 countries for secure checkout with your credit card, bank account, debit card world wide.', '175', '44'));
    }
    ...
    function process_button() {
	  ...
	  if ($order->info['total'] == $order->info['shipping_cost'])
							   {
								   $final_amount = 0.00;
								   $final_shipping = $order->info['shipping_cost'];
								   } else {
								$final_amount =    $order->info['total'] - $order->info['shipping_cost'];
								$final_shipping =  $order->info['shipping_cost'];
								   }
      ...
      $process_button_string = tep_draw_hidden_field('cmd', '_xclick') .
                               ...
                               tep_draw_hidden_field('amount', number_format($final_amount * $currencies->get_value($my_currency), $currencies->get_decimal_places($my_currency))) .
                               tep_draw_hidden_field('shipping', number_format($final_shipping * $currencies->get_value($my_currency), $currencies->get_decimal_places($my_currency))) .
