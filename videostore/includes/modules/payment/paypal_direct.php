...
// class constructor
    ...
    function before_process() { 
      global $HTTP_POST_VARS, $order, $sendto, $currencies, $order_totals, $checkout;
      
      unset($_SESSION['paypal_error_message']);
        ...
        switch ($case_product_type){
            ...
            case 'standard':
                $params = array('USER' => MODULE_PAYMENT_PAYPAL_DIRECT_API_USERNAME,
                                ...
                                'NOTIFYURL' => HTTP_SERVER . '/ext/modules/payment/paypal/paypal_direct_notify.php',
                                'BUTTONSOURCE' => 'osCommerce22_v20177_DP');
      ...
      foreach ($order->products as $product){
        ...
        $params['L_AMT'.$o] = $this->format_raw($product['final_price']);
        //$params['L_AMT'.$o] = round($product['final_price'], 2);
        $item_amount += $this->format_raw($product['final_price']) * $product['qty'];
        ...
        if (is_array($order_totals)) {
            //$checkout->die_error($order_totals[1]['code'], true);
        } else {
            //$checkout->die_error('not array', true);
        }
    ...
    for ($i=0, $n=sizeof($order_totals); $i<$n; $i++) {
        ...
        //if ($order_totals[$i]['code'] == 'ot_subtotal') {         
                //$item_amount = $order_totals[$i]['value'];
                
            if ($order_totals[$i]['code'] == 'ot_subtotal') {
                
                //$item_amount = $order_totals[$i]['value'];
                
            } elseif ($order_totals[$i]['code'] == 'ot_tax') {
                
                $params['TAXAMT'] = $this->format_raw($order_totals[$i]['value']);
                //$params['TAXAMT'] = round($order_totals[$i]['value'], 2);
                $tax_amount = $this->format_raw($order_totals[$i]['value']);
                
            } elseif ($order_totals[$i]['code'] == 'ot_qty_discount') {
                
                $params['L_NAME'.$o] = strip_tags($order_totals[$i]['title']);
                $params['L_QTY'.$o] = 1;
                $params['L_AMT'.$o] = $this->format_raw($order_totals[$i]['value']);
                //$params['L_AMT'.$o] = $order_totals[$i]['value'];
                //$item_amount += $order_totals[$i]['value'];
                $item_amount += $this->format_raw($order_totals[$i]['value']);
                $o++;

            } elseif ($order_totals[$i]['code'] == 'ot_coupon') {
            ...
            } elseif ($order_totals[$i]['code'] == 'ot_giftwrap') {
            ...
            } elseif ($order_totals[$i]['code'] == 'ot_shipping') {
                
                $params['SHIPPINGAMT'] = $this->format_raw($order_totals[$i]['value']);
                //$params['SHIPPINGAMT'] = $order_totals[$i]['value'];
                
            } elseif ($order_totals[$i]['code'] == 'ot_total') {
                //$params['AMT'] = $this->format_raw($order_totals[$i]['value']);
            }
              ...
              if (isset($order->info['shipping_cost'])) {
                $params['SHIPPINGAMT'] = $this->format_raw(round($order->info['shipping_cost'], 2));
              }
              if (isset($order->info['tax'])) {
                $params['TAXAMT'] = $this->format_raw(round($order->info['tax'], 2));
              }
              if (isset($order->info['subtotal'])) {
                $params['ITEMAMT'] = $this->format_raw(round($order->info['subtotal'], 2));
              }
              if (isset($order->info['total'])) {
                $params['AMT'] = $this->format_raw(round($order->info['total'], 2));
              }
              */
    ...
    $params['AMT'] = round($params['ITEMAMT'], 2) + round($params['SHIPPINGAMT'], 2) + round($params['TAXAMT'], 2);
        ...
        $fp = fopen(DIR_FS_CATALOG.'logs/paypal_logs/ppd-request-'.date('YmdHis').'.log', 'a');
        foreach ($params as $key => $value) {
            if ($key != 'ACCT') {
                fwrite($fp, $key.' - '.$value."\r\n");
            }
        ...
        $fp = fopen(DIR_FS_CATALOG.'logs/paypal_logs/ppd-resp-'.date('YmdHis').'.log', 'a');
        ...
        fclose($fp);
                ...
                // log the response
//@tep_mail("TVS", "donwyatt@travelvideostore.com", "Sale", $response . ' ' . $response_array, "TVS", "donwyatt@travelvideostore.com");
                $this->recordToLog($response_array, 'response');
                if (($response_array['ACK'] != 'Success') && ($response_array['ACK'] != 'SuccessWithWarning')) {
                    //$_SESSION['paypal_error_message'] = $response_array['L_LONGMESSAGE0'].' - '.$response_array['L_SHORTMESSAGE0'];
                ...
                $_SESSION['pp_corellation_id'] = $response_array['CORRELATIONID'];
            break;
            // recurring only
            case 'recurring':
                $no=0;
...
@tep_mail("TVS", "donwyatt@travelvideostore.com", "Sale", $response . ' ' . $response_array, "TVS", "donwyatt@travelvideostore.com");
            ...
            case 'mixed':
              ...
              // log the request
                ...
                parse_str($response, $response_array);
                // log the response
@tep_mail("TVS", "donwyatt@travelvideostore.com", "Sale", $response. ' '. $response_array, "TVS", STORE_OWNER_EMAIL_ADDRESS);
                $this->recordToLog($response_array, 'response');
                if (($response_array['ACK'] != 'Success') && ($response_array['ACK'] != 'SuccessWithWarning')) {
                    tep_redirect(tep_href_link(FILENAME_SHOPPING_CART, 'error_message=' . stripslashes($response_array['L_LONGMESSAGE0']), 'SSL'));
                }
...
@tep_mail("TVS", "donwyatt@travelvideostore.com", "Sale", $response . ' ' . $response_array, "TVS", "donwyatt@travelvideostore.com");
        ...
        //tep_redirect(tep_href_link('checkout.php', 'error_message=' . MODULE_PAYMENT_PAYPAL_DIRECT_ERROR_ALL_FIELDS_REQUIRED, 'SSL'));
      }
    ...
    function after_process() {
        ...
        $comments = 'Transaction ID: '.$_SESSION['pp_transaction_id']."\n".'Corellation ID: '.$_SESSION['pp_corellation_id'];
      ...
      tep_session_unregister('pp_corellation_id');
	  // remove 3dsecure
