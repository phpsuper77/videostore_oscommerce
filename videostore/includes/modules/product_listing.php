<script>
	...
	function getPdf(string, filter, sort){
		window.open('<?php echo tep_href_link("generate_catalog.php","");?>?search_string='+string+'&filter='+filter+'&sort='+sort);
	}
...
if (theForm.length>1){
	...
	window.location.href="<?=substr($REQUEST_URI,1)?>&per_page="+val;
	}
...
<?php
$listing_sql=str_replace("p.products_id,p.products_quantity", "p.products_id, p.products_quantity, p.adgregate", $listing_sql);
...
<tr><td><?php echo $pw_string; ?>
</td></tr>
</table>
<?php
 } //end added search enhancements mod

//if (getenv('REMOTE_ADDR') == '194.242.117.148') {
//echo $listing_sql;
//}
...
<?
    ...
    for ($i=0, $n=sizeof($search_keywords); $i<$n; $i++ ) {
      switch ($search_keywords[$i]) {
        ...
        case 'and':
        case 'or':
          $where_str_spec .= " " . $search_keywords[$i] . " ";
          break;
        default:
          $keyword = tep_db_prepare_input($search_keywords[$i]);
          $where_str_spec .= "(pd.products_head_keywords_tag like '%" . tep_db_input($keyword) . "%' or p.products_model like '%" . tep_db_input($keyword) . "%'";
          //if (isset($HTTP_GET_VARS['search_in_description']) && ($HTTP_GET_VARS['search_in_description'] == '1'))
		  $where_str_spec .= " or pd.products_description like '%" . tep_db_input($keyword) . "%'";
		  $where_str_spec .= " or p.products_upc like '%" . tep_db_input($keyword) . "%'";
		  $where_str_spec .= " or p.products_isbn like '%" . tep_db_input($keyword) . "%'";
          $where_str_spec .= ')';
          break;
      }
    }
...
<select name="per_page" onChange="redirection(this.value);" style='font-size:11px;'>
  ...
  <tr>
  	<td class="smallText" width="50%"><?php echo $listing_split->display_count(TEXT_DISPLAY_NUMBER_OF_PRODUCTS); ?></td>
    <td class="smallText" width="50%" align="right"><?php echo TEXT_RESULT_PAGE . ' ' . $listing_split->display_links(MAX_DISPLAY_PAGE_LINKS, tep_get_all_get_params(array('page', 'info', 'x', 'y'))); ?></td>
  </tr>
</table>
<?php
  }
  $list_box_contents = array();
    ...
    switch ($column_list[$col]) {
	   case 'PRODUCT_LIST_MEDIA': $lc_text = TABLE_HEADING_MEDIA; $lc_align = 'right'; break;
      	   case 'PRODUCT_LIST_MODEL': $lc_text = TABLE_HEADING_MODEL; $lc_align = ''; break;
      	   case 'PRODUCT_LIST_NAME': $lc_text = TABLE_HEADING_PRODUCTS; $lc_align = ''; break;
      	   case 'PRODUCT_LIST_MANUFACTURER': $lc_text = TABLE_HEADING_MANUFACTURER; $lc_align = ''; break;
           case 'PRODUCT_LIST_PRICE': $lc_text = TABLE_HEADING_PRICE; $lc_align = 'right'.""; break;
           case 'PRODUCT_LIST_QUANTITY': $lc_text = TABLE_HEADING_QUANTITY; $lc_align = 'right'; break;
      	   case 'PRODUCT_LIST_WEIGHT': $lc_text = TABLE_HEADING_WEIGHT; $lc_align = 'right'; break;
	   case 'PRODUCT_LIST_URL': $lc_text = TABLE_HEADING_URL; $lc_align = 'right'; break;
...
if ($sort_col==2 & $sort_order==a){
	...
	$text2="descendingly";
        ...
        $lc_text = '<a href="advanced_search_result.php?keywords='.$keywords.'&page=1&sort='.$sort2.'" title="Sort products '.$text2.' by Release Date" class="productListing-heading">Release Date</a><BR>&nbsp;<img src="images/sort'.$img2.'" width="18" height="10" border="0">&nbsp;';
        $lc_align = 'center';
 	   break;
	   case 'PRODUCT_LIST_BUY_NOW': $lc_text = 'Add to Cart<input type="checkbox" name="checkall" onclick="checkUncheckAll(this);"'; $lc_align = 'center'; break;
    }
...
$curr_date = mktime(date("H"),date("i"),date("s"),date("m"),date("d"),date("Y"));
        ...
        switch ($column_list[$col]) {
		  case 'PRODUCT_LIST_MEDIA':
			...
			{
					$lc_text = '<a href="javascript:void(0)" onClick=javascript:window.open("./includes/languages/english/vhs.html","VHS","width=400,height=400,scrollbars=yes")><img src=../images/media_type/' .	$listing['products_media_type_id'].'.gif border=0 alt="VHS - Click Here for More Info in VHS Formats"></img></a><BR>' . $listing['products_set_type_name'].'<br><b class="boxText">'.$listing['products_region_code_desc'].'<b>';
				}
			...
			$listing['products_media_type_id'].'.gif border=0 alt="DVD - Click Here for More Info on DVD Formats"></img></a><BR>' . $listing['products_set_type_name'].'<br><b class="boxText">'.$listing['products_region_code_desc'].'<b>';
				}
			...
			$listing['products_media_type_id'].'.gif border=0 alt="CD - Click Here for More Info on CD Formats"></img></a><BR>' . $listing['products_set_type_name'].'<br><b class="boxText">'.$listing['products_region_code_desc'].'<b>';
				}

				else
				{
					$lc_text = '<img src=../images/media_type/' . $listing['products_media_type_id'].'.gif border=0></img><BR>' . $listing['products_set_type_name'].'<br><b class="boxText">'.$listing['products_region_code_desc'].'<b>';
			}
            break;
...
if (($listing['products_date_available']!='') and ($date_available<$curr_date)) $preorder = ''; else $preorder = "<div><span style='font-family:comic sans MS, Verdana, Arial, sans-serif; font-weight:bold; font-size:13px; color:blue;'>PRE-ORDER</span><br/><span style='font-family:comic sans MS, Verdana, Arial, sans-serif; font-weight:bold; font-size:10px; color:#4D4D4D;'>Ships ".$part[1]."-".$part[2]."-".$part[0]."</span></div>";
            ...
            if (isset($HTTP_GET_VARS['manufacturers_id'])) {
				...
				//$listing_products_summary = ((strlen($products_combine) > 100) ? substr($products_combine, 0, 500) . '...' : $products_combine);

if (tep_not_null($listing['products_clip_url'])) {
if ($listing['adgregate'] ==1) {
$listing_products_summary = ''.substr($products_combine, 0, 500) .'...</b></td><td></i>'.$preorder.'<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $listing['products_id']) . '" title="More Video Info"><img src="images/moreinfo.gif" border="0" Align="Top" alt="More Info"</a>';
...
if ($listing['adgregate']==1) $listing_products_summary .= '<br/><a href="#" onclick="window.open(\'adgregate.php?id=' . ($listing[products_id]) . '\',\'_blank\',\'width=580,height=480\');return false" target=_new><img src="images/rentme.gif" border="0" alt="Can\'t wait for the DVD, Click here to rent it online" title="Can\'t wait for the DVD, Click here to rent it online"></a>';
...
if ($listing['adgregate']==1) $listing_products_summary .= '<br/><a href="#" onclick="window.open(\'adgregate.php?id=' . ($listing[products_id]) . '\',\'_blank\',\'width=580,height=480\');return false" target=_new><img src="images/video_clip.gif" border="0" alt="Can\'t wait for the DVD, Click here to rent it online" title="View Free Video Clip"></a>';
...
if ($listing['adgregate']==1) $listing_products_summary .= '<br/><a href="#" onclick="window.open(\'adgregate.php?id=' . ($listing[products_id]) . '\',\'_blank\',\'width=580,height=480\');return false" target=_new><img src="images/download.gif" border="0" alt="Can\'t wait for the DVD, Click here to rent it online" title="Can\'t wait for the DVD, Click here to rent it online"></a>';
  ...
  if (tep_not_null($listing['products_buy_download_link'])) $listing_products_summary .= '<br/><a href="'.$listing[products_buy_download_link].'" target="paypal"><img src="images/download_mp4.gif" border="0" alt="Can\'t wait for the DVD, click here to purchase the MP4 download file instantly" title="Can\'t wait for the DVD, click here to purchase the MP4 download file instantly"></a>';
				...
				$listing_products_year="";
              	$lc_text = '&nbsp;<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, ($cPath ? 'cPath=' . $cPath . '&' : '') . 'products_id=' . $listing['products_id']) . '"><b>' . $listing['series_name'] . '</b><BR>' . $listing['products_name_prefix'] . '&nbsp;<b>' . $listing['products_name'] . '</b>&nbsp;' . $listing['products_name_suffix'] . '</a>'.$listing_products_year.'<BR>'.$has_rights.'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<i>' . stripslashes($listing_products_summary) . '</i>' ;
            }
            break;
...
<?php  if (($listing['products_quantity'] <= '0') and ($listing['products_out_of_print'] == '0')) echo ''; else { echo $img.'<a onclick="getChecked('.$listing[products_id].', this); return false;" href="' . tep_href_link(FILENAME_FEATURED_PRODUCTS, tep_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $listing['products_id']) . '">' . tep_image_button('button_buy_now.gif', IMAGE_BUTTON_IN_CART) . '</a>'; ?></td><td valign=top style='padding-top:8px;'><?php echo tep_draw_checkbox_field('cartadd[]', $listing['products_id'],'', $params); }?></TD></tr>
		</table>
<?
...
<? echo tep_image_submit('button_in_cart_OLD.gif', IMAGE_BUTTON_IN_CART); ?></td>		
      ...
      <tr>
        <td height="15" colspan="2"><?php //echo tep_draw_separator('pixel_trans.gif', '100%', '10'); ?></td>
      </tr>
  <tr>
