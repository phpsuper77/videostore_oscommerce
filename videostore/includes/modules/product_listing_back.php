      ...
      for ($col=0, $n=sizeof($column_list); $col<$n; $col++) {
        ...
        switch ($column_list[$col]) {
          ...
          case 'PRODUCT_LIST_NAME':
			...
			$lc_text = '<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'manufacturers_id=' . $HTTP_GET_VARS['manufacturers_id'] . '&products_id=' . $listing['products_id']) . '"><b>' . $listing['series_name'] . '</b><BR>' . $listing['products_name_prefix'] . '&nbsp;<b>' . $listing['products_name'] . '</b>&nbsp;' . $listing['products_name_suffix'] . '</a><BR>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' . $listing_products_summary ;
            } else {
				$strip_tag_desc = strip_tags($listing['products_description'], '<b>');
				$products_combine = ($listing['products_byline'] . '&nbsp;&nbsp;' . $strip_tag_desc);	
				$listing_products_summary = ((strlen($products_combine) > 200) ? substr($products_combine, 0, 200) . '...' : $products_combine);
              $lc_text = '&nbsp;<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, ($cPath ? 'cPath=' . $cPath . '&' : '') . 'products_id=' . $listing['products_id']) . '"><b>' . $listing['series_name'] . '</b><BR>' . $listing['products_name_prefix'] . '&nbsp;<b>' . $listing['products_name'] . '</b>&nbsp;' . $listing['products_name_suffix'] . '</a><BR>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<i>' . stripslashes($listing_products_summary) . '</i>' ;
            }
            break;
          case 'PRODUCT_LIST_MANUFACTURER':
            $lc_align = '';
