...
// build the message content
      ...
      tep_mail(STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, EMAIL_OWNER_SUBJECT, EMAIL_OWNER_TEXT, $name, $links_contact_email);
 	  ...
 	  $cPath_submit = $HTTP_GET_VARS['cPath'];

  	  tep_redirect(tep_href_link(FILENAME_LINKS_SUBMIT_SUCCESS.'?cPath='.$cPath_submit, '', 'SSL'));
    }
  }
  ...
  if (isset($HTTP_GET_VARS['lPath'])) {
	//CHANGES DONE TO EXTRACT THE LPATH#################
  	...
  	if ($org <> 1)
	{
		$categories_reciprocal_link = HTTP_SERVER."/index.php/cPath/".$org;
	}



  	//$HTTP_GET_VARS["lPath"] = $Category_Path ;
	//########################################

  	$link_categories_query = tep_db_query("select link_categories_name from " . TABLE_LINK_CATEGORIES_DESCRIPTION . " where link_categories_id = '" . (int)$Category_Path . "' and language_id = '" . (int)$languages_id . "'");
    ...
    //****************CODE TO FETCH THE CATEGORY TITLE****************/
		$categories_heading_query = tep_db_query("select categories_heading_title from " . TABLE_CATEGORIES_DESCRIPTION . " where categories_id = '" . (int)$Category_Path . "' and language_id = '" . (int)$languages_id . "'");
		$categories_heading_value = tep_db_fetch_array($categories_heading_query);
		$categories_heading_title = $categories_heading_value['categories_heading_title'];
		$checking_category = $categories_heading_value['categories_heading_title'];
		$categories_heading_title .= "- Great for planning your vacation, learning more about the culture or simply experiencing what this unique place has to offer.";
	//****************CODE TO FETCH THE CATEGORY TITLE****************/
  }
...
<!-- body_text //-->
	<?php $Submit_Link = FILENAME_LINKS_SUBMIT.'?cPath='.$HTTP_GET_VARS['cPath'];?>
    <td width="100%" valign="top"><?php echo tep_draw_form('submit_link', tep_href_link($Submit_Link, '', 'SSL'), 'post', 'onSubmit="return check_form(submit_link);"') . tep_draw_hidden_field('action', 'process'); ?><table border="0" width="100%" cellspacing="0" cellpadding="0">
      <tr>
...
<?php
  ...
  if (isset($HTTP_GET_VARS['lPath'])) {
    $current_categories_id = $Category_Path;
...
?>
      ...
      <?php
	  	if ((isset($checking_category) && tep_not_null($checking_category)) && (isset($categories_reciprocal_link) && tep_not_null($categories_reciprocal_link)))
	  	{
	  ?>
			<tr>
				<td><?php echo tep_draw_separator('pixel_trans.gif', '100%', '10'); ?></td>
			...
			<tr>
				<td class="main"><b><?php echo "Links to Us"; ?></b></td>
			</tr>
			<tr>
				<td><table border="0" width="100%" cellspacing="1" cellpadding="2" class="infoBox">
					<tr class="infoBoxContents">
						...
						<?php
							$link_variable = "&lt;a href='$categories_reciprocal_link'&gt;$categories_heading_title&lt;/a&gt;";
							echo $link_variable ;
						?>
						</td>
					</tr>
