 ...
 # End of tsergiy's modification
	...
	function check_PHPVersion()
	{
		...
		if (($arrPhpVersion[0] == '5') && ($arrPhpVersion[1] == '2') && (intval(($arrPhpVersion[2]) >= 10))) {
			$result['check_issue_solution'] = 'Please use ionCube encoded version of the module with the current PHP version. Check an <a href="http://forums.zend.com/viewtopic.php?f=57&t=1655#p7109">article</a> for details';
		}
...
ERRMSG
	...
	{
		$matches = array();
		preg_match('(\\d+)', '$Revision: 53 $', $matches);
		return $matches[0];
	}
