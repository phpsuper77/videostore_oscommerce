...
if (isset($HTTP_GET_VARS['products_id'])) {
    if ($HTTP_GET_VARS['products_id'] == 'site') {
        $valid_product = true;
    } else {
...
if (isset($HTTP_GET_VARS['action']) && ($HTTP_GET_VARS['action'] == 'process')) {
    ...
    // UNCOMMENT NEXT LINE ONLY IF YOU WANT THE ADDED FEATURE OF HAVING THE MAIL SENT TO YOU! CHANGE THE yourname@yourstore.com
     tep_mail($to_name, 'donwyatt@travelvideostore.com', 'Tell A Friend', $email_body, $from_name, $from_email_address);
    
    //Inserting into TELL_A_FRIEND File
	$date = date ("Y-m-d");
	$referred_link = tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $HTTP_GET_VARS['products_id']);
	$rs_query = tep_db_query("Insert into " . TABLE_TELL_A_FRIEND . " values ('".mysql_real_escape_string($to_name)."','". $to_email_address ."','". mysql_real_escape_string($from_name) ."','". $from_email_address ."','". mysql_real_escape_string($email_body) ."','". $referred_link . "','". $date ."')");
          ...
          <tr class="infoBoxContents">
            <td><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr>
                ...
                <td>
<?php if ($HTTP_GET_VARS['products_id'] == 'site') {
