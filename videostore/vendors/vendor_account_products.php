...
if ($_POST[action]==save_rep){
  	$products_sel = tep_db_query("select * from ". TABLE_PRODUCTS_TO_VENDORS ." where vendors_id = ". $vendors_id);	
	while($products_fetch = tep_db_fetch_array($products_sel)){
		$sql_query = "update ".TABLE_PRODUCTS_TO_VENDORS ." set vendors_item_number='".$item_no[$products_fetch[products_id]]."', replenish='".$replenish[$products_fetch[products_id]]."' where products_id=".$products_fetch[products_id];
		//echo $sql_query;
		tep_db_query($sql_query);
	}
	echo '<script>window.location.href="vendor_account_products.php";</script>';
}
?>
...
<form action="vendor_account_products.php" method="post">
...
<?
				...
				while($products_fetch = tep_db_fetch_array($products_sel))
				{
					...
					echo "<td class=dataTableContent align=left valign=top><input type='text' name='item_no[".$products_fetch[products_id]."]' size='6' value='".$products_fetch[vendors_item_number]."'/></td>";
					...
					echo "<td class=dataTableContent align=left valign=top><input type='text' name='replenish[".$products_fetch[products_id]."]' size='2' value='".$products_fetch[replenish]."'/></td>";
