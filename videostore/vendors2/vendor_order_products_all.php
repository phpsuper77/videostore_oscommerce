...
elseif($products_fetch['products_sale_type']==2) echo 'Consignment';
elseif($products_fetch['products_sale_type']==3) echo 'Royalty';
elseif ($products_fetch['products_sale_type']==4) echo 'Duplication';
elseif ($products_fetch['products_sale_type']==5) echo 'Sample';
else echo 'Screener';
...
while($products_fetch = tep_db_fetch_array($products_sel_all)){
	...
	$total_qty_sold = $total_qty_sold + $products_fetch['products_quantity'];
					...
					echo "<tr><td></td></tr>";
$total_pay = tep_db_fetch_array(tep_db_query("SELECT sum(payment) AS sum FROM `vendor_payments` WHERE vendor_id=".$_SESSION['vendors_id']." and status='active'"));
					...
					echo "<tr><td></td></tr>";
$total = tep_db_fetch_array(tep_db_query("SELECT sum(products_item_cost*products_quantity) as sum FROM orders_products op, products_to_vendors pv, orders o, products_description pd WHERE o.orders_id = op.orders_id AND pd.products_id = pv.products_id AND op.products_id = pv.products_id AND pv.vendors_id =".$_SESSION['vendors_id']." ORDER  BY o.date_purchased DESC"));
...
?>
		             ...
		             <tr>
	              	...
	              	<tr><td><?php echo tep_draw_separator('pixel_trans.gif', 1, 10);?></td></tr>
<?php
	              echo "<tr><td colspan=10 align=right valign=top><a href='".FILENAME_VENDOR_PRODUCTS."'>".tep_image(DIR_WS_INCLUDES_LOCAL.'images/button_back.gif')."</td></tr>";
?>
        </table><br></td>
      </tr>
    </table></td>
